...
def resolve_stage_commands(
    ...
    """Resolve stage commands for the public Quarto-first pipeline."""
...
def run_pipeline_with_logs(
    ...
    logs_dir: Path,
    *,
    stage_commands: tuple[tuple[str, str], ...] = QUARTO_STAGE_COMMANDS,
    run_consistency_check: bool = False,
    consistency_command: str = "make quarto-pipeline",
) -> dict[str, Any]:
...
def load_and_validate_artifacts(
    ...
    *,
    ...
    qa_path = cohort_qa_dir(work_dir) / "qa_summary.json"
    ...
    analysis_results_dir = results_dir(work_dir)
    analysis_paths = [analysis_results_dir / name for name in ANALYSIS_EXPORT_FILENAMES]
