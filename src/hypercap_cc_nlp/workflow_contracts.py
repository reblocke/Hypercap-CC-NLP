...
QA_DIRNAME = "qa"
COHORT_QA_DIRNAME = "cohort"
RATER_QA_DIRNAME = "rater_agreement"
ANALYSIS_QA_DIRNAME = "analysis"
BASELINES_QA_DIRNAME = "baselines"
JUPYTER_BASELINES_DIRNAME = "jupyter"
RESULTS_DATE_ENV_VAR = "RESULTS_DATE"
...
def artifacts_dir(work_dir: Path) -> Path:
    """Return artifacts directory rooted at ``work_dir``."""
    return (work_dir / ARTIFACTS_DIRNAME).expanduser().resolve()


def resolve_results_date(
    results_date: str | None = None,
    *,
    run_dt: datetime | None = None,
) -> str:
    """Return canonical results date string in ``YYYY-MM-DD`` format."""
    if results_date:
        return results_date
    env_value = os.getenv(RESULTS_DATE_ENV_VAR, "").strip()
    if env_value:
        return env_value
...
def results_dir(
    ...
    *,
    ...
    """Return flat results directory rooted at ``work_dir``."""
    return (
        work_dir
        / RESULTS_DIRNAME
        / resolve_results_date(results_date, run_dt=run_dt)
    ).expanduser().resolve()


def qa_dir(work_dir: Path) -> Path:
    """Return QA artifact root rooted at ``work_dir``."""
    return artifacts_dir(work_dir) / QA_DIRNAME


def cohort_qa_dir(work_dir: Path) -> Path:
    """Return cohort QA artifact directory rooted at ``work_dir``."""
    return qa_dir(work_dir) / COHORT_QA_DIRNAME


def rater_qa_dir(work_dir: Path) -> Path:
    """Return rater QA artifact directory rooted at ``work_dir``."""
    return qa_dir(work_dir) / RATER_QA_DIRNAME


def analysis_qa_dir(work_dir: Path) -> Path:
    """Return analysis QA artifact directory rooted at ``work_dir``."""
    return qa_dir(work_dir) / ANALYSIS_QA_DIRNAME


def baselines_qa_dir(work_dir: Path) -> Path:
    """Return parity baseline directory rooted at ``work_dir``."""
    return qa_dir(work_dir) / BASELINES_QA_DIRNAME


def jupyter_baselines_dir(work_dir: Path) -> Path:
    """Return captured Jupyter baseline root rooted at ``work_dir``."""
    return baselines_qa_dir(work_dir) / JUPYTER_BASELINES_DIRNAME
