...
from .workflow_contracts import (
    ...
    jupyter_baselines_dir,
    cohort_qa_dir,
    results_dir,
    rater_qa_dir,
)

BASELINE_ROOT = Path("artifacts") / "qa" / "baselines" / "jupyter"
...
def _required_artifact_map(work_dir: Path) -> dict[str, Path]:
    base_results_dir = results_dir(work_dir)
    cohort_qc_dir = cohort_qa_dir(work_dir)
    rater_dir = rater_qa_dir(work_dir)
    required_relative_artifacts: tuple[Path, ...] = (
        ...
        cohort_qc_dir.relative_to(work_dir) / "qa_summary.json",
        rater_dir.relative_to(work_dir) / "R3_vs_NLP_join_audit.json",
        rater_dir.relative_to(work_dir) / "R3_vs_NLP_summary.txt",
        *(base_results_dir.relative_to(work_dir) / name for name in ANALYSIS_EXPORT_FILENAMES),
    )
...
def _build_snapshot(work_dir: Path) -> dict[str, Any]:
    ...
    qa_summary = _read_json(cohort_qa_dir(work_dir) / "qa_summary.json")
    ...
    analysis_signatures = {
        name: _analysis_file_signature(results_dir(work_dir) / name)
        for name in ANALYSIS_EXPORT_FILENAMES
    }
    rater_join_audit = _read_json(
        rater_qa_dir(work_dir) / "R3_vs_NLP_join_audit.json"
    )
...
def capture_jupyter_baseline(
    ...
    *,
    ...
    baseline_dir = jupyter_baselines_dir(work_dir) / f"{baseline_name}{_safe_label(label)}"
...
def resolve_baseline_dir(work_dir: Path, baseline: str) -> Path:
    ...
    root = jupyter_baselines_dir(work_dir)
    if baseline == "latest":
        if not root.exists():
            raise FileNotFoundError(
                "No baseline directory exists under artifacts/qa/baselines/jupyter."
            )
        candidates = [path for path in root.iterdir() if path.is_dir()]
        if not candidates:
            raise FileNotFoundError(
                "No baseline snapshots found under artifacts/qa/baselines/jupyter."
            )
        return max(candidates, key=lambda path: path.stat().st_mtime)
