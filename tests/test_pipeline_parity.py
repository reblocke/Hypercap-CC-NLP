...
TEST_RESULTS_DATE = "2026-04-18"
os.environ.setdefault("RESULTS_DATE", TEST_RESULTS_DATE)
...
def _write_required_workspace_artifacts(work_dir: Path) -> None:
    ...
    (cohort_qc_dir / "qa_summary.json").write_text(json.dumps(qa_summary))
...
def test_compare_missing_artifact_is_fail(tmp_path: Path) -> None:
    ...
    assert "/artifacts/qa/baselines/jupyter/" in baseline["baseline_dir"]

    (cohort_qa_dir(tmp_path) / "qa_summary.json").unlink()
...
def test_compare_analysis_sheet_shape_mismatch_is_fail(tmp_path: Path) -> None:
    ...
    target = results_dir(tmp_path) / target_name
