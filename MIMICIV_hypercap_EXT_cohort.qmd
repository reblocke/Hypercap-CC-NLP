...
1.  **Setup section** verifies credentials, datasets, and configuration.\
    If this fails, nothing else should be trusted.
2.  **SQL registry** defines every query template in one place.\
    This is where to look when you need to understand data extraction rules.
3.  **Cohort build sections** create two linked tables:
    -   an admission-level table (`hadm_id`) for enrollment logic,
    -   an ED-stay table (`ed_stay_id`) for downstream analysis exports.
4.  **Transform sections** derive clinically meaningful columns (gas thresholds, timing, source classes, vitals/anthropometrics, ICD unions).
5.  **QA/audit sections** generate explicit checks and dated audit files in `MIMIC tabular data/prior runs/`.
6.  **Output section** writes the canonical workbook used by the next pipeline stage (`MIMICIV all with CC.xlsx`).

## Summary of functionality

-   **Purpose:** Build the canonical cohort workbook used by downstream classifier/rater/analysis stages.
-   **Inputs:** BigQuery MIMIC-IV datasets (`hosp`, `icu`, auto-detected `ed`) + `.env` configuration.
-   **Canonical outputs:**
    -   `MIMIC tabular data/MIMICIV all with CC.xlsx = the dataset`
    -   `artifacts/qa/cohort/qa_summary.json = primary workbook-aligned QA summary`
-   **Run-archived outputs:** dated workbooks/parquet/audits under `MIMIC tabular data/prior runs/`.
-   **Key env vars:** `WORK_DIR`, `WORK_PROJECT`, `BQ_PHYSIONET_PROJECT`, `BQ_DATASET_HOSP`, `BQ_DATASET_ICU`, `BQ_DATASET_ED`, `WRITE_ARCHIVE_XLSX_EXPORTS`.
-   **Quality Assurance (QA) artifacts emitted:** timing/ventilation audits, gas-source audits, pCO2/HCO3 audits, anthropometric audits, contract report.

> **Assumptions** - You already configured BigQuery auth (`gcloud auth application-default login`) and created `.env` from `.env.example` per `README.md`.
> - The PhysioNet hosting project is `physionet-data`.
> - Datasets exist (e.g., `mimiciv_3_1_hosp`, `mimiciv_3_1_icu`, and an ED dataset such as `mimiciv_ed`).
...
This notebook is Stage 1 of the canonical Quarto pipeline; use `README.md` as the source of truth for full repository setup.

## 0. Prerequisites (one-time)

**Accounts & access** - PhysioNet access to MIMIC-IV on BigQuery; in BigQuery Console star project `physionet-data`.
- A Google Cloud **Project ID** with BigQuery API enabled (this is your **billing** project).

**CLI & environment** - Google Cloud SDK (gcloud) installed and on PATH.
- Python environment synced with `uv` (run `uv sync` at repo root).
- A project-local **`.env`** created from `.env.example`.
...
Then edit `.env` and fill your project-specific values.

**Minimum `.env` keys for this cohort notebook**
...
WORK_DIR=/path/to/Hypercap-CC-NLP  # optional but recommended
...
Install `gcloud` using the method appropriate for your OS (see Google Cloud SDK install docs).
Do not rely on a single package-manager command in this notebook.
...
# --- Imports & environment
import os, re, json, math, textwrap, sys, hashlib, subprocess
...
load_dotenv()  # Load repository .env so notebook behavior is environment-driven and reproducible.

WORK_DIR = Path(os.getenv("WORK_DIR", Path.cwd())).expanduser().resolve()  # Project root used for all relative paths.
DATA_DIR = WORK_DIR / "MIMIC tabular data"  # Canonical data output directory for workbook artifacts.
DATA_DIR.mkdir(parents=True, exist_ok=True)
PRIOR_RUNS_DIR = DATA_DIR / "prior runs"  # Date-stamped run outputs and audits.
PRIOR_RUNS_DIR.mkdir(parents=True, exist_ok=True)
RUN_TIMESTAMP = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")  # Run-level unique timestamp for archive filenames.
RUN_DATE = pd.Timestamp.now().strftime("%Y-%m-%d")  # Run-level date for dated audit outputs.
WRITE_ARCHIVE_XLSX_EXPORTS = os.getenv("WRITE_ARCHIVE_XLSX_EXPORTS", "0").strip() == "1"  # Optional archival XLSX exports.
COHORT_DEBUG_INVENTORY = os.getenv("COHORT_DEBUG_INVENTORY", "0").strip() == "1"  # Toggle for verbose inventory/debug writes.


def archive_xlsx_path(stem: str) -> Path:
    """Return a prior-runs XLSX path using the run-level timestamp."""
    return PRIOR_RUNS_DIR / f"{stem}_{RUN_TIMESTAMP}.xlsx"
...
# Helper group: robust IMV timing relative to the qualifying gas.
# These helpers use exact timestamps without rounding, buffers, or tolerance windows.

IMV_OBSERVED_SOURCE_VALUES = frozenset(
    {
...
IMV_QUALIFYING_GAS_ORDER_VALUES = frozenset(
    {
...
# Internal construction/QA field. This must never enter a cohort workbook.
...
def build_imv_source_provenance(frame: pd.DataFrame) -> dict[str, Any]:
    """Capture restricted untimed evidence from the internal source flag, never the stratum."""

    projection_sha256 = imv_source_projection_sha256(frame)
    if IMV_SOURCE_RECORD_NO_TIME_COLUMN not in frame.columns:
        raise KeyError("IMV source provenance requires the in-memory source-evidence flag.")
    evidence = pd.to_numeric(frame[IMV_SOURCE_RECORD_NO_TIME_COLUMN], errors="coerce")
    if not bool(evidence.isin([0, 1]).all()):
        raise ValueError("IMV source provenance requires a nonmissing binary source-evidence flag.")
    robust_columns = IMV_SOURCE_PROJECTION_COLUMNS[5:8]
    if bool((evidence.eq(1) & frame.loc[:, robust_columns].notna().any(axis=1)).any()):
        raise ValueError("Untimed IMV source evidence contradicts a reliable source timestamp.")
    members = sorted(
        [
            {"subject_id": int(subject_id), "hadm_id": int(hadm_id)}
            for subject_id, hadm_id in frame.loc[
                evidence.eq(1), ["subject_id", "hadm_id"]
            ].itertuples(index=False, name=None)
        ],
        key=lambda row: (row["hadm_id"], row["subject_id"]),
    )
    payload = {
        "schema_version": IMV_SOURCE_PROVENANCE_SCHEMA_VERSION,
        "admission_count": int(len(frame)),
        "source_projection_columns": list(IMV_SOURCE_PROJECTION_COLUMNS),
        "source_projection_sha256": projection_sha256,
        "untimed_evidence_admissions": members,
    }
    encoded = json.dumps(payload, sort_keys=True, separators=(",", ":"), allow_nan=False)
...
def write_imv_source_provenance(payload: dict[str, Any], target_path: Path) -> None:
    """Atomically write a private owner-only sidecar; it is not a result export."""
...
def prepare_archive_export_frame(
    frame: pd.DataFrame,
    *,
    export_name: str,
) -> pd.DataFrame:
    """Return a detached archive frame with internal-only IMV fields removed."""
    if not isinstance(frame, pd.DataFrame):
        raise TypeError(f"{export_name} archive source must be a pandas DataFrame")

    export_frame = frame.drop(
        columns=[IMV_SOURCE_RECORD_NO_TIME_COLUMN],
        errors="ignore",
    ).copy(deep=True)
    forbidden_columns = sorted(
        {IMV_SOURCE_RECORD_NO_TIME_COLUMN}.intersection(export_frame.columns)
    )
    if forbidden_columns:
        raise AssertionError(
            f"{export_name} archive retains internal-only columns: "
            f"{forbidden_columns}"
        )
    return export_frame


def normalize_mimic_item_label(value: Any) -> str:
    """Normalize a MIMIC d_items label for strict expected-label validation."""
    if value is None or pd.isna(value):
        return ""
    return re.sub(r"[^a-z0-9]+", " ", str(value).strip().casefold()).strip()


def select_first_observed_imv(
    *,
    ...
    """Return the earliest reliable observed IMV time and its exact source enum."""
    raw_values = {
    ...
    source_times = {
        source: pd.to_datetime(value, errors="coerce")
    ...
    earliest_sources = [
        source for source, timestamp in observed.items() if timestamp == earliest_time
    ]
...
def classify_imv_qualifying_gas_order(
    *,
    gas_positive: bool,
    ...
    source_record_imv_evidence_without_reliable_timestamp: bool = False,
) -> str:
    """Classify strict temporal ordering without a buffer or tolerance window."""
    if not gas_positive:
        return "not_applicable_no_qualifying_gas"

    qualifying_time = pd.to_datetime(qualifying_pco2_time, errors="coerce")
    if pd.isna(qualifying_time):
        return "timing_indeterminate"

    if not robust_imv_observed:
        # A source record without a usable time, or a legacy-only signal, is
        # insufficient to declare no observed IMV.
        if source_record_imv_evidence_without_reliable_timestamp or legacy_imv_flag:
            return "timing_indeterminate"
        return "no_observed_imv"

    observed_imv_time = pd.to_datetime(first_observed_imv_time, errors="coerce")
    if pd.isna(observed_imv_time):
        return "timing_indeterminate"

    if qualifying_time < observed_imv_time:
        return "qualifying_gas_before_imv"
    if observed_imv_time < qualifying_time:
        return "imv_before_qualifying_gas"
    return "timing_indeterminate"


def legacy_imv_timing_discordant(
    *,
    gas_positive: bool,
    qualifying_pco2_time: pd.Timestamp | None,
    robust_imv_observed: bool,
    ...
    """Flag robust-versus-legacy disagreement in IMV presence or comparable ordering.

    Presence discordance is flagged whenever robust evidence and the combined
    legacy IMV flag differ. Ordering discordance is evaluated only when the
    admission is gas-positive and both robust and legacy IMV timestamps are
    available; missing legacy timing is not itself treated as disagreement.
    """
...
# Helper group: local cohort contract helpers.
# Contract validation stays in the cohort notebook so the stage renders without src/ imports.
...
COHORT_REQUIRED_AUDIT_SUFFIXES = (
    ...
    "imv_qualifying_gas_timing_audit.csv",
...
def validate_cohort_contract(
    ...
    *,
    ...
    imv_gas_positive_n = 0
    imv_strata_reconciliation_n = 0
    imv_classification_mismatch_n = 0
    imv_source_mismatch_n = 0
    imv_hours_mismatch_n = 0
    imv_required_fields = {
        "first_derived_imv_starttime",
        "first_intubation_procedure_time",
        "first_invasive_ventilation_procedure_time",
    ...
    missing_imv_fields = sorted(imv_required_fields.difference(df.columns))
    if missing_imv_fields:
        findings.append(
            {
                "severity": "error",
                "code": "imv_qualifying_gas_fields_missing",
                "message": f"Missing required robust IMV timing fields: {missing_imv_fields}.",
            }
        )
    else:
        gas_positive = (
        ...
        imv_gas_positive_n = int(gas_positive.sum())
        order_values = df["imv_qualifying_gas_order"].astype("string")
        invalid_order_values = sorted(
            set(order_values.dropna().astype(str)).difference(
                IMV_QUALIFYING_GAS_ORDER_VALUES
            )
        )
        if invalid_order_values:
            findings.append(
                {
                    "severity": "error",
                    "code": "imv_qualifying_gas_order_invalid_values",
                    "message": (
                        "imv_qualifying_gas_order contains unsupported values: "
        ...
        invalid_source_values = sorted(
            set(
                df["first_observed_imv_source"]
                .astype("string")
                .dropna()
                .astype(str)
            ).difference(IMV_OBSERVED_SOURCE_VALUES)
        )
        if invalid_source_values:
            findings.append(
                {
                    "severity": "error",
                    "code": "first_observed_imv_source_invalid_values",
                    "message": (
                        "first_observed_imv_source contains unsupported values: "
        ...
        gas_invalid_n = int((gas_positive & ~order_values.isin(gas_order_values)).sum())
        nongas_invalid_n = int(
            (
                ~gas_positive
                & ~order_values.eq("not_applicable_no_qualifying_gas").fillna(False)
            ).sum()
        )
        imv_strata_reconciliation_n = int(
            order_values.loc[gas_positive].isin(gas_order_values).sum()
        )
        if gas_invalid_n or imv_strata_reconciliation_n != imv_gas_positive_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "imv_gas_positive_strata_do_not_reconcile",
                    "message": (
                        "Every gas-positive admission must have exactly one of the four IMV timing strata; "
                        f"gas_positive={imv_gas_positive_n}, classified={imv_strata_reconciliation_n}, "
                        f"invalid={gas_invalid_n}."
                    ),
                }
            )
        if nongas_invalid_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "imv_nongas_classification_invalid",
                    "message": (
                        "All admissions without a qualifying gas must be classified as "
                        "not_applicable_no_qualifying_gas; "
                        f"violations={nongas_invalid_n}."
                    ),
                }
            )
        ...
        source_times = df[source_time_columns].apply(
            lambda column: pd.to_datetime(column, errors="coerce")
        )
        robust_expected = source_times.notna().any(axis=1)
        robust_actual = (
            pd.to_numeric(df["robust_imv_observed"], errors="coerce")
            .fillna(0)
            .astype(int)
            .eq(1)
        )
        source_record_no_time_numeric = pd.to_numeric(
            df[IMV_SOURCE_RECORD_NO_TIME_COLUMN], errors="coerce"
        )
        source_record_no_time_invalid_n = int(
            (
                source_record_no_time_numeric.isna()
                | ~source_record_no_time_numeric.isin({0, 1})
            ).sum()
        )
        source_record_no_time_actual = (
            source_record_no_time_numeric.fillna(0).astype(int).eq(1)
        )
        if source_record_no_time_invalid_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "imv_source_record_no_time_flag_invalid",
                    "message": (
                        "Internal source-record IMV evidence-without-time flag must be "
                        f"nonmissing binary; violations={source_record_no_time_invalid_n}."
                    ),
                }
            )
        robust_mismatch_n = int((robust_expected != robust_actual).sum())
        if robust_mismatch_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "robust_imv_observed_mismatch",
                    "message": (
                        "robust_imv_observed must equal presence of any reliable source timestamp; "
                        f"violations={robust_mismatch_n}."
                    ),
                }
            )
        ...
        expected_selected = [
            ...
            for row in df[source_time_columns].itertuples(index=False)
        ]
        expected_first_time = pd.to_datetime(
            pd.Series([value[0] for value in expected_selected], index=df.index),
            errors="coerce",
        )
        actual_first_time = pd.to_datetime(df["first_observed_imv_time"], errors="coerce")
        source_record_no_time_inconsistent_n = int(
            (
                source_record_no_time_actual
                & (robust_actual | actual_first_time.notna())
            ).sum()
        )
        if source_record_no_time_inconsistent_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "imv_source_record_no_time_flag_inconsistent",
                    "message": (
                        "Source-record evidence without a reliable timestamp must have "
                        "robust_imv_observed=0 and missing first_observed_imv_time; "
                        f"violations={source_record_no_time_inconsistent_n}."
                    ),
                }
            )
        first_time_matches = (
            (expected_first_time.isna() & actual_first_time.isna())
            | expected_first_time.eq(actual_first_time).fillna(False)
        )
        first_time_mismatch_n = int((~first_time_matches).sum())
        expected_source = pd.Series(
            [value[1] for value in expected_selected],
            index=df.index,
            dtype="string",
        )
        actual_source = df["first_observed_imv_source"].astype("string")
        imv_source_mismatch_n = int(
            (~actual_source.eq(expected_source).fillna(False)).sum()
        )
        if first_time_mismatch_n or imv_source_mismatch_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "first_observed_imv_selection_mismatch",
                    "message": (
                        "First observed IMV time/source must equal the exact earliest reliable source; "
                        f"time_violations={first_time_mismatch_n}, "
                        f"source_violations={imv_source_mismatch_n}."
                    ),
                }
            )

        qualifying_times = pd.to_datetime(df["qualifying_pco2_time"], errors="coerce")
        legacy_flags = (
            pd.to_numeric(df["imv_flag"], errors="coerce")
            .fillna(0)
            .astype(int)
            .eq(1)
        )
        expected_orders = pd.Series(
            [
                classify_imv_qualifying_gas_order(
                    ...
                    first_observed_imv_time=imv_time,
                    legacy_imv_flag=bool(legacy_flag),
                    source_record_imv_evidence_without_reliable_timestamp=bool(
                        source_record_no_time_flag
                    ),
                )
                for (
                    gas_flag,
        ...
        imv_classification_mismatch_n = int(
            (~order_values.eq(expected_orders).fillna(False)).sum()
        )
        if imv_classification_mismatch_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "imv_qualifying_gas_classification_mismatch",
                    "message": (
                        "Stored IMV/gas ordering differs from strict timestamp classification in "
                        f"{imv_classification_mismatch_n} rows."
                    ),
                }
            )

        expected_preceded = pd.Series(pd.NA, index=df.index, dtype="boolean")
        expected_preceded.loc[expected_orders.eq("imv_before_qualifying_gas")] = True
        expected_preceded.loc[
            expected_orders.isin({"no_observed_imv", "qualifying_gas_before_imv"})
        ] = False
        expected_no_prior = pd.Series(pd.NA, index=df.index, dtype="boolean")
        expected_no_prior.loc[
            expected_orders.isin({"no_observed_imv", "qualifying_gas_before_imv"})
        ] = True
        expected_no_prior.loc[expected_orders.eq("imv_before_qualifying_gas")] = False
        for column_name, expected_boolean in (
            ("imv_preceded_qualifying_gas", expected_preceded),
            ("no_prior_observed_imv", expected_no_prior),
        ):
            actual_boolean = df[column_name].astype("boolean")
            matches = (
                (actual_boolean.isna() & expected_boolean.isna())
                | actual_boolean.eq(expected_boolean).fillna(False)
            )
            mismatch_n = int((~matches).sum())
            if mismatch_n:
                findings.append(
                    {
                        "severity": "error",
                        "code": f"{column_name}_mismatch",
                        "message": (
                            f"{column_name} differs from the prespecified nullable mapping in "
                            f"{mismatch_n} rows."
                        ),
                    }
                )
        ...
        expected_hours = (
            qualifying_times - actual_first_time
        ).dt.total_seconds() / 3600.0
        actual_hours = pd.to_numeric(
            df["hours_from_imv_to_qualifying_gas"], errors="coerce"
        )
        comparable_hours = expected_hours.notna() & actual_hours.notna()
        imv_hours_mismatch_n = int(
            (
                (expected_hours.notna() != actual_hours.notna())
                | (
                    comparable_hours
                    & ~np.isclose(
                        expected_hours.fillna(0.0),
                        actual_hours.fillna(0.0),
                        rtol=0.0,
                        atol=1e-12,
                    )
                )
        ...
        sign_violation_n = int(
            (
                (expected_orders.eq("qualifying_gas_before_imv") & actual_hours.ge(0))
                | (expected_orders.eq("imv_before_qualifying_gas") & actual_hours.le(0))
            ).sum()
        )
        exact_tie_not_indeterminate_n = int(
            (
                gas_positive
                & robust_actual
                & qualifying_times.notna()
                & actual_first_time.notna()
                & qualifying_times.eq(actual_first_time)
                & ~order_values.eq("timing_indeterminate").fillna(False)
            ).sum()
        )
        if imv_hours_mismatch_n or sign_violation_n or exact_tie_not_indeterminate_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "imv_qualifying_gas_hours_or_sign_mismatch",
                    "message": (
                        "IMV-to-qualifying-gas hours/sign or exact-tie classification drifted; "
                        f"value_violations={imv_hours_mismatch_n}, "
                        f"sign_violations={sign_violation_n}, "
                        f"tie_violations={exact_tie_not_indeterminate_n}."
                    ),
                }
            )

        expected_legacy_discordance = pd.Series(
            [
                legacy_imv_timing_discordant(
                    gas_positive=bool(gas_flag),
                    qualifying_pco2_time=gas_time,
                ...
                for gas_flag, gas_time, robust_flag, imv_time, legacy_flag, legacy_time in zip(
                    gas_positive,
                    qualifying_times,
                    robust_actual,
                    actual_first_time,
                    legacy_flags,
                    pd.to_datetime(df["first_imv_time"], errors="coerce"),
                    strict=False,
                )
            ],
            index=df.index,
        ...
        actual_legacy_discordance = pd.to_numeric(
            df["legacy_imv_timing_discordant"], errors="coerce"
        ).astype("Int64")
        legacy_discordance_mismatch_n = int(
            (~actual_legacy_discordance.eq(expected_legacy_discordance).fillna(False)).sum()
        )
        if legacy_discordance_mismatch_n:
            findings.append(
                {
                    "severity": "error",
                    "code": "legacy_imv_timing_discordant_mismatch",
                    "message": (
                        "legacy_imv_timing_discordant differs from its documented presence/order rule in "
                        f"{legacy_discordance_mismatch_n} rows."
                    ),
                }
            )
    ...
    return {
        ...
        "imv_gas_positive_n": imv_gas_positive_n,
        "imv_strata_reconciliation_n": imv_strata_reconciliation_n,
        "imv_classification_mismatch_n": imv_classification_mismatch_n,
        "imv_source_mismatch_n": imv_source_mismatch_n,
        "imv_hours_mismatch_n": imv_hours_mismatch_n,
        "imv_source_record_evidence_without_reliable_timestamp_n": (
            int(source_record_no_time_actual.sum()) if not missing_imv_fields else 0
        ),
...
OMR_RESULT_NAMES = ("bmi", "height", "weight")  # Canonical OMR metric names accepted from source records.
OMR_OUTPUT_COLUMNS = (
    "bmi_closest_pre_ed",
    "height_closest_pre_ed",
    "weight_closest_pre_ed",
)  # Primary anthropometric values written to cohort.

ANTHRO_UNIT_COLUMNS: dict[str, str] = {  # Unit columns paired to each anthropometric value.
    "bmi": "bmi_closest_pre_ed_uom",
    "height": "height_closest_pre_ed_uom",
    "weight": "weight_closest_pre_ed_uom",
}

ANTHRO_TIME_COLUMNS: dict[str, str] = {  # Timestamp columns paired to each anthropometric value.
    "bmi": "bmi_closest_pre_ed_time",
    "height": "height_closest_pre_ed_time",
    "weight": "weight_closest_pre_ed_time",
}

ANTHRO_VALUE_COLUMNS: dict[str, str] = {  # Value columns for each anthropometric metric.
    "bmi": "bmi_closest_pre_ed",
    "height": "height_closest_pre_ed",
    "weight": "weight_closest_pre_ed",
}

ANTHRO_CANONICAL_UNITS: dict[str, str] = {  # Target units after normalization/conversion.
    "bmi": "kg/m2",
    "height": "cm",
    "weight": "kg",
}

ANTHRO_SOURCE_PRIORITY: dict[str, int] = {  # Deterministic tie-break order when values are equally close in time.
    "ED": 0,
    "ICU": 1,
    "HOSPITAL": 2,
    "missing": 99,
...
ANTHRO_BMI_PAIR_WINDOW_HOURS = 24.0 * 7.0  # Max time gap (hours) allowed to pair height+weight for BMI backfill.
...
OMR_PROVENANCE_COLUMNS = (
    ...
    "anthro_hours_offset",
    "anthro_timing_basis",
)  # Provenance/quality columns for anthropometric selection diagnostics.

OMR_TIMING_TIERS = ("pre_ed_365", "post_ed_365", "missing")  # Allowed anthropometric timing categories.

EXPECTED_STRUCTURAL_NULL_FIELDS: tuple[str, ...] = ()  # Fields expected to be all-null by design (currently none).

PACO2_VALUE_UOM_PAIRS = (
    ("poc_abg_paco2", "poc_abg_paco2_uom"),
    ("poc_vbg_paco2", "poc_vbg_paco2_uom"),
)  # Required pCO2/uom column pairs used in uom consistency checks.

DEFAULT_VITALS_MODEL_RANGES: dict[str, tuple[float, float]] = {
    "ed_triage_hr": (20.0, 300.0),
    "ed_first_hr": (20.0, 300.0),
    "ed_triage_rr": (4.0, 80.0),
    "ed_first_rr": (4.0, 80.0),
}  # Plausibility bounds for ED vital-sign cleanup.
...
}  # Plausibility bounds for gas/lactate model-safe columns.

_NUMERIC_TOKEN_PATTERN = re.compile(r"(-?\d+(?:\.\d+)?)")  # Generic numeric extractor for mixed text fields.
_PAIN_FRACTION_PATTERN = re.compile(r"^\s*(-?\d+(?:\.\d+)?)\s*/\s*10\s*$")  # Parse pain forms like "7/10".
_PAIN_TEXT_NUMBER_PATTERN = re.compile(r"(-?\d+(?:\.\d+)?)")  # Fallback numeric extraction from free-text pain.
_ARTERIAL_HINT_PATTERN = re.compile(
    r"(arterial|abg|a[- ]?line|art line|\bart\b|\bartery\b)",
    re.I,
)  # Tokens implying arterial sample/source.
_VENOUS_HINT_PATTERN = re.compile(
    r"(venous|vbg|cvbg|mixed venous|central venous|\bven\b)",
    re.I,
)  # Tokens implying venous sample/source.
_PO2_LABEL_DENY_PATTERN_DEFAULT = re.compile(
    ...
    re.I,
)  # Denylist regex preventing PO2/O2/saturation fields from being misread as pCO2.

BLOOD_GAS_SPEC_PATH = WORK_DIR / "specs" / "blood_gas_itemids.json"  # Versioned source-of-truth itemid manifest.
CC_PSEUDO_MISSING_EXACT = {"-", "n", "___"}  # Exact ED chief-complaint placeholders treated as missing.
CC_PSEUDO_MISSING_UNDERSCORE_REGEX = re.compile(r"^_+$")  # Underscore-only placeholders treated as missing.


def _coerce_itemid_list(values: Sequence[Any], *, key_name: str) -> list[int]:
    """Cast manifest itemid values to deduplicated sorted integers with strict validation."""
    cleaned: list[int] = []
    for raw in values:
        try:
...
def load_blood_gas_itemid_manifest(path: Path) -> dict[str, Any]:
    ...
    Returns a normalized manifest dict used by SQL builders and QC checks.
    """
    if not path.exists():
    ...
    for section in ("lab", "icu", "denylist"):  # Required top-level schema.
    ...
    lab = payload["lab"]  # LAB branch itemids and fluid constraints.
    icu = payload["icu"]  # ICU/POC branch itemids and QC policy.
    denylist = payload["denylist"]  # Label deny tokens used for contamination safeguards.

    # LAB analyte itemids: pCO2/pO2/pH/HCO3 plus specimen-type itemids.
    ...
    if not lab_pco2:  # Hard requirement: at least one lab pCO2 itemid.
        raise ValueError("Blood-gas manifest requires at least one lab.pco2_itemids value.")
    if not lab_po2:  # Hard requirement: at least one lab pO2 itemid.
        raise ValueError("Blood-gas manifest requires at least one lab.po2_itemids value.")
    if not lab_ph:  # Hard requirement: at least one lab pH itemid.
        raise ValueError("Blood-gas manifest requires at least one lab.ph_itemids value.")
    if not lab_hco3:  # Hard requirement: at least one lab HCO3 itemid.
        raise ValueError("Blood-gas manifest requires at least one lab.hco3_itemids value.")
    if not lab_spec:  # Hard requirement: at least one lab specimen-type itemid.
        raise ValueError(
            "Blood-gas manifest requires at least one lab.specimen_type_itemids value."
        )

    blood_fluid_terms = [str(term).strip().lower() for term in lab.get("blood_fluid_terms", ["blood"])]  # Allowed blood-fluid labels for LAB gas filtering.
    blood_fluid_terms = sorted(set(term for term in blood_fluid_terms if term))
    if not blood_fluid_terms:
        blood_fluid_terms = ["blood"]

    # ICU analyte itemids: pCO2/pO2/pH/HCO3 plus subtype itemids and exclusions.
    ...
    icu_pco2_unknown = _coerce_itemid_list(
        icu.get("pco2_unknown_itemids", []),
        key_name="icu.pco2_unknown_itemids",
    )
    ...
    if icu_pco2_excluded:  # Apply explicit ICU pCO2 denylist exclusions.
        ...
        icu_pco2_unknown = sorted(set(icu_pco2_unknown).difference(excluded_set))
    if icu_po2_excluded:  # Apply explicit ICU pO2 denylist exclusions.
    ...
    if icu_pco2_abg and not set(icu_pco2_abg).issubset(set(icu_pco2)):  # ABG subtype ids must be in overall pCO2 ids.
        raise ValueError(
            "Blood-gas manifest icu.pco2_abg_itemids must be a subset of icu.pco2_itemids."
        )
    if icu_pco2_vbg and not set(icu_pco2_vbg).issubset(set(icu_pco2)):  # VBG subtype ids must be in overall pCO2 ids.
        raise ValueError(
            "Blood-gas manifest icu.pco2_vbg_itemids must be a subset of icu.pco2_itemids."
        )
    if icu_pco2_unknown and not set(icu_pco2_unknown).issubset(set(icu_pco2)):  # Unknown subtype ids must be in overall pCO2 ids.
        raise ValueError(
            "Blood-gas manifest icu.pco2_unknown_itemids must be a subset of icu.pco2_itemids."
        )
    if set(icu_pco2_unknown).intersection(set(icu_pco2_abg)) or set(  # Unknown subtype ids cannot overlap ABG/VBG subtype ids.
        icu_pco2_unknown
    ).intersection(set(icu_pco2_vbg)):
        raise ValueError(
            "Blood-gas manifest icu.pco2_unknown_itemids must be disjoint from "
            "icu.pco2_abg_itemids and icu.pco2_vbg_itemids."
        )
    if icu_po2_abg and not set(icu_po2_abg).issubset(set(icu_po2)):  # ABG subtype ids must be in overall pO2 ids.
        raise ValueError(
            "Blood-gas manifest icu.po2_abg_itemids must be a subset of icu.po2_itemids."
        )
    if icu_po2_vbg and not set(icu_po2_vbg).issubset(set(icu_po2)):  # VBG subtype ids must be in overall pO2 ids.
        raise ValueError(
            "Blood-gas manifest icu.po2_vbg_itemids must be a subset of icu.po2_itemids."
        )

    deny_tokens = [  # Normalized label deny tokens used for ICU item filtering.
        str(token).strip().lower()
        for token in denylist.get("label_tokens", [])
        if str(token).strip()
    ]
    allow_pattern_fallback = bool(icu.get("allow_pattern_fallback", True))  # Whether label-pattern fallback is allowed when manifest ids are sparse.
    inclusion_mode = str(icu.get("inclusion_mode", "validated_only")).strip().lower()  # ICU inclusion policy mode.
    ...
    pco2_qc_payload = icu.get("pco2_qc", {})  # Optional per-itemid QC policy block for ICU pCO2.
    ...
    qc_allowed_uoms = [  # Permitted unit strings for ICU pCO2 values.
    ...
    qc_label_include = [  # Tokens that must appear in ICU item labels to pass QC.
        str(value).strip().lower()
        for value in pco2_qc_payload.get("label_must_include_tokens", ["pco2"])
        if str(value).strip()
    ]
    qc_label_include = sorted(set(qc_label_include))
    qc_label_exclude = [  # Tokens that invalidate ICU item labels (likely non-pCO2 concepts).
        str(value).strip().lower()
    ...
    qc_median_min = float(pco2_qc_payload.get("median_min", 45.0))  # Lower bound for pCO2 median plausibility.
    qc_median_max = float(pco2_qc_payload.get("median_max", 80.0))  # Upper bound for pCO2 median plausibility.
    if qc_median_min > qc_median_max:
        raise ValueError(
            "Blood-gas manifest icu.pco2_qc median_min must be <= median_max."
        )

    manifest = {  # Normalized manifest object used downstream by SQL and audit logic.
        ...
        "icu": {
            ...
            "pco2_unknown_itemids": icu_pco2_unknown,
...
BLOOD_GAS_MANIFEST = load_blood_gas_itemid_manifest(BLOOD_GAS_SPEC_PATH)  # Validated manifest payload for this run.
LAB_PCO2_ITEMIDS = BLOOD_GAS_MANIFEST["lab"]["pco2_itemids"]  # LAB pCO2 itemids allowed for extraction.
LAB_EXCLUDED_PCO2_ITEMIDS = BLOOD_GAS_MANIFEST["lab"]["excluded_pco2_itemids"]  # LAB pCO2 itemids explicitly excluded.
LAB_PO2_ITEMIDS = BLOOD_GAS_MANIFEST["lab"]["po2_itemids"]  # LAB pO2 itemids allowed for extraction.
LAB_EXCLUDED_PO2_ITEMIDS = BLOOD_GAS_MANIFEST["lab"]["excluded_po2_itemids"]  # LAB pO2 itemids explicitly excluded.
...
LAB_PH_ITEMIDS = BLOOD_GAS_MANIFEST["lab"]["ph_itemids"]  # LAB pH itemids.
LAB_HCO3_ITEMIDS = BLOOD_GAS_MANIFEST["lab"]["hco3_itemids"]  # LAB HCO3 itemids.
LAB_SPECIMEN_TYPE_ITEMIDS = BLOOD_GAS_MANIFEST["lab"]["specimen_type_itemids"]  # LAB specimen-type itemids.
LAB_BLOOD_FLUID_TERMS = BLOOD_GAS_MANIFEST["lab"]["blood_fluid_terms"]  # Allowed LAB blood-fluid labels.
ICU_PCO2_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["pco2_itemids"]  # ICU pCO2 itemids allowed for extraction.
ICU_PCO2_ABG_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["pco2_abg_itemids"]  # ICU pCO2 itemids pre-typed as ABG.
ICU_PCO2_VBG_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["pco2_vbg_itemids"]  # ICU pCO2 itemids pre-typed as VBG.
ICU_PCO2_UNKNOWN_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["pco2_unknown_itemids"]  # ICU pCO2 itemids typed as unknown source.
ICU_EXCLUDED_PCO2_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["excluded_pco2_itemids"]  # ICU pCO2 exclusions.
ICU_PO2_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["po2_itemids"]  # ICU pO2 itemids allowed for extraction.
ICU_PO2_ABG_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["po2_abg_itemids"]  # ICU pO2 ABG subtype itemids.
ICU_PO2_VBG_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["po2_vbg_itemids"]  # ICU pO2 VBG subtype itemids.
ICU_EXCLUDED_PO2_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["excluded_po2_itemids"]  # ICU pO2 exclusions.
ICU_PH_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["ph_itemids"]  # ICU pH itemids.
ICU_HCO3_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["hco3_itemids"]  # ICU HCO3 itemids.
ICU_SPECIMEN_TYPE_ITEMIDS = BLOOD_GAS_MANIFEST["icu"]["specimen_type_itemids"]  # ICU specimen-type itemids.
ICU_ALLOW_PATTERN_FALLBACK = bool(BLOOD_GAS_MANIFEST["icu"]["allow_pattern_fallback"])  # Whether ICU label-pattern fallback is permitted.
ICU_INCLUSION_MODE = str(BLOOD_GAS_MANIFEST["icu"]["inclusion_mode"]).strip().lower()  # ICU inclusion mode (`validated_only`/`disabled`).
ICU_PCO2_QC = BLOOD_GAS_MANIFEST["icu"]["pco2_qc"]  # ICU pCO2 QC settings block.
ICU_LABEL_DENY_TOKENS = BLOOD_GAS_MANIFEST["denylist"]["label_tokens"]  # Global deny tokens for ICU label filtering.
...
POC_QC_MIN_VALID_N_WARN = 100  # Warning threshold for low valid-row counts per ICU pCO2 itemid.
POC_QC_MIN_VALID_N_CHECK = 50  # Minimum rows required to evaluate distribution-based ICU pCO2 QC.
POC_QC_P05_MIN = 10.0  # Lower bound for p05 plausibility in ICU pCO2 QC.
POC_QC_P50_MIN = 20.0  # Lower bound for p50 plausibility in ICU pCO2 QC.
POC_QC_P50_MAX = 80.0  # Upper bound for p50 plausibility in ICU pCO2 QC.
POC_QC_P95_MAX = 120.0  # Upper bound for p95 plausibility in ICU pCO2 QC.
POC_QC_PO2_CONTAM_P50_MIN = 75.0  # p50 cutoff suggesting probable PO2 contamination.
POC_QC_PO2_CONTAM_P25_MIN = 60.0  # p25 cutoff paired with p50 for PO2 contamination heuristic.
POC_QC_OUT_OF_RANGE_REMOVED_RATE_WARN = 0.001  # Warning threshold for removed out-of-range ICU pCO2 rows.
POC_QC_OUT_OF_RANGE_REMOVED_RATE_FAIL = 0.005  # Fail threshold for removed out-of-range ICU pCO2 rows.
POC_QC_OUT_OF_RANGE_REMOVED_N_FAIL = 25  # Absolute-count fail threshold for removed out-of-range ICU pCO2 rows.

LAB_PCO2_SQL_ARRAY = _sql_int_array(LAB_PCO2_ITEMIDS)  # BigQuery array literal for LAB pCO2 itemids.
LAB_PO2_SQL_ARRAY = _sql_int_array(LAB_PO2_ITEMIDS)  # BigQuery array literal for LAB pO2 itemids.
LAB_PH_SQL_ARRAY = _sql_int_array(LAB_PH_ITEMIDS)  # BigQuery array literal for LAB pH itemids.
LAB_HCO3_SQL_ARRAY = _sql_int_array(LAB_HCO3_ITEMIDS)  # BigQuery array literal for LAB HCO3 itemids.
LAB_SPECIMEN_SQL_ARRAY = _sql_int_array(LAB_SPECIMEN_TYPE_ITEMIDS)  # BigQuery array literal for LAB specimen-type itemids.
ICU_PCO2_SQL_ARRAY = _sql_int_array(ICU_PCO2_ITEMIDS)  # BigQuery array literal for ICU pCO2 itemids.
ICU_PCO2_ABG_SQL_ARRAY = _sql_int_array(ICU_PCO2_ABG_ITEMIDS)  # BigQuery array literal for ICU ABG pCO2 subtype itemids.
ICU_PCO2_VBG_SQL_ARRAY = _sql_int_array(ICU_PCO2_VBG_ITEMIDS)  # BigQuery array literal for ICU VBG pCO2 subtype itemids.
ICU_PCO2_UNKNOWN_SQL_ARRAY = _sql_int_array(ICU_PCO2_UNKNOWN_ITEMIDS)  # BigQuery array literal for ICU unknown-source pCO2 subtype itemids.
ICU_PO2_SQL_ARRAY = _sql_int_array(ICU_PO2_ITEMIDS)  # BigQuery array literal for ICU pO2 itemids.
ICU_PO2_ABG_SQL_ARRAY = _sql_int_array(ICU_PO2_ABG_ITEMIDS)  # BigQuery array literal for ICU ABG pO2 subtype itemids.
ICU_PO2_VBG_SQL_ARRAY = _sql_int_array(ICU_PO2_VBG_ITEMIDS)  # BigQuery array literal for ICU VBG pO2 subtype itemids.
ICU_PH_SQL_ARRAY = _sql_int_array(ICU_PH_ITEMIDS)  # BigQuery array literal for ICU pH itemids.
ICU_HCO3_SQL_ARRAY = _sql_int_array(ICU_HCO3_ITEMIDS)  # BigQuery array literal for ICU HCO3 itemids.
ICU_SPECIMEN_SQL_ARRAY = _sql_int_array(ICU_SPECIMEN_TYPE_ITEMIDS)  # BigQuery array literal for ICU specimen-type itemids.
LAB_BLOOD_FLUID_SQL_LIST = ", ".join(f"'{term}'" for term in LAB_BLOOD_FLUID_TERMS)  # SQL IN-list for accepted LAB blood-fluid labels.
...
)  # Regex used to reject ICU labels that imply non-pCO2 concepts.
...
def normalize_temperature_to_f(temp: pd.Series) -> pd.DataFrame:
    ...
    Rules:
    - values in ``[25, 45]`` are treated as Celsius-like and converted to Fahrenheit,
    - all other numeric values are treated as Fahrenheit already,
    - cleaned output is valid only for Fahrenheit ``[50, 115]``.
    """
    numeric = pd.to_numeric(temp, errors="coerce")
    celsius_like = numeric.between(25.0, 45.0, inclusive="both")
    temp_f = numeric.where(~celsius_like, numeric * 9.0 / 5.0 + 32.0)
    out_of_range = temp_f.notna() & ~temp_f.between(50.0, 115.0, inclusive="both")
...
def clean_pain_score(pain: pd.Series) -> pd.DataFrame:
    """Parse pain scores to a numeric 0–10 scale with conservative salvage rules."""
    pain_fraction_pattern = globals().get(
        "_PAIN_FRACTION_PATTERN", r"^\s*(-?\d+(?:\.\d+)?)\s*/\s*10\s*$"
    )
    pain_text_number_pattern = globals().get(
        "_PAIN_TEXT_NUMBER_PATTERN", r"(-?\d+(?:\.\d+)?)"
    )
    raw_series = pain.astype("string")
    normalized = raw_series.fillna("").str.strip()

    numeric_direct = pd.to_numeric(normalized, errors="coerce")
    fraction_numeric = pd.to_numeric(
        normalized.str.extract(pain_fraction_pattern, expand=False),
        errors="coerce",
    )
    text_numeric = pd.to_numeric(
        normalized.str.extract(pain_text_number_pattern, expand=False),
        errors="coerce",
    )

    parsed_from_fraction = numeric_direct.isna() & fraction_numeric.notna()
    numeric = numeric_direct.where(~parsed_from_fraction, fraction_numeric)
    parsed_from_text_numeric = numeric.isna() & text_numeric.notna()
    numeric = numeric.where(~parsed_from_text_numeric, text_numeric)
    ...
    scaled_numeric = numeric.where(~scaled_from_hundred, numeric / 10.0)

    sentinel = scaled_numeric.eq(13.0)
    out_of_range = (
        scaled_numeric.notna()
        & (~sentinel)
        & ((scaled_numeric < 0.0) | (scaled_numeric > 10.0))
    )
    non_numeric_set_na = normalized.ne("") & scaled_numeric.isna()
    clean = scaled_numeric.where(~(sentinel | out_of_range))
    return pd.DataFrame(
        {
            "pain_clean": clean,
            "pain_is_sentinel_13": sentinel.astype("boolean"),
            "pain_out_of_range": out_of_range.astype("boolean"),
            "pain_parsed_from_fraction": parsed_from_fraction.astype("boolean"),
            "pain_parsed_from_text_numeric": parsed_from_text_numeric.astype("boolean"),
            "pain_non_numeric_set_na": non_numeric_set_na.astype("boolean"),
        }
    )
...
def clean_o2sat(o2sat: pd.Series) -> pd.DataFrame:
    ...
    gt_100 = numeric > 100
    out_of_range = numeric.notna() & ((numeric < 0) | gt_100)
...
def canonicalize_cc_for_inclusion(series: pd.Series) -> pd.Series:
    """Normalize ED chief-complaint strings for deterministic inclusion filtering."""
    canonical = series.astype("string").fillna("").str.strip().str.lower()
    canonical = canonical.str.replace("–", "-", regex=False).str.replace("—", "-", regex=False)
    return canonical.str.replace(r"\s+", " ", regex=True)


def chief_complaint_inclusion_mask(series: pd.Series) -> pd.Series:
    """Return rows with meaningful ED chief complaint content (pseudo-missing excluded)."""
    canonical = canonicalize_cc_for_inclusion(series)
    pseudo_missing = (
        canonical.eq("")
        | canonical.isin(CC_PSEUDO_MISSING_EXACT)
        | canonical.str.match(CC_PSEUDO_MISSING_UNDERSCORE_REGEX, na=False)
    )
    return (~pseudo_missing).astype(bool)
    ...
    *,
    ...
    for prefix in ("ed_triage", "ed_first"):
        ...
        if pain_raw_col in updated.columns:
            ...
            pain_fraction_col = f"{prefix}_pain_parsed_from_fraction"
            pain_text_col = f"{prefix}_pain_parsed_from_text_numeric"
            pain_non_numeric_col = f"{prefix}_pain_non_numeric_set_na"
            ...
            updated[pain_fraction_col] = pain_clean["pain_parsed_from_fraction"]
            updated[pain_text_col] = pain_clean["pain_parsed_from_text_numeric"]
            updated[pain_non_numeric_col] = pain_clean["pain_non_numeric_set_na"]
            updated[f"{prefix}_pain_model"] = updated[pain_clean_col]
            pain_outlier = (
                updated[pain_sentinel_col].fillna(False)
                | updated[pain_range_col].fillna(False)
                | updated[pain_non_numeric_col].fillna(False)
            )
    ...
    specs = [
        ...
        },
        {
            ...
            "flags": [
                "ed_triage_pain_is_sentinel_13",
                "ed_triage_pain_out_of_range",
                "ed_triage_pain_non_numeric_set_na",
                "ed_triage_pain_parsed_from_fraction",
                "ed_triage_pain_parsed_from_text_numeric",
            ],
            "extra_cols": [],
        },
        {
            ...
            "flags": [
                "ed_first_pain_is_sentinel_13",
                "ed_first_pain_out_of_range",
                "ed_first_pain_non_numeric_set_na",
                "ed_first_pain_parsed_from_fraction",
                "ed_first_pain_parsed_from_text_numeric",
            ],
            "extra_cols": ["ed_first_vitals_time"],
        },
        {
...
VITAL_OUTLIER_AUDIT_SPECS: tuple[dict[str, Any], ...] = (
    {"vital_name": "ed_triage_temp", "raw_col": "ed_triage_temp", "clean_col": "ed_triage_temp_model", "lo": 50.0, "hi": 115.0},
    {"vital_name": "ed_first_temp", "raw_col": "ed_first_temp", "clean_col": "ed_first_temp_model", "lo": 50.0, "hi": 115.0},
    {"vital_name": "ed_triage_hr", "raw_col": "ed_triage_hr", "clean_col": "ed_triage_hr_model", "lo": 20.0, "hi": 300.0},
    {"vital_name": "ed_first_hr", "raw_col": "ed_first_hr", "clean_col": "ed_first_hr_model", "lo": 20.0, "hi": 300.0},
    {"vital_name": "ed_triage_rr", "raw_col": "ed_triage_rr", "clean_col": "ed_triage_rr_model", "lo": 4.0, "hi": 80.0},
    {"vital_name": "ed_first_rr", "raw_col": "ed_first_rr", "clean_col": "ed_first_rr_model", "lo": 4.0, "hi": 80.0},
    {"vital_name": "ed_triage_o2sat", "raw_col": "ed_triage_o2sat", "clean_col": "ed_triage_o2sat_model", "lo": 0.0, "hi": 100.0},
    {"vital_name": "ed_first_o2sat", "raw_col": "ed_first_o2sat", "clean_col": "ed_first_o2sat_model", "lo": 0.0, "hi": 100.0},
    {"vital_name": "ed_triage_sbp", "raw_col": "ed_triage_sbp", "clean_col": "ed_triage_sbp_model", "lo": 20.0, "hi": 300.0},
    {"vital_name": "ed_first_sbp", "raw_col": "ed_first_sbp", "clean_col": "ed_first_sbp_model", "lo": 20.0, "hi": 300.0},
    {"vital_name": "ed_triage_dbp", "raw_col": "ed_triage_dbp", "clean_col": "ed_triage_dbp_model", "lo": 10.0, "hi": 200.0},
    {"vital_name": "ed_first_dbp", "raw_col": "ed_first_dbp", "clean_col": "ed_first_dbp_model", "lo": 10.0, "hi": 200.0},
    {"vital_name": "ed_triage_pain", "raw_col": "ed_triage_pain", "clean_col": "ed_triage_pain_model", "lo": 0.0, "hi": 10.0},
    {"vital_name": "ed_first_pain", "raw_col": "ed_first_pain", "clean_col": "ed_first_pain_model", "lo": 0.0, "hi": 10.0},
)


def build_vitals_outlier_phase_audit(
    frame: pd.DataFrame,
    *,
    scope_name: str,
    phase_name: str,
    use_clean_columns: bool,
) -> pd.DataFrame:
    """Summarize out-of-range vitals for a frame/scope/phase audit snapshot."""
    rows: list[dict[str, Any]] = []
    for spec in VITAL_OUTLIER_AUDIT_SPECS:
        source_column = spec["clean_col"] if use_clean_columns and spec["clean_col"] in frame.columns else spec["raw_col"]
        if source_column not in frame.columns:
            continue
        values = pd.to_numeric(frame[source_column], errors="coerce")
        out_of_range = values.notna() & ((values < spec["lo"]) | (values > spec["hi"]))
        ...
        rows.append(
            {
                ...
                "lower_bound": float(spec["lo"]),
                "upper_bound": float(spec["hi"]),
                "n_rows": int(len(frame)),
                "n_non_missing": non_missing_n,
                "out_of_range_n": out_of_range_n,
                "out_of_range_pct": float(out_of_range_n / non_missing_n) if non_missing_n else 0.0,
            }
        )
    return pd.DataFrame(rows).sort_values(["scope_name", "phase_name", "vital_name"]).reset_index(drop=True)
...
DERIVED = os.getenv("BQ_DATASET_DERIVED", "mimiciv_derived").strip()
if not DERIVED:
    raise ValueError("BQ_DATASET_DERIVED must name the MIMIC-IV derived dataset.")
...
def validate_derived_dataset(
    dataset: str,
    *,
    expected_mimic_version: str = "3.1",
) -> dict[str, str]:
    """Fail closed unless the configured derived dataset is built from MIMIC-IV 3.1."""
    # Required release contract: mimic_version = 3.1.
    metadata_fqtn = f"{PHYS}.{dataset}._metadata"
    ventilation_fqtn = f"{PHYS}.{dataset}.ventilation"
    if not table_exists(metadata_fqtn):
        raise RuntimeError(
            "Configured MIMIC-IV derived dataset metadata table is absent or inaccessible: "
            f"{metadata_fqtn}. Set BQ_DATASET_DERIVED to the release-managed derived dataset."
        )
    ...
    FROM `{metadata_fqtn}`
    WHERE LOWER(TRIM(CAST(attribute AS STRING))) = 'mimic_version'
    """
    try:
        metadata = run_sql_bq(metadata_sql)
    except Exception as exc:
        raise RuntimeError(
            "Unable to read the expected attribute/value schema from derived metadata table "
            f"{metadata_fqtn}."
        ) from exc

    if set(metadata.columns) != {"attribute", "value"}:
        raise RuntimeError(
            f"Derived metadata query returned unexpected columns for {metadata_fqtn}: "
            f"{sorted(metadata.columns)}."
        )
    if len(metadata) != 1:
        raise RuntimeError(
            f"Derived metadata must contain exactly one mimic_version row; found {len(metadata)} "
            f"in {metadata_fqtn}."
        )
    observed_version = str(metadata.iloc[0]["value"]).strip()
    if observed_version != expected_mimic_version:
        raise RuntimeError(
            "MIMIC-IV derived dataset version mismatch: "
            f"expected {expected_mimic_version}, observed {observed_version!r} in {metadata_fqtn}."
        )
    if not table_exists(ventilation_fqtn):
        raise RuntimeError(
            "Required official derived ventilation table is absent or inaccessible: "
            f"{ventilation_fqtn}. No legacy timing fallback is permitted for IMV classification."
        )
    return {
        "dataset": dataset,
        "metadata_table": metadata_fqtn,
        "ventilation_table": ventilation_fqtn,
        "mimic_version": observed_version,
    }
...
ICU_MIMIC_VERSION = "3.1"
ICU_3_1_DATASET_ALIASES = frozenset({"mimiciv_3_1_icu", "mimiciv_v3_1_icu"})
if ICU not in ICU_3_1_DATASET_ALIASES:
    raise RuntimeError(
        "The official MIMIC-IV derived ventilation join requires an explicitly "
        "versioned MIMIC-IV 3.1 ICU dataset. "
        f"Resolved ICU dataset {ICU!r}; accepted aliases are "
        f"{sorted(ICU_3_1_DATASET_ALIASES)}. Unversioned fallback is not permitted."
    )
if not ED:
    ED = "mimiciv_ed"
...
DERIVED_VALIDATION = validate_derived_dataset(DERIVED)

RUN_METADATA = {
    "run_utc": datetime.now(timezone.utc).isoformat(),
    "work_project": WORK_PROJECT,
    "physionet_project": PHYS,
    "datasets": {"hosp": HOSP, "icu": ICU, "ed": ED, "derived": DERIVED},
    "dataset_probes": {
        "hosp": HOSP_PROBE,
        "icu": ICU_PROBE,
        "ed": ED_PROBE,
        "derived_metadata": DERIVED_VALIDATION["metadata_table"],
        "derived_ventilation": DERIVED_VALIDATION["ventilation_table"],
    },
    "derived_mimic_version": DERIVED_VALIDATION["mimic_version"],
    "icu_release_validation": {
        "dataset": ICU,
        "mimic_version": ICU_MIMIC_VERSION,
        "accepted_dataset_aliases": sorted(ICU_3_1_DATASET_ALIASES),
        "unversioned_fallback_permitted": False,
    },
    "notebook": "MIMICIV_hypercap_EXT_cohort.qmd",
...
# Purpose: Create reusable data-quality helpers to prevent silent data integrity errors.

# --- Helper utilities for reproducibility and data-quality guardrails
...
SQL["ed_only_exclusion_audit_sql"] = f"""
-- Diagnostic-only audit of ED stays excluded because they do not map to an admission.
WITH target_codes AS (
  SELECT 'J9602' AS code, 10 AS ver UNION ALL
  SELECT 'J9612', 10 UNION ALL
  SELECT 'J9622', 10 UNION ALL
...
target_by_stay AS (
  SELECT
    subject_id,
    stay_id,
    1 AS any_target_hypercap_ohs_ed_icd
  FROM target_ed_dx
  GROUP BY subject_id, stay_id
)
SELECT
  'total_ed_stays' AS metric,
  COUNT(DISTINCT stay_id) AS count,
  'all_ed_stays' AS count_scope,
  'assessable_from_mimiciv_ed_edstays' AS assessability,
  'All rows in mimiciv_ed.edstays.' AS note
FROM ed_stays
UNION ALL
SELECT
  'ed_stays_without_hadm_id' AS metric,
  COUNT(DISTINCT IF(hadm_id IS NULL, stay_id, NULL)) AS count,
  'ed_only_stays' AS count_scope,
  'assessable_from_mimiciv_ed_edstays' AS assessability,
  'ED stays excluded from admission-level cohort construction because hadm_id is missing.' AS note
FROM ed_stays
UNION ALL
SELECT
  'ed_only_with_nonmissing_triage_chief_complaint' AS metric,
  COUNT(DISTINCT IF(hadm_id IS NULL AND chiefcomplaint IS NOT NULL AND TRIM(CAST(chiefcomplaint AS STRING)) != '', stay_id, NULL)) AS count,
  'ed_only_stays' AS count_scope,
  'assessable_from_mimiciv_ed_edstays' AS assessability,
  'ED-only stays that still contain a nonmissing triage chief complaint field.' AS note
FROM ed_stays
UNION ALL
SELECT
  'ed_only_with_target_hypercap_ohs_ed_icd' AS metric,
  COUNT(DISTINCT IF(s.hadm_id IS NULL AND IFNULL(t.any_target_hypercap_ohs_ed_icd, 0) = 1, s.stay_id, NULL)) AS count,
  'ed_only_stays' AS count_scope,
  'assessable_from_mimiciv_ed_diagnosis' AS assessability,
  'ED-only stays carrying target hypercapnic respiratory failure or OHS ED diagnosis codes.' AS note
FROM ed_stays s
LEFT JOIN target_by_stay t
  ON t.subject_id = s.subject_id AND t.stay_id = s.stay_id
"""

ed_only_exclusion_audit = run_sql_bq(sql("ed_only_exclusion_audit_sql")).copy()
if ed_only_exclusion_audit.empty:
    raise ValueError("ED-only exclusion audit returned no rows.")
ed_only_exclusion_audit["count"] = pd.to_numeric(
    ed_only_exclusion_audit["count"], errors="coerce"
).astype("Int64")
ed_only_gas_eligibility_note = (
    "ED-only blood-gas eligibility is not assessable from the current cohort-stage "
    "admitted-patient HOSP/ICU blood-gas extraction path; no ED-only gas qualification "
    "count is inferred."
)
ed_only_exclusion_audit = pd.concat(
    [
        ed_only_exclusion_audit,
        pd.DataFrame(
            [
                {
                    "metric": "ed_only_gas_eligibility_count",
                    "count": pd.NA,
                    "count_scope": "ed_only_stays",
                    "assessability": "not_assessable",
                    "note": ed_only_gas_eligibility_note,
                }
            ]
        ),
    ],
...
ed_only_exclusion_audit_records = json.loads(
    ed_only_exclusion_audit.to_json(orient="records")
)
cohort_qa_dir = WORK_DIR / "artifacts" / "qa" / "cohort"
cohort_qa_dir.mkdir(parents=True, exist_ok=True)
ed_only_exclusion_audit_csv_path = cohort_qa_dir / "ed_only_exclusion_audit.csv"
ed_only_exclusion_audit_json_path = cohort_qa_dir / "ed_only_exclusion_audit.json"
ed_only_exclusion_audit.to_csv(ed_only_exclusion_audit_csv_path, index=False)
ed_only_exclusion_audit_json_path.write_text(
    json.dumps(
        {
            "generated_utc": datetime.utcnow().isoformat() + "Z",
            "scope": "diagnostic_only_ed_stays_without_hadm_id",
            "cohort_logic_changed": False,
            "gas_eligibility_assessability": "not_assessable",
            "gas_eligibility_note": ed_only_gas_eligibility_note,
            "records": ed_only_exclusion_audit_records,
        },
        indent=2,
    )
)
print("Wrote:", ed_only_exclusion_audit_csv_path)
print("Wrote:", ed_only_exclusion_audit_json_path)
```

### 2) Blood gas inclusion thresholds — qualifying hypercapnic gases any-time in stay (LAB + POC)
...
hosp_ph_raw AS (
  ...
  JOIN `{PHYS}.{HOSP}.d_labitems` di
    ON di.itemid = le.itemid
  WHERE le.itemid IN UNNEST({LAB_PH_SQL_ARRAY})
...
hosp_ph_clean AS (
  SELECT
    ...
    ph_value
  FROM hosp_ph_raw
  WHERE ph_value BETWEEN 6.3 AND 7.8
),
hosp_specimen AS (
  SELECT
    le.specimen_id,
...
hosp_pco2_clean AS (
  SELECT
    p.*,
    h.ph_value AS qualifying_ph,
    h.charttime AS qualifying_ph_time,
    CASE
      WHEN h.ph_value IS NOT NULL THEN 'LAB'
      ELSE NULL
    END AS qualifying_ph_source_branch,
    CASE
      WHEN h.ph_value IS NOT NULL THEN p.site
      ELSE NULL
    END AS qualifying_ph_site,
    CASE
      WHEN h.ph_value IS NOT NULL THEN 'paired_same_specimen_panel'
      ELSE 'qualifying_panel_missing_ph'
    END AS qualifying_ph_pairing_status
  FROM hosp_pco2_base p
  LEFT JOIN hosp_ph_clean h
    ON h.hadm_id = p.hadm_id
   AND h.specimen_id = p.specimen_id
   AND ABS(TIMESTAMP_DIFF(h.charttime, p.charttime, MINUTE)) <= 10
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY p.hadm_id, p.charttime, p.specimen_id, p.itemid
    ORDER BY
      CASE WHEN h.charttime = p.charttime THEN 0 ELSE 1 END,
      ABS(TIMESTAMP_DIFF(h.charttime, p.charttime, SECOND)),
      h.itemid
  ) = 1
),
icu_pco2_itemids AS (
...
icu_ph_raw AS (
  ...
  WHERE ce.itemid IN UNNEST({ICU_PH_SQL_ARRAY})
...
icu_pco2_raw AS (
  SELECT
    ...
    CASE
      ...
      WHEN ce.itemid IN UNNEST({ICU_PCO2_UNKNOWN_SQL_ARRAY}) THEN 'unknown'
      ELSE 'unknown'
...
icu_pco2_clean AS (
  SELECT
    ...
    h.charttime AS qualifying_ph_time,
    CASE
      WHEN h.ph_value IS NOT NULL THEN 'POC'
      ELSE NULL
    END AS qualifying_ph_source_branch,
    CASE
      WHEN h.ph_value IS NOT NULL THEN h.site
      ELSE NULL
    END AS qualifying_ph_site,
    CASE
      WHEN h.ph_value IS NOT NULL THEN 'paired_same_time_panel'
      ELSE 'qualifying_panel_missing_ph'
    END AS qualifying_ph_pairing_status
  FROM icu_pco2_base p
  LEFT JOIN icu_ph_clean h
    ON h.hadm_id = p.hadm_id
   AND h.stay_id = p.stay_id
   AND h.site = p.site
   AND ABS(TIMESTAMP_DIFF(h.charttime, p.charttime, MINUTE)) <= 10
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY p.hadm_id, p.stay_id, p.charttime, p.itemid
    ORDER BY
      CASE WHEN h.charttime = p.charttime THEN 0 ELSE 1 END,
      ABS(TIMESTAMP_DIFF(h.charttime, p.charttime, SECOND)),
      h.itemid
...
stay_pco2 AS (
  SELECT
    ...
    p.source_branch,
    p.qualifying_ph,
    p.qualifying_ph_time,
    p.qualifying_ph_source_branch,
    p.qualifying_ph_site,
    p.qualifying_ph_pairing_status,
    a.ed_intime_first,
    a.admittime,
    a.dischtime,
    a.presentation_anchor_time,
    a.used_admittime_anchor,
    TIMESTAMP_DIFF(p.charttime, a.presentation_anchor_time, SECOND) / 3600.0 AS dt_hours
  FROM all_pco2 p
  JOIN anchor_times a
...
qualifying_candidates AS (
  SELECT
    ...
    itemid,
    dt_hours,
    IF(used_admittime_anchor = 1, 'ADMITTIME_FALLBACK', 'ED_INTIME') AS anchor_source,
...
SELECT
  ...
  IF(q.dt_qualifying_hypercapnia_hours <= 24.0, 1, 0) AS pco2_threshold_0_24h,
  ...
  q.qualifying_ph,
  q.qualifying_ph_time,
  q.qualifying_ph_source_branch,
  q.qualifying_ph_site,
  q.qualifying_ph_pairing_status,
...
qualifying_anchor_fallback_n = 0
qualifying_anchor_ed_n = 0
if "qualifying_anchor_source" in co2_thresh.columns:
    ...
    qualifying_anchor_fallback_n = int(
        qualifying_anchor_source.eq("ADMITTIME_FALLBACK").sum()
    )
    qualifying_anchor_ed_n = int(qualifying_anchor_source.eq("ED_INTIME").sum())
    print(
        "Qualifying anchor source counts:",
        f"ED_INTIME={qualifying_anchor_ed_n}",
        f"ADMITTIME_FALLBACK={qualifying_anchor_fallback_n}",
    )
...
# Final enrollment flags (primary definition: ICD OR first qualifying hypercapnic gas any-time in stay)
...
cohort_any["enrollment_route"] = np.select(
    [
        (cohort_any["any_hypercap_icd"] == 0) & (cohort_any["pco2_threshold_any"] == 1),
        (cohort_any["any_hypercap_icd"] == 1) & (cohort_any["pco2_threshold_any"] == 0),
        (cohort_any["any_hypercap_icd"] == 1) & (cohort_any["pco2_threshold_any"] == 1),
    ],
...
print("Gas-qualified any-time admissions          :", int((cohort_any["pco2_threshold_any"] == 1).sum()))
...
print("Final enrolled admissions (ICD OR gas any-time):", int((cohort_any["enrolled_any"] == 1).sum()))
print("Enrollment routes:")
print(cohort_any["enrollment_route"].value_counts(dropna=False).to_string())
if {"qualifying_source_branch", "pco2_threshold_any"}.issubset(cohort_any.columns):
    qualifying_branch = (
        cohort_any.loc[cohort_any["pco2_threshold_any"] == 1, "qualifying_source_branch"]
        .fillna("UNKNOWN")
        .astype("string")
    )
...
# If no ICU unknown-site pCO2 itemids are configured, drop POC unknown detail columns.
# This keeps the canonical export schema parsimonious when no rows can populate them.
if not ICU_PCO2_UNKNOWN_ITEMIDS:
    poc_unknown_detail_columns = [
        "poc_other_ph",
        "poc_other_ph_uom",
        "poc_other_po2",
        "poc_other_paco2",
    ...
    poc_unknown_drop_existing = [
        column_name
        for column_name in poc_unknown_detail_columns
        if column_name in bg_pairs.columns
    ]
    if poc_unknown_drop_existing:
        bg_pairs = bg_pairs.drop(columns=poc_unknown_drop_existing)
        print(
            "Dropped POC unknown-site detail columns (no unknown itemids configured):",
            ", ".join(poc_unknown_drop_existing),
        )
```
...
### 9) Reliable observed IMV sources

**Rationale:** Establish the earliest documented IMV time from the official MIMIC-IV derived ventilation concept and explicit ICU procedure events. These sources are fail-closed and do not use the broad legacy chart regex.

```{python}
#| label: mimiciv-hypercap-ext-cohort-cell-013b
# Purpose: Validate and extract official derived IMV episodes and explicit ICU procedure events.

IMV_PROCEDURE_ITEM_LABELS = {
    224385: "Intubation",
    225792: "Invasive Ventilation",
}

SQL["derived_imv_episodes_sql"] = f"""
WITH hadms AS (SELECT x AS hadm_id FROM UNNEST(@hadms) AS x)
SELECT
  v.stay_id,
  i.hadm_id,
...
FROM `{PHYS}.{DERIVED}.ventilation` AS v
JOIN `{PHYS}.{ICU}.icustays` AS i
...
# Keep episode-level rows in memory for QA; only admission-level minima enter the handoff.
derived_imv_episodes = run_sql_bq(
    sql("derived_imv_episodes_sql"),
    {"hadms": hadm_list},
)
required_derived_episode_columns = {
    "stay_id",
    "hadm_id",
    "starttime",
    "endtime",
...
missing_derived_episode_columns = sorted(
    required_derived_episode_columns.difference(derived_imv_episodes.columns)
)
if missing_derived_episode_columns:
    raise KeyError(
        "Official derived ventilation query missing required columns: "
        f"{missing_derived_episode_columns}."
    )
derived_imv_episodes["stay_id"] = pd.to_numeric(
    derived_imv_episodes["stay_id"], errors="coerce"
).astype("Int64")
derived_imv_episodes["hadm_id"] = pd.to_numeric(
    derived_imv_episodes["hadm_id"], errors="coerce"
).astype("Int64")
for timestamp_column in ("starttime", "endtime"):
    derived_imv_episodes[timestamp_column] = pd.to_datetime(
        derived_imv_episodes[timestamp_column], errors="coerce"
    )
invalid_derived_statuses = sorted(
    set(derived_imv_episodes["ventilation_status"].dropna().astype(str)).difference(
        {"InvasiveVent"}
    )
)
if invalid_derived_statuses:
    raise ValueError(
        "Official derived IMV extraction returned non-InvasiveVent statuses: "
        f"{invalid_derived_statuses}."
    )
derived_imv_missing_starttime_rows = int(
    derived_imv_episodes["starttime"].isna().sum()
)

first_derived_imv = (
    derived_imv_episodes.groupby("hadm_id", as_index=False, dropna=False)
    .agg(first_derived_imv_starttime=("starttime", "min"))
)

SQL["imv_procedure_item_validation_sql"] = f"""
SELECT
  itemid,
  label
FROM `{PHYS}.{ICU}.d_items`
WHERE itemid IN UNNEST(@itemids)
ORDER BY itemid
"""
imv_procedure_item_validation = run_sql_bq(
    sql("imv_procedure_item_validation_sql"),
    {"itemids": list(IMV_PROCEDURE_ITEM_LABELS)},
)
required_item_validation_columns = {"itemid", "label"}
if not required_item_validation_columns.issubset(imv_procedure_item_validation.columns):
    raise KeyError(
        "IMV procedure d_items validation missing required columns: "
        f"{sorted(required_item_validation_columns.difference(imv_procedure_item_validation.columns))}."
    )
imv_procedure_item_validation["itemid"] = pd.to_numeric(
    imv_procedure_item_validation["itemid"], errors="coerce"
).astype("Int64")
observed_itemids = set(
    imv_procedure_item_validation["itemid"].dropna().astype(int).tolist()
)
expected_itemids = set(IMV_PROCEDURE_ITEM_LABELS)
if observed_itemids != expected_itemids or len(imv_procedure_item_validation) != len(expected_itemids):
    raise RuntimeError(
        "Required explicit IMV procedure item IDs are absent or duplicated in active ICU d_items: "
        f"expected={sorted(expected_itemids)}, observed={sorted(observed_itemids)}."
    )
observed_imv_procedure_labels: dict[str, str] = {}
for itemid, expected_label in IMV_PROCEDURE_ITEM_LABELS.items():
    observed_label = imv_procedure_item_validation.loc[
        imv_procedure_item_validation["itemid"].eq(itemid), "label"
    ].iloc[0]
    observed_imv_procedure_labels[str(itemid)] = str(observed_label)
    if normalize_mimic_item_label(observed_label) != normalize_mimic_item_label(expected_label):
        raise RuntimeError(
            "MIMIC-IV ICU procedure item label mismatch: "
            f"itemid={itemid}, expected={expected_label!r}, observed={observed_label!r}."
        )

SQL["imv_procedure_events_sql"] = f"""
WITH hadms AS (SELECT x AS hadm_id FROM UNNEST(@hadms) AS x)
SELECT
  pe.hadm_id,
  pe.stay_id,
  pe.itemid,
  pe.starttime
FROM `{PHYS}.{ICU}.procedureevents` AS pe
JOIN hadms AS h
  ON h.hadm_id = pe.hadm_id
WHERE pe.itemid IN UNNEST(@itemids)
"""
imv_procedure_events = run_sql_bq(
    sql("imv_procedure_events_sql"),
    {
        "hadms": hadm_list,
        "itemids": list(IMV_PROCEDURE_ITEM_LABELS),
    },
)
required_procedure_event_columns = {"hadm_id", "stay_id", "itemid", "starttime"}
missing_procedure_event_columns = sorted(
    required_procedure_event_columns.difference(imv_procedure_events.columns)
)
if missing_procedure_event_columns:
    raise KeyError(
        "Explicit IMV procedureevents query missing required columns: "
        f"{missing_procedure_event_columns}."
    )
for id_column in ("hadm_id", "stay_id", "itemid"):
    imv_procedure_events[id_column] = pd.to_numeric(
        imv_procedure_events[id_column], errors="coerce"
    ).astype("Int64")
imv_procedure_events["starttime"] = pd.to_datetime(
    imv_procedure_events["starttime"], errors="coerce"
)
imv_procedure_missing_starttime_rows = int(
    imv_procedure_events["starttime"].isna().sum()
)
imv_procedure_missing_starttime_rows_by_itemid = {
    str(itemid): int(
        (
            imv_procedure_events["itemid"].eq(itemid)
            & imv_procedure_events["starttime"].isna()
        ).sum()
    )
    for itemid in IMV_PROCEDURE_ITEM_LABELS
}

# Preserve admission-level evidence even when every matching source record has
# a missing/unparseable starttime; only the aggregate flag continues downstream.
imv_source_record_evidence = (
    pd.concat(
        [
            derived_imv_episodes[["hadm_id"]],
            imv_procedure_events[["hadm_id"]],
        ],
        ignore_index=True,
    )
...
imv_source_record_evidence[IMV_SOURCE_RECORD_NO_TIME_COLUMN] = 1

robust_imv_sources = pd.DataFrame(
    {"hadm_id": pd.Series(hadm_list, dtype="Int64")}
).drop_duplicates()
robust_imv_sources = robust_imv_sources.merge(
    first_derived_imv,
    on="hadm_id",
    how="left",
    validate="one_to_one",
...
for itemid, output_column in (
    (224385, "first_intubation_procedure_time"),
    (225792, "first_invasive_ventilation_procedure_time"),
):
    first_event = (
        imv_procedure_events.loc[imv_procedure_events["itemid"].eq(itemid)]
        .groupby("hadm_id", as_index=False, dropna=False)
        .agg(**{output_column: ("starttime", "min")})
    )
    robust_imv_sources = robust_imv_sources.merge(
        first_event,
        on="hadm_id",
...
robust_source_time_columns = [
    "first_intubation_procedure_time",
    "first_invasive_ventilation_procedure_time",
    "first_derived_imv_starttime",
]
for timestamp_column in robust_source_time_columns:
    robust_imv_sources[timestamp_column] = pd.to_datetime(
        robust_imv_sources[timestamp_column], errors="coerce"
    )
...
selected_imv = [
    ...
    for row in robust_imv_sources.itertuples(index=False)
]
robust_imv_sources["first_observed_imv_time"] = [value[0] for value in selected_imv]
robust_imv_sources["first_observed_imv_source"] = pd.Series(
    [value[1] for value in selected_imv],
    index=robust_imv_sources.index,
    dtype="string",
)
robust_imv_sources["robust_imv_observed"] = (
    robust_imv_sources[robust_source_time_columns].notna().any(axis=1).astype("Int64")
)
robust_imv_sources[IMV_SOURCE_RECORD_NO_TIME_COLUMN] = (
    pd.to_numeric(
        robust_imv_sources[IMV_SOURCE_RECORD_NO_TIME_COLUMN], errors="coerce"
    )
    .fillna(0)
    .astype(int)
    .eq(1)
    & robust_imv_sources["robust_imv_observed"].eq(0)
).astype("Int64")
robust_time_missing_n = int(
    (
        robust_imv_sources["robust_imv_observed"].eq(1)
        & robust_imv_sources["first_observed_imv_time"].isna()
    ).sum()
)
if robust_time_missing_n:
    raise AssertionError(
        "Robust IMV evidence must have a first observed time; "
        f"violations={robust_time_missing_n}."
    )

RUN_METADATA["imv_source_validation"] = {
    "derived_mimic_version": DERIVED_VALIDATION["mimic_version"],
    "derived_ventilation_status": "InvasiveVent",
    "procedure_item_labels": observed_imv_procedure_labels,
    "derived_episode_rows": int(len(derived_imv_episodes)),
    "derived_episode_missing_starttime_rows": derived_imv_missing_starttime_rows,
    "explicit_procedure_event_rows": int(len(imv_procedure_events)),
    "explicit_procedure_event_missing_starttime_rows": imv_procedure_missing_starttime_rows,
    "explicit_procedure_event_missing_starttime_rows_by_itemid": (
        imv_procedure_missing_starttime_rows_by_itemid
    ),
    "admissions_with_robust_imv": int(
        robust_imv_sources["robust_imv_observed"].sum()
    ),
    "admissions_with_source_record_imv_evidence_without_reliable_timestamp": int(
        robust_imv_sources[IMV_SOURCE_RECORD_NO_TIME_COLUMN].sum()
    ),
}

print(
    "Reliable observed IMV sources:",
    f"derived episodes={len(derived_imv_episodes):,}",
    f"procedure events={len(imv_procedure_events):,}",
    f"admissions with robust IMV={int(robust_imv_sources['robust_imv_observed'].sum()):,}",
)
```

### 10) Legacy ventilation flags (ICD procedures and broad chart regex)
...
df = safe_merge_on_hadm(df, robust_imv_sources,   right_name="robust_imv_sources")

# Canonical reliable IMV fields and strict temporal classification.
robust_source_time_columns = [
    "first_intubation_procedure_time",
    "first_invasive_ventilation_procedure_time",
    "first_derived_imv_starttime",
...
for timestamp_column in [
    *robust_source_time_columns,
    ...
    if timestamp_column not in df.columns:
        raise KeyError(f"Required IMV timing field missing after assembly: {timestamp_column}.")
    df[timestamp_column] = pd.to_datetime(df[timestamp_column], errors="coerce")

df["robust_imv_observed"] = (
    df[robust_source_time_columns].notna().any(axis=1).astype("Int64")
)
df["first_observed_imv_source"] = (
    df["first_observed_imv_source"].astype("string").fillna("missing")
)
df[IMV_SOURCE_RECORD_NO_TIME_COLUMN] = (
    pd.to_numeric(df[IMV_SOURCE_RECORD_NO_TIME_COLUMN], errors="coerce")
    .fillna(0)
    .astype("Int64")
)

gas_positive_for_imv = (
    pd.to_numeric(df["pco2_threshold_any"], errors="coerce")
    .fillna(0)
    .astype(int)
    .eq(1)
...
source_record_no_time_for_imv = df[IMV_SOURCE_RECORD_NO_TIME_COLUMN].eq(1)
df["imv_qualifying_gas_order"] = pd.Series(
    [
        classify_imv_qualifying_gas_order(
            gas_positive=bool(gas_positive),
            qualifying_pco2_time=qualifying_time,
            robust_imv_observed=bool(robust_observed),
            first_observed_imv_time=observed_imv_time,
            legacy_imv_flag=bool(legacy_flag),
            source_record_imv_evidence_without_reliable_timestamp=bool(
                source_record_no_time_flag
            ),
...
df["imv_preceded_qualifying_gas"] = pd.Series(pd.NA, index=df.index, dtype="boolean")
df.loc[
    df["imv_qualifying_gas_order"].eq("imv_before_qualifying_gas"),
    "imv_preceded_qualifying_gas",
] = True
df.loc[
    df["imv_qualifying_gas_order"].isin(
        {"no_observed_imv", "qualifying_gas_before_imv"}
    ),
    "imv_preceded_qualifying_gas",
] = False

df["no_prior_observed_imv"] = pd.Series(pd.NA, index=df.index, dtype="boolean")
df.loc[
    df["imv_qualifying_gas_order"].isin(
        {"no_observed_imv", "qualifying_gas_before_imv"}
    ),
...
df["hours_from_imv_to_qualifying_gas"] = (
    df["qualifying_pco2_time"] - df["first_observed_imv_time"]
).dt.total_seconds() / 3600.0
df["legacy_imv_timing_discordant"] = pd.Series(
    [
        legacy_imv_timing_discordant(
            gas_positive=bool(gas_positive),
            qualifying_pco2_time=qualifying_time,
        ...
        for gas_positive, qualifying_time, robust_observed, observed_imv_time, legacy_flag, legacy_time in zip(
            gas_positive_for_imv,
            df["qualifying_pco2_time"],
            df["robust_imv_observed"],
            df["first_observed_imv_time"],
...
if "ed_intime_first" not in locals():
    SQL["ed_anchor_sql"] = f"""
    WITH hadms AS (SELECT x AS hadm_id FROM UNNEST(@hadms) AS x)
    SELECT
      hadm_id,
    ...
    GROUP BY hadm_id
    """
    ed_intime_first = run_sql_bq(sql("ed_anchor_sql"), {"hadms": hadm_list})
    ed_intime_first["ed_intime_first"] = pd.to_datetime(
        ed_intime_first.get("ed_intime_first"),
        errors="coerce",
    )
df = safe_merge_on_hadm(df, ed_intime_first, right_name="ed_intime_first")
if "ed_intime_first" not in df.columns:
    raise KeyError("ed_intime_first missing after deterministic ED anchor merge.")
if "first_imv_time" not in df.columns or "first_niv_time" not in df.columns:
    raise KeyError("vent_combined merge missing expected first_imv_time/first_niv_time columns.")
...
# Primary cohort contract: earliest hypercapnic blood gas (LAB/POC) any-time during stay.
...
qualifying_dt_hours = pd.to_numeric(
    df.get("dt_qualifying_hypercapnia_hours", pd.Series(np.nan, index=df.index)),
    errors="coerce",
)
df["pco2_threshold_0_24h"] = (
    (pd.to_numeric(df["pco2_threshold_any"], errors="coerce").fillna(0).astype(int) == 1)
...
df["enrollment_route"] = np.select(
    [
        (df["any_hypercap_icd"] == 0) & (df["pco2_threshold_any"] == 1),
        (df["any_hypercap_icd"] == 1) & (df["pco2_threshold_any"] == 0),
        (df["any_hypercap_icd"] == 1) & (df["pco2_threshold_any"] == 1),
    ],
    ["GAS_ONLY", "ICD_ONLY", "ICD+GAS"],
    default="NONE",
)
...
if "enrolled_any" not in cohort_any.columns:
    raise KeyError("cohort_any missing enrolled_any during flow-count summary.")
cohort_union = int((cohort_any["enrolled_any"] == 1).sum())
cohort_df_n = len(df)
...
if "ed_triage_cc" in df.columns:
    mask_cc_present = chief_complaint_inclusion_mask(df["ed_triage_cc"])
...
flow_counts_path = PRIOR_RUNS_DIR / f"{RUN_DATE} cohort_flow_counts.csv"
flow_counts.to_csv(flow_counts_path, index=False)
print("Wrote:", flow_counts_path)

flow_counts_lookup = {
    str(step): int(count)
    for step, count in flow_counts[["step", "count"]].itertuples(index=False, name=None)
}
flow_counts
```
...
if "ed_triage_cc" in df.columns:
    cc_present = chief_complaint_inclusion_mask(df["ed_triage_cc"])
    summary_rows.append({
...
if WRITE_ARCHIVE_XLSX_EXPORTS:
    out_path = archive_xlsx_path("mimic_hypercap_EXT_bq_abg_vbg")
    archive_cohort = prepare_archive_export_frame(
        df,
        export_name="mimic_hypercap_EXT_bq_abg_vbg",
    )
    with pd.ExcelWriter(out_path, engine="openpyxl") as xw:
        archive_cohort.to_excel(xw, sheet_name="cohort", index=False)
        try:
...
if COHORT_DEBUG_INVENTORY:
    ...
    inventory_dir = WORK_DIR / "artifacts" / "qa" / "cohort"
    inventory_dir.mkdir(parents=True, exist_ok=True)

    # Persist columns snapshot for debug-only inventory audits
    cols_out = inventory_dir / "current_columns.json"
    cols_out.write_text(json.dumps(sorted(df.columns), indent=2))
    print("Wrote:", cols_out)

    if "ed_df" in globals():
        ed_cols_out = inventory_dir / "ed_columns.json"
...
TARGET_RAW_FIELDS = {
    "ed_edstays": [
        ...
        "arrival_transport","disposition_ed",
    ],
    "ed_triage": [
    ...
    "labs_gas": [
        "first_ph","first_hco3","first_hco3_time","first_hco3_source","first_hco3_itemid","first_hco3_qc_flag","first_lactate",
        ...
        "qualifying_ph","qualifying_ph_time","qualifying_ph_source_branch","qualifying_ph_site","qualifying_ph_pairing_status",
        ...
        # Gas provenance fields are exported intentionally for source-trace audits.
        # They support reviewer verification when ABG/VBG typing is missing/ambiguous.
    ...
    "timing": [
        ...
        "first_derived_imv_starttime","first_intubation_procedure_time","first_invasive_ventilation_procedure_time",
        "first_observed_imv_time","first_observed_imv_source","robust_imv_observed",
        "imv_qualifying_gas_order","imv_preceded_qualifying_gas","no_prior_observed_imv",
        "hours_from_imv_to_qualifying_gas","legacy_imv_timing_discordant","imv_chart_flag",
...
SELECT
  ...
  s.arrival_transport,
  s.disposition AS disposition_ed,
...
# ED triage and first ED vitals (deterministic canonical path).
# We intentionally run explicit SQL here instead of "reuse if already loaded"
# branches so the stage has one predictable execution path in pipeline runs.
SQL["ed_triage_sql"] = f"""
WITH hadms AS (SELECT x AS hadm_id FROM UNNEST(@hadms) AS x)
SELECT
  s.stay_id AS ed_stay_id,
...
SQL["ed_first_vitals_by_stay_sql"] = f"""
SELECT
...
"""
stay_id_map = (
    ed_spine[["ed_stay_id", "hadm_id"]]
    .dropna(subset=["ed_stay_id"])
    .copy()
)
stay_id_map["ed_stay_id"] = pd.to_numeric(stay_id_map["ed_stay_id"], errors="coerce")
stay_id_map = stay_id_map.dropna(subset=["ed_stay_id"])
stay_id_map["ed_stay_id"] = stay_id_map["ed_stay_id"].astype(int)
stay_ids = sorted(stay_id_map["ed_stay_id"].unique().tolist())

first_vitals_chunks: list[pd.DataFrame] = []
for start_index in range(0, len(stay_ids), 5000):
    stay_chunk = stay_ids[start_index:start_index + 5000]
    chunk_df = run_sql_bq(sql("ed_first_vitals_by_stay_sql"), {"stay_ids": stay_chunk})
    if len(chunk_df) == 0:
        continue
    chunk_df["ed_stay_id"] = pd.to_numeric(chunk_df["ed_stay_id"], errors="coerce")
    chunk_df = chunk_df.dropna(subset=["ed_stay_id"])
    chunk_df["ed_stay_id"] = chunk_df["ed_stay_id"].astype(int)
    first_vitals_chunks.append(chunk_df)

if first_vitals_chunks:
    ed_vitals_first_long = pd.concat(first_vitals_chunks, ignore_index=True)
    ed_vitals_first_long["charttime"] = pd.to_datetime(
        ed_vitals_first_long["charttime"], errors="coerce"
    )
    ed_first = (
        ed_vitals_first_long
        .sort_values(["ed_stay_id", "charttime"], kind="stable")
        .groupby("ed_stay_id", as_index=False)
        .first()
        .rename(
            columns={
                "charttime": "ed_first_vitals_time",
                "temperature": "ed_first_temp",
                "heartrate": "ed_first_hr",
                "resprate": "ed_first_rr",
                "o2sat": "ed_first_o2sat",
                "sbp": "ed_first_sbp",
                "dbp": "ed_first_dbp",
                "rhythm": "ed_first_rhythm",
                "pain": "ed_first_pain",
            }
        )
    )
else:
    ed_first = pd.DataFrame(
        columns=[
            "ed_stay_id",
            "ed_first_vitals_time",
            "ed_first_temp",
            "ed_first_hr",
            "ed_first_rr",
            "ed_first_o2sat",
...
ed_first = ed_first.merge(
    stay_id_map.drop_duplicates("ed_stay_id"),
    on="ed_stay_id",
    how="left",
)
print("ED first vitals rows:", len(ed_first))
...
if "ed_stay_id" not in ed_df.columns:
    raise KeyError("ed_df missing ed_stay_id; ensure ED spine extraction ran.")

required_triage_cols = {"ed_stay_id", "hadm_id"}
if not required_triage_cols.issubset(ed_triage.columns):
    raise KeyError(f"ED triage query missing required columns: {required_triage_cols}")
required_first_cols = {"ed_stay_id", "hadm_id"}
if not required_first_cols.issubset(ed_first.columns):
    raise KeyError(f"ED first-vitals query missing required columns: {required_first_cols}")

ed_df = ed_df.merge(ed_triage, on=["ed_stay_id", "hadm_id"], how="left")
ed_df = ed_df.merge(ed_first, on=["ed_stay_id", "hadm_id"], how="left")
```
...
cohort_qa_dir = WORK_DIR / "artifacts" / "qa" / "cohort"
cohort_qa_dir.mkdir(parents=True, exist_ok=True)
lab_item_map_path = cohort_qa_dir / "lab_item_map.json"
...
unit_audit_path = cohort_qa_dir / "lab_unit_audit.csv"
...
gas_source_audit_path = PRIOR_RUNS_DIR / f"{RUN_DATE} gas_source_audit.json"
...
if len(icu_itemids) == 0:
    ...
    # ensure ICU stay ids are available from the canonical ICU extraction phase
    if "icu_stay_id" not in icu.columns or "hadm_id" not in icu.columns:
        raise KeyError("ICU dataframe missing required columns: icu_stay_id/hadm_id.")
...
if len(icu_poc_long) > 0:
    ...
    panel_poc["site"] = np.select(
        [
            ...
            .fillna(-1)
            .astype(int)
            .isin(set(ICU_PCO2_UNKNOWN_ITEMIDS)),
        ],
        ["arterial", "venous", "unknown"],
        default="unknown",
    )
...
if len(icu_poc_long) > 0 and pco2_ids:
    ...
    if not pco2_long.empty:
        ...
        for itemid_value, group in pco2_long.groupby("itemid", dropna=False):
            ...
            elif int(itemid_value) in set(ICU_PCO2_UNKNOWN_ITEMIDS):
            ...
            p05_value = float(clean_numeric.quantile(0.05)) if clean_n else np.nan
            p25_value = float(clean_numeric.quantile(0.25)) if clean_n else np.nan
            p50_value = float(clean_numeric.median()) if clean_n else np.nan
            p95_value = float(clean_numeric.quantile(0.95)) if clean_n else np.nan
            max_clean_value = float(clean_numeric.max()) if clean_n else np.nan
            minimum_data_warning = bool(clean_n < POC_QC_MIN_VALID_N_WARN)
            distribution_check_applicable = bool(clean_n >= POC_QC_MIN_VALID_N_CHECK)
            median_ok = bool(
                clean_n > 0
            ...
            if (
                out_of_range_removed_rate > POC_QC_OUT_OF_RANGE_REMOVED_RATE_WARN
            ...
            pco2_itemid_qc_rows.append(
                {
                    ...
                    "raw_p50_mmhg": float(raw_numeric.median()) if raw_n else np.nan,
                    "raw_p95_mmhg": float(raw_numeric.quantile(0.95))
                    if raw_n
                    else np.nan,
                    "raw_max_mmhg": float(raw_numeric.max()) if raw_n else np.nan,
                    ...
                    "possible_po2_contamination": bool(possible_po2_contamination),
...
# optional export
if len(panel_poc) > 0:
    poc_path = PRIOR_RUNS_DIR / f"gas_panels_poc_{RUN_TIMESTAMP}.parquet"
...
if "pco2_threshold_any" not in ed_df.columns:
    ed_df["pco2_threshold_any"] = ed_df["qualifying_pco2_time"].notna().astype("Int64")
else:
...
threshold_any = (
    pd.to_numeric(ed_df.get("pco2_threshold_any", pd.Series(0, index=ed_df.index)), errors="coerce")
    .fillna(0)
    .astype(int)
)
any_icd = (
...
hco3_values_for_band = pd.to_numeric(ed_df.get("first_hco3"), errors="coerce")
hco3_qc_pass = (
...
hco3_band_qc_inconsistency_n = int((ed_df["hco3_band"].notna() & ~hco3_qc_pass).sum())
if hco3_band_qc_inconsistency_n > 0:
    raise ValueError(
...
ed_df["enrolled_any"] = (
    ...
    | (ed_df["pco2_threshold_any"] == 1)
).astype("Int64")
ed_df["enrolled_any_icd_union_secondary"] = ed_df["enrolled_any"].astype("Int64")
ed_df["enrollment_route"] = np.select(
    [
        (ed_df["any_hypercap_icd"] == 0) & (ed_df["pco2_threshold_any"] == 1),
        (ed_df["any_hypercap_icd"] == 1) & (ed_df["pco2_threshold_any"] == 0),
        (ed_df["any_hypercap_icd"] == 1) & (ed_df["pco2_threshold_any"] == 1),
    ],
    ["GAS_ONLY", "ICD_ONLY", "ICD+GAS"],
    default="NONE",
)
...
max_pco2_0_24h_lt_qualifying_n = 0
enrolled_mask = pd.to_numeric(ed_df["pco2_threshold_any"], errors="coerce").fillna(0).astype(int).eq(1)
if enrolled_mask.any():
    ...
    if out_of_window_n:
        raise ValueError(
            f"Found {out_of_window_n} enrolled rows with negative/missing dt_qualifying_hypercapnia_hours."
        )
    ...
    if {"max_pco2_0_24h", "qualifying_pco2_mmhg"}.issubset(ed_df.columns):
        qualifying_values_24h = pd.to_numeric(
            ed_df.loc[within_24_mask, "qualifying_pco2_mmhg"], errors="coerce"
        )
...
# 1) Key uniqueness
if "ed_stay_id" not in ed_df.columns:
    raise KeyError("ed_df missing ed_stay_id in QA block.")
try:
...
# 2) Inclusion sanity: enrolled rows must match ICD OR gas-qualified any-time.
enrolled = pd.to_numeric(ed_df.get('enrolled_any'), errors='coerce').fillna(0).astype(int)
threshold = pd.to_numeric(ed_df.get('pco2_threshold_any'), errors='coerce').fillna(0).astype(int)
...
print('QA: enrolled_any != (any_hypercap_icd OR pco2_threshold_any) count:', mismatch_n)
if STRICT_QA and mismatch_n > 0:
    raise AssertionError('Found rows where enrolled_any disagrees with ICD OR any-time gas rule.')

# 3) Temporal sanity: first_gas_time >= ed_intime (allow small negative drift)
if 'first_gas_time' in ed_df.columns and 'ed_intime' in ed_df.columns:
...
# Snapshot pre-clean frames for scope-specific raw-vs-clean audit artifacts.
ed_df_pre_clean = ed_df.copy(deep=True)
df_pre_clean = df.copy(deep=True) if "df" in globals() else pd.DataFrame()
...
clean_bounds = {
    "ed_triage_temp_f_clean": (50.0, 115.0),
    "ed_first_temp_f_clean": (50.0, 115.0),
    "ed_triage_hr_model": (20.0, 300.0),
    "ed_first_hr_model": (20.0, 300.0),
    "ed_triage_o2sat_clean": (0.0, 100.0),
    "ed_first_o2sat_clean": (0.0, 100.0),
    "ed_triage_sbp_clean": (20.0, 300.0),
    "ed_first_sbp_clean": (20.0, 300.0),
...
for temp_clean_column in ("ed_triage_temp_f_clean", "ed_first_temp_f_clean"):
    ...
    residual_celsius_like_n = int(numeric.between(25.0, 45.0, inclusive="both").sum())
    if residual_celsius_like_n:
        raise ValueError(
            f"Found {residual_celsius_like_n} cleaned values still in 25–45°F band in {temp_clean_column}."
        )
...
pain_debug_rows = []
for pain_column in ("ed_triage_pain", "ed_first_pain"):
    if pain_column not in ed_df.columns:
        continue
    top_values = (
        ed_df[pain_column]
        .astype("string")
    ...
    for raw_value, count in top_values.items():
        pain_debug_rows.append(
            {
                "column_name": pain_column,
                "raw_pain_value": str(raw_value),
                "count": int(count),
            }
        )
pain_value_examples = pd.DataFrame(pain_debug_rows)
pain_value_examples_path = PRIOR_RUNS_DIR / f"{out_date} ed_pain_raw_top20.csv"
pain_value_examples.to_csv(pain_value_examples_path, index=False)
print("Wrote:", pain_value_examples_path)

audits_root = WORK_DIR / "debug" / "audits"
ed_spine_audits_dir = audits_root / "ed_spine"
ed_spine_audits_dir.mkdir(parents=True, exist_ok=True)
vitals_outlier_ed_spine_pre_clean = build_vitals_outlier_phase_audit(
    ed_df_pre_clean,
    scope_name="ed_spine",
    phase_name="raw_pre_clean",
    use_clean_columns=False,
)
vitals_outlier_ed_spine_post_clean = build_vitals_outlier_phase_audit(
    ed_df,
    scope_name="ed_spine",
    phase_name="clean_post_clean",
    use_clean_columns=True,
)
vitals_outlier_ed_spine_pre_clean_path = (
    ed_spine_audits_dir / "vitals_outlier_audit_raw_pre_clean.csv"
)
vitals_outlier_ed_spine_post_clean_path = (
    ed_spine_audits_dir / "vitals_outlier_audit_clean_post_clean.csv"
)
vitals_outlier_ed_spine_pre_clean.to_csv(
    vitals_outlier_ed_spine_pre_clean_path, index=False
)
vitals_outlier_ed_spine_post_clean.to_csv(
    vitals_outlier_ed_spine_post_clean_path, index=False
)
print("Wrote:", vitals_outlier_ed_spine_pre_clean_path)
print("Wrote:", vitals_outlier_ed_spine_post_clean_path)
...
qualifying_ph_numeric = pd.to_numeric(
    ed_df.get("qualifying_ph", pd.Series(index=ed_df.index, dtype="float64")),
    errors="coerce",
)
qualifying_ph_status = (
    ed_df.get(
        "qualifying_ph_pairing_status",
        pd.Series("missing", index=ed_df.index, dtype="string"),
    )
    .astype("string")
    .fillna("missing")
)
qualifying_ph_present = qualifying_ph_numeric.notna()
pco2_any_bool = threshold_any.eq(1)
any_icd_bool = any_icd.eq(1)

qualifying_ph_scope_specs = [
    (
        "broad_gas_positive",
        "Broad gas-positive admissions",
        pco2_any_bool,
    ),
    (
        "first_qualifying_gas_within_24h",
        "First qualifying gas within 24h",
        pco2_any_bool & qualifying_hours.le(24.0),
    ),
    (
        "first_qualifying_gas_within_6h",
        "First qualifying gas within 6h",
        pco2_any_bool & qualifying_hours.lt(6.0),
    ),
    (
        "late_gas_after_24h",
        "First qualifying gas after 24h",
        pco2_any_bool & qualifying_hours.gt(24.0),
    ),
    (
        "icd_plus_gas",
        "ICD-positive plus qualifying gas",
        any_icd_bool & pco2_any_bool,
    ),
    (
        "icd_only",
        "ICD-only admissions without qualifying gas",
        any_icd_bool & ~pco2_any_bool,
    ),
]

qualifying_ph_rows: list[dict[str, Any]] = []
for scope_key, scope_label, scope_mask_raw in qualifying_ph_scope_specs:
    scope_mask = scope_mask_raw.fillna(False).astype(bool)
    denominator_n = int(scope_mask.sum())
    qualifying_gas_n = int((scope_mask & pco2_any_bool).sum())
    paired_ph_available_n = int((scope_mask & pco2_any_bool & qualifying_ph_present).sum())
    paired_ph_missing_n = int((scope_mask & pco2_any_bool & ~qualifying_ph_present).sum())
    status_counts = (
        qualifying_ph_status.loc[scope_mask]
        .value_counts(dropna=False)
        .to_dict()
    )
    qualifying_ph_rows.append(
        {
            "scope_key": scope_key,
            "scope_label": scope_label,
            "denominator_n": denominator_n,
            "qualifying_gas_n": qualifying_gas_n,
            "paired_ph_available_n": paired_ph_available_n,
            "paired_ph_missing_n": paired_ph_missing_n,
            "paired_ph_available_pct": float(paired_ph_available_n / qualifying_gas_n * 100)
            if qualifying_gas_n
            else 0.0,
            "no_qualifying_gas_n": int(denominator_n - qualifying_gas_n),
            "paired_same_specimen_panel_n": int(status_counts.get("paired_same_specimen_panel", 0)),
            "paired_same_time_panel_n": int(status_counts.get("paired_same_time_panel", 0)),
            "qualifying_panel_missing_ph_n": int(status_counts.get("qualifying_panel_missing_ph", 0)),
        }
    )

qualifying_ph_pairing_completeness_audit = pd.DataFrame(qualifying_ph_rows)
qualifying_ph_pairing_completeness_audit_path = (
    PRIOR_RUNS_DIR / f"{out_date} qualifying_ph_pairing_completeness_audit.csv"
)
qualifying_ph_pairing_completeness_audit.to_csv(
    qualifying_ph_pairing_completeness_audit_path,
    index=False,
)
print("Wrote:", qualifying_ph_pairing_completeness_audit_path)
...
# Compute UOM checks against a deterministic preview of final export scope.
_mask_df_cc = chief_complaint_inclusion_mask(df["ed_triage_cc"])
_df_cc = df.loc[_mask_df_cc].copy()
_mask_cc = chief_complaint_inclusion_mask(ed_df["ed_triage_cc"])
...
if len(panel) > 0 and "pco2" in panel.columns:
...
if "df" in globals() and "ed_triage_cc" in df.columns:
    _hadm_cc_mask = chief_complaint_inclusion_mask(df["ed_triage_cc"])
...
qualifying_ph_pairing_completeness_audit_records = (
    json.loads(qualifying_ph_pairing_completeness_audit.to_json(orient="records"))
    if "qualifying_ph_pairing_completeness_audit" in globals()
    else []
)
...
dt_iqr = [
    float(dt_non_missing.quantile(0.25)) if not dt_non_missing.empty else None,
    float(dt_non_missing.quantile(0.75)) if not dt_non_missing.empty else None,
]
dt_within_24h_rate = float(
    (pco2_any_flag.eq(1) & pco2_24h_flag.eq(1)).sum() / max(int(pco2_any_flag.eq(1).sum()), 1)
)
dt_after_24h_rate = float(
    (pco2_any_flag.eq(1) & pco2_24h_flag.eq(0)).sum() / max(int(pco2_any_flag.eq(1).sum()), 1)
)

# T4 — QA summary artifact.
qa_summary = {
    "run_id": f"cohort_{RUN_TIMESTAMP}",
    "scope_name": "ed_spine",
    "n_rows": int(len(ed_df)),
    "exported_workbook_path": "MIMIC tabular data/MIMICIV all with CC.xlsx",
    ...
    "dt_first_qualifying_gas_hours_non_missing_n": int(dt_non_missing.shape[0]),
    "dt_first_qualifying_gas_hours_median": float(dt_non_missing.median()) if not dt_non_missing.empty else None,
    "dt_first_qualifying_gas_hours_iqr": dt_iqr,
    "dt_first_qualifying_gas_hours_max": float(dt_non_missing.max()) if not dt_non_missing.empty else None,
    ...
    "qualifying_source_branch_counts": (
        ed_df.loc[
            pd.to_numeric(ed_df.get("pco2_threshold_any"), errors="coerce").fillna(0).astype(int).eq(1),
            "qualifying_source_branch",
        ]
        .fillna("UNKNOWN")
        .astype("string")
    ...
    "qualifying_anchor_ed_n": int(globals().get("qualifying_anchor_ed_n", 0)),
    "qualifying_anchor_fallback_n": int(globals().get("qualifying_anchor_fallback_n", 0)),
    ...
    "qualifying_ph_pairing_completeness_audit": qualifying_ph_pairing_completeness_audit_records,
    ...
    "ed_vitals_audit_paths": {
        ...
        "pain_raw_top20": str(pain_value_examples_path),
        "ed_spine_raw_pre_clean": str(vitals_outlier_ed_spine_pre_clean_path),
        "ed_spine_clean_post_clean": str(vitals_outlier_ed_spine_post_clean_path),
    },
    "cohort_flow_counts": (
        json.loads(flow_counts.to_json(orient="records"))
        if "flow_counts" in globals()
        else []
    ),
    "cohort_flow_counts_lookup": flow_counts_lookup if "flow_counts_lookup" in globals() else {},
    "cohort_flow_counts_path": str(flow_counts_path) if "flow_counts_path" in globals() else None,
    "ed_only_exclusion_audit": ed_only_exclusion_audit_records
    if "ed_only_exclusion_audit_records" in globals()
    else [],
    "ed_only_exclusion_audit_paths": {
        "csv": str(ed_only_exclusion_audit_csv_path)
        if "ed_only_exclusion_audit_csv_path" in globals()
        else None,
        "json": str(ed_only_exclusion_audit_json_path)
        if "ed_only_exclusion_audit_json_path" in globals()
        else None,
    },
    "ed_only_gas_eligibility_assessability": "not_assessable",
    "ed_only_gas_eligibility_note": ed_only_gas_eligibility_note
    if "ed_only_gas_eligibility_note" in globals()
    else None,
    "blood_gas_audit_paths": {
        ...
        "qualifying_ph_pairing_completeness": str(
            qualifying_ph_pairing_completeness_audit_path
        )
        if "qualifying_ph_pairing_completeness_audit_path" in globals()
        else None,
...
qa_dir = WORK_DIR / "artifacts" / "qa" / "cohort"
qa_dir.mkdir(parents=True, exist_ok=True)
qa_ed_spine_path = qa_dir / "qa_summary_ed_spine.json"
qa_ed_spine_path.write_text(json.dumps(qa_summary, indent=2))
qa_path = qa_dir / "qa_summary.json"
qa_path.write_text(json.dumps(qa_summary, indent=2))
print("Wrote:", qa_ed_spine_path)
print("Wrote:", qa_path)
```
...
cohort_path = PRIOR_RUNS_DIR / f"cohort_ed_stay_{RUN_TIMESTAMP}.parquet"
ed_vitals_path = PRIOR_RUNS_DIR / f"ed_vitals_long_{RUN_TIMESTAMP}.parquet"
labs_path = PRIOR_RUNS_DIR / f"labs_long_{RUN_TIMESTAMP}.parquet"
gas_panels_path = PRIOR_RUNS_DIR / f"gas_panels_{RUN_TIMESTAMP}.parquet"
...
if WRITE_ARCHIVE_XLSX_EXPORTS:
    xlsx_path = archive_xlsx_path("mimic_hypercap_EXT_EDstay_bq_gas")
    archive_ed_df = prepare_archive_export_frame(
        ed_df,
        export_name="mimic_hypercap_EXT_EDstay_bq_gas",
    )

    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as xw:
        archive_ed_df.to_excel(xw, sheet_name="cohort_ed_stay", index=False)
...
if WRITE_ARCHIVE_XLSX_EXPORTS:
    all_path = archive_xlsx_path("mimic_hypercap_EXT_all_encounters_bq")
    archive_all_encounters = prepare_archive_export_frame(
        all_encounters,
        export_name="mimic_hypercap_EXT_all_encounters_bq",
    )
    with pd.ExcelWriter(all_path, engine="openpyxl") as xw:
        archive_all_encounters.to_excel(
            xw,
            sheet_name="all_encounters",
            index=False,
        )
...
mask_cc = chief_complaint_inclusion_mask(ed_df["ed_triage_cc"])
ed_cc_only = ed_df.loc[mask_cc].copy()

if WRITE_ARCHIVE_XLSX_EXPORTS:
    cc_path = archive_xlsx_path("mimic_hypercap_EXT_EDcc_only_edstay_bq")
    archive_ed_cc_only = prepare_archive_export_frame(
        ed_cc_only,
        export_name="mimic_hypercap_EXT_EDcc_only_edstay_bq",
    )
    with pd.ExcelWriter(cc_path, engine="openpyxl") as xw:
        archive_ed_cc_only.to_excel(xw, sheet_name="ed_cc_only", index=False)
    print("Saved:", cc_path)
else:
...
# Deterministic production path: always derive export frames from canonical
# in-memory stage outputs (`df` and `ed_df`) instead of globals-based fallback.
...
cc_inclusion_mask = chief_complaint_inclusion_mask(final_cc["ed_triage_cc"])
if (~cc_inclusion_mask).any():
    pseudo_rows = int((~cc_inclusion_mask).sum())
    raise ValueError(
        "Final CC export contains pseudo-missing ED chief complaints; "
        f"unexpected rows={pseudo_rows}."
    )

# Canonicalize cohort-facing demographic fields so downstream stages carry one stable name.
if "gender" in final_cc.columns:
    gender_series = final_cc["gender"].astype("string").str.strip()
else:
    gender_series = pd.Series(pd.NA, index=final_cc.index, dtype="string")
if "ed_gender" in final_cc.columns:
    ed_gender_series = final_cc["ed_gender"].astype("string").str.strip()
    final_cc["gender"] = gender_series.mask(
        gender_series.isna() | gender_series.eq(""),
        ed_gender_series,
    )
else:
    final_cc["gender"] = gender_series

if "age" in final_cc.columns:
    age_series = pd.to_numeric(final_cc["age"], errors="coerce")
else:
    age_series = pd.Series(np.nan, index=final_cc.index, dtype="float64")
if "age_at_admit" in final_cc.columns:
    age_at_admit_series = pd.to_numeric(final_cc["age_at_admit"], errors="coerce")
    final_cc["age"] = age_series.where(age_series.notna(), age_at_admit_series)
else:
    final_cc["age"] = age_series

if "race" in final_cc.columns:
    race_series = final_cc["race"].astype("string").str.strip()
else:
    race_series = pd.Series(pd.NA, index=final_cc.index, dtype="string")
if "ed_race" in final_cc.columns:
    ed_race_series = final_cc["ed_race"].astype("string").str.strip()
    race_series = race_series.mask(race_series.isna() | race_series.eq(""), ed_race_series)
if "hosp_race" in final_cc.columns:
    hosp_race_series = final_cc["hosp_race"].astype("string").str.strip()
    race_series = race_series.mask(race_series.isna() | race_series.eq(""), hosp_race_series)
final_cc["race"] = race_series
...
final_cc["pco2_threshold_0_24h"] = (
    ...
    & pd.to_numeric(
        final_cc.get("dt_qualifying_hypercapnia_hours", pd.Series(np.nan, index=final_cc.index)),
        errors="coerce",
    ).le(24.0)
).astype("Int64")
...
final_cc["enrollment_route"] = np.select(
    [
        (final_cc["any_hypercap_icd"] == 0) & (final_cc["pco2_threshold_any"] == 1),
        (final_cc["any_hypercap_icd"] == 1) & (final_cc["pco2_threshold_any"] == 0),
        (final_cc["any_hypercap_icd"] == 1) & (final_cc["pco2_threshold_any"] == 1),
    ],
    ["GAS_ONLY", "ICD_ONLY", "ICD+GAS"],
    default="NONE",
)
...
missing_imv_timing_columns = sorted(
    imv_timing_required_columns.difference(final_cc.columns)
)
if missing_imv_timing_columns:
    raise KeyError(
        "Cannot build IMV/qualifying-gas timing audit; missing fields: "
        f"{missing_imv_timing_columns}."
    )

analytic_n = int(len(final_cc))
imv_gas_positive_mask = (
    pd.to_numeric(final_cc["pco2_threshold_any"], errors="coerce")
    .fillna(0)
    .astype(int)
    .eq(1)
)
imv_gas_positive_n = int(imv_gas_positive_mask.sum())
imv_icd_only_mask = final_cc["enrollment_route"].astype("string").eq("ICD_ONLY")
imv_icd_only_n = int(imv_icd_only_mask.sum())
imv_order = final_cc["imv_qualifying_gas_order"].astype("string")
imv_temporal_strata = [
    "no_observed_imv",
    "qualifying_gas_before_imv",
    "imv_before_qualifying_gas",
...
imv_temporal_counts = {
    stratum: int((imv_gas_positive_mask & imv_order.eq(stratum)).sum())
    for stratum in imv_temporal_strata
}
if sum(imv_temporal_counts.values()) != imv_gas_positive_n:
    raise AssertionError(
        "IMV temporal strata do not reconcile to all gas-positive admissions: "
        f"strata_sum={sum(imv_temporal_counts.values())}, gas_positive={imv_gas_positive_n}."
    )
if not bool(
    imv_order.loc[imv_icd_only_mask]
    .eq("not_applicable_no_qualifying_gas")
    .fillna(False)
    .all()
):
    raise AssertionError(
        "Every ICD-only admission must be classified as not_applicable_no_qualifying_gas."
    )

imv_source = final_cc["first_observed_imv_source"].astype("string")
observed_imv_source_values = set(imv_source.dropna().astype(str))
if not observed_imv_source_values.issubset(IMV_OBSERVED_SOURCE_VALUES):
    raise AssertionError(
        "Unexpected first_observed_imv_source values: "
        f"{sorted(observed_imv_source_values.difference(IMV_OBSERVED_SOURCE_VALUES))}."
    )

robust_imv_mask = (
    pd.to_numeric(final_cc["robust_imv_observed"], errors="coerce")
    .fillna(0)
    .astype(int)
    .eq(1)
)
source_record_no_time_mask = (
    pd.to_numeric(
        final_cc[IMV_SOURCE_RECORD_NO_TIME_COLUMN], errors="coerce"
    )
    .fillna(0)
    .astype(int)
    .eq(1)
...
legacy_imv_mask = (
    pd.to_numeric(final_cc["imv_flag"], errors="coerce")
    .fillna(0)
    .astype(int)
    .eq(1)
)
qualifying_times_for_imv = pd.to_datetime(
    final_cc["qualifying_pco2_time"], errors="coerce"
)
first_observed_times_for_imv = pd.to_datetime(
    final_cc["first_observed_imv_time"], errors="coerce"
)
first_derived_times_for_imv = pd.to_datetime(
    final_cc["first_derived_imv_starttime"], errors="coerce"
)
first_icu_times_for_imv = pd.to_datetime(final_cc["icu_intime"], errors="coerce")
exact_imv_gas_tie_mask = (
    imv_gas_positive_mask
    & robust_imv_mask
    & qualifying_times_for_imv.notna()
    & first_observed_times_for_imv.notna()
    & qualifying_times_for_imv.eq(first_observed_times_for_imv)
)
legacy_without_robust_mask = legacy_imv_mask & ~robust_imv_mask
robust_without_legacy_mask = robust_imv_mask & ~legacy_imv_mask
derived_first_icu_comparable_mask = (
    first_derived_times_for_imv.notna() & first_icu_times_for_imv.notna()
)
derived_at_first_icu_mask = (
    derived_first_icu_comparable_mask
    & first_derived_times_for_imv.eq(first_icu_times_for_imv)
)
hours_from_imv_to_gas = pd.to_numeric(
    final_cc["hours_from_imv_to_qualifying_gas"], errors="coerce"
).dropna()

imv_audit_rows: list[dict[str, Any]] = []


def _append_imv_audit_count(
    *,
    section: str,
    metric: str,
    count: int,
    ...
    imv_audit_rows.append(
        {
            ...
            "percentage": (
                float(count / denominator * 100.0) if denominator > 0 else np.nan
            ),
            "value": np.nan,
            "units": "count_and_percent",
            "definition": definition,
        }
    )


_append_imv_audit_count(
    section="cohort_size",
    metric="total_analytic_admissions",
    count=analytic_n,
    denominator=analytic_n,
    definition="Final ED-chief-complaint analytic admissions after unchanged primary enrollment filtering.",
)
_append_imv_audit_count(
    section="cohort_size",
    metric="total_gas_positive_admissions",
    count=imv_gas_positive_n,
    denominator=analytic_n,
    definition="Analytic admissions with pco2_threshold_any equal to 1.",
)
_append_imv_audit_count(
    section="cohort_size",
    metric="total_icd_only_admissions",
    count=imv_icd_only_n,
    denominator=analytic_n,
    definition="Analytic admissions in the mutually exclusive ICD_ONLY enrollment route.",
)
for stratum in imv_temporal_strata:
    _append_imv_audit_count(
        section="temporal_stratum",
        metric="gas_positive_temporal_stratum",
        category=stratum,
        count=imv_temporal_counts[stratum],
        denominator=imv_gas_positive_n,
        definition="Mutually exclusive strict timestamp classification among gas-positive admissions.",
    )
...
for source_value in (
    ...
    _append_imv_audit_count(
        section="imv_source",
        metric="first_observed_imv_source",
        category=source_value,
        count=int(imv_source.eq(source_value).sum()),
        denominator=analytic_n,
        definition="Source attaining the exact earliest reliable observed IMV timestamp; missing denotes no reliable source timestamp.",
    )

for metric, mask, denominator, definition in (
    (
        "exact_timestamp_ties",
        exact_imv_gas_tie_mask,
        imv_gas_positive_n,
        "Qualifying pCO2 time exactly equals first observed IMV time; classified timing_indeterminate.",
    ),
    (
        "robust_imv_evidence_with_missing_first_observed_time",
        source_record_no_time_mask,
        int(robust_imv_mask.sum() + source_record_no_time_mask.sum()),
        "An official derived/procedure IMV source record is present but no reliable source timestamp is available; robust_imv_observed remains zero and gas-positive admissions are timing_indeterminate.",
    ),
    (
        "legacy_imv_flag_without_robust_imv_evidence",
        legacy_without_robust_mask,
        int(legacy_imv_mask.sum()),
        "Combined legacy IMV flag is positive without any official derived/procedure IMV timestamp.",
    ),
    (
        "robust_imv_evidence_without_legacy_imv_flag",
        robust_without_legacy_mask,
        int(robust_imv_mask.sum()),
        "Official derived/procedure IMV evidence is present while the combined legacy IMV flag is negative.",
    ),
    (
        "first_derived_imv_episode_at_first_icu_intime",
        derived_at_first_icu_mask,
        int(derived_first_icu_comparable_mask.sum()),
        "First derived InvasiveVent episode starts exactly at the admission's first ICU intime; descriptive left-censoring marker only.",
    ),
    (
        "legacy_imv_timing_discordant",
        pd.to_numeric(
            final_cc["legacy_imv_timing_discordant"], errors="coerce"
        ).fillna(0).astype(int).eq(1),
        analytic_n,
        "Robust and legacy IMV evidence disagree in presence or in strict ordering when both timestamps are comparable.",
    ),
):
    _append_imv_audit_count(
        section="qa_reconciliation",
        metric=metric,
        count=int(mask.sum()),
        denominator=int(denominator),
        definition=definition,
...
imv_audit_rows.append(
    {
        "section": "timing_distribution",
        "metric": "hours_from_imv_to_qualifying_gas_nonmissing_n",
        "category": "",
        "count": int(len(hours_from_imv_to_gas)),
        "denominator": analytic_n,
        "percentage": float(len(hours_from_imv_to_gas) / analytic_n * 100.0) if analytic_n else np.nan,
        "value": np.nan,
        "units": "count_and_percent",
        "definition": "Admissions with both qualifying gas and first observed IMV timestamps.",
    }
)
for metric, quantile in (
    ("minimum", 0.00),
    ("p05", 0.05),
    ("p25", 0.25),
    ("median", 0.50),
    ...
    imv_audit_rows.append(
        {
            "section": "timing_distribution",
            "metric": f"hours_from_imv_to_qualifying_gas_{metric}",
            "category": "",
            "count": pd.NA,
            "denominator": int(len(hours_from_imv_to_gas)),
            "percentage": np.nan,
            "value": (
                float(hours_from_imv_to_gas.quantile(quantile))
                if not hours_from_imv_to_gas.empty
                else np.nan
            ),
            "units": "hours",
            "definition": "qualifying_pco2_time minus first_observed_imv_time; negative means gas first, positive means IMV first.",
        }
    )

imv_qualifying_gas_timing_audit = pd.DataFrame(imv_audit_rows)
...
if restricted_audit_columns.intersection(imv_qualifying_gas_timing_audit.columns):
    raise AssertionError("Aggregate IMV timing audit unexpectedly contains row-level fields.")
imv_qualifying_gas_timing_audit_path = (
    cohort_qa_dir / "imv_qualifying_gas_timing_audit.csv"
)
imv_qualifying_gas_timing_audit.to_csv(
    imv_qualifying_gas_timing_audit_path,
    index=False,
)
print("Wrote:", imv_qualifying_gas_timing_audit_path)
...
if "ed_stay_id" not in final_cc.columns:
    raise KeyError("ed_stay_id is required to export gas source diagnostics artifact.")
...
for rate_column in ("gas_source_hint_conflict_rate", "gas_source_resolved_rate"):
...
if gas_source_diag_export["ed_stay_id"].duplicated().any():
    raise ValueError("gas_source_diagnostics_by_ed_stay requires one row per ed_stay_id.")
gas_source_diagnostics_artifact_path = cohort_qa_dir / "gas_source_diagnostics_by_ed_stay.csv"
gas_source_diag_export.to_csv(gas_source_diagnostics_artifact_path, index=False)
...
qa_dir.mkdir(parents=True, exist_ok=True)
qa_ed_spine_path = qa_dir / "qa_summary_ed_spine.json"
if qa_ed_spine_path.exists():
    qa_payload = json.loads(qa_ed_spine_path.read_text())
else:
    qa_payload = {}
...
qa_payload["imv_qualifying_gas_timing_audit_path"] = str(
    imv_qualifying_gas_timing_audit_path
)
qa_payload["imv_qualifying_gas_temporal_counts"] = imv_temporal_counts
qa_payload["robust_imv_observed_n"] = int(robust_imv_mask.sum())
qa_payload["run_id"] = f"cohort_{RUN_TIMESTAMP}"
qa_payload["scope_name"] = "ed_cc"
qa_payload["n_rows"] = int(len(final_cc))
qa_payload["ed_cc_rows"] = int(len(final_cc))
qa_payload["ed_spine_rows"] = int(len(ed_df))
qa_payload["hadm_rows"] = int(len(final_cc))
qa_payload["hadm_cc_rows"] = int(len(final_cc))
qa_payload["exported_workbook_path"] = "MIMIC tabular data/MIMICIV all with CC.xlsx"
qa_payload["qa_scope_notes"] = {
    "primary_summary_scope": "ed_cc",
    "secondary_summary_scope": "ed_spine",
    "secondary_summary_path": "artifacts/qa/cohort/qa_summary_ed_spine.json",
}
qa_payload["metric_scope"] = {
    "default": "ed_cc",
    "legacy_ed_spine_metrics": [
        "pct_any_gas_0_6h",
        "pct_any_gas_0_24h",
...
qa_payload["pco2_threshold_any_n"] = int(
    pd.to_numeric(final_cc.get("pco2_threshold_any"), errors="coerce")
    .fillna(0)
    .astype(int)
    .sum()
)
qa_payload["pco2_threshold_any_rate"] = float(
    pd.to_numeric(final_cc.get("pco2_threshold_any"), errors="coerce")
    .fillna(0)
    .astype(int)
    .mean()
)
qa_payload["pco2_threshold_0_24h_n"] = int(
    pd.to_numeric(final_cc.get("pco2_threshold_0_24h"), errors="coerce")
    .fillna(0)
    .astype(int)
    .sum()
)
qa_payload["pco2_threshold_0_24h_rate"] = float(
    pd.to_numeric(final_cc.get("pco2_threshold_0_24h"), errors="coerce")
    .fillna(0)
    .astype(int)
    .mean()
)
qa_payload["qualifying_source_branch_counts"] = (
    final_cc.get("qualifying_source_branch", pd.Series(dtype="string"))
    .fillna("UNKNOWN")
    .astype("string")
    .value_counts(dropna=False)
    .to_dict()
)
qa_payload["ed_only_exclusion_audit"] = (
    ed_only_exclusion_audit_records
    if "ed_only_exclusion_audit_records" in globals()
    else qa_payload.get("ed_only_exclusion_audit", [])
)
qa_payload["ed_only_exclusion_audit_paths"] = {
    "csv": str(ed_only_exclusion_audit_csv_path)
    if "ed_only_exclusion_audit_csv_path" in globals()
    else qa_payload.get("ed_only_exclusion_audit_paths", {}).get("csv"),
    "json": str(ed_only_exclusion_audit_json_path)
    if "ed_only_exclusion_audit_json_path" in globals()
    else qa_payload.get("ed_only_exclusion_audit_paths", {}).get("json"),
}
qa_payload["ed_only_gas_eligibility_assessability"] = "not_assessable"
qa_payload["ed_only_gas_eligibility_note"] = (
    ed_only_gas_eligibility_note
    if "ed_only_gas_eligibility_note" in globals()
    else qa_payload.get("ed_only_gas_eligibility_note")
)
ed_cc_audits_dir = (WORK_DIR / "debug" / "audits" / "ed_cc")
vitals_outlier_ed_cc_pre_clean_path = (
    ed_cc_audits_dir / "vitals_outlier_audit_raw_pre_clean.csv"
)
vitals_outlier_ed_cc_post_clean_path = (
    ed_cc_audits_dir / "vitals_outlier_audit_clean_post_clean.csv"
)
qa_payload["vitals_outlier_scope_audit_paths"] = {
    "ed_spine_raw_pre_clean": str(vitals_outlier_ed_spine_pre_clean_path),
    "ed_spine_clean_post_clean": str(vitals_outlier_ed_spine_post_clean_path),
    "ed_cc_raw_pre_clean": str(vitals_outlier_ed_cc_pre_clean_path),
    "ed_cc_clean_post_clean": str(vitals_outlier_ed_cc_post_clean_path),
}
qa_ed_cc_path = qa_dir / "qa_summary_ed_cc.json"
qa_ed_cc_path.write_text(json.dumps(qa_payload, indent=2))
qa_path = qa_dir / "qa_summary.json"
qa_path.write_text(json.dumps(qa_payload, indent=2))
print("Updated:", qa_ed_cc_path, f"(qa_status_final={qa_status_final})")
...
# Overwrite ED vitals with cleaned/model values for analysis-facing exports.
# Gas fields keep the <=5% safety gate to avoid broad semantic drift.
mandatory_model_overwrite_specs = [
    ("ed_triage_hr", "ed_triage_hr_model"),
    ("ed_first_hr", "ed_first_hr_model"),
    ("ed_triage_rr", "ed_triage_rr_model"),
    ("ed_first_rr", "ed_first_rr_model"),
    ("ed_triage_temp", "ed_triage_temp_model"),
    ("ed_first_temp", "ed_first_temp_model"),
    ("ed_triage_pain", "ed_triage_pain_model"),
    ("ed_first_pain", "ed_first_pain_model"),
    ("ed_triage_sbp", "ed_triage_sbp_model"),
    ("ed_triage_dbp", "ed_triage_dbp_model"),
    ("ed_first_sbp", "ed_first_sbp_model"),
    ("ed_first_dbp", "ed_first_dbp_model"),
    ("ed_triage_o2sat", "ed_triage_o2sat_model"),
    ("ed_first_o2sat", "ed_first_o2sat_model"),
]
conditional_model_overwrite_specs = [
    ("first_ph", "first_ph_model"),
    ("first_lactate", "first_lactate_model"),
]
model_overwrite_threshold = 0.05
model_overwrite_rows: list[dict[str, object]] = []
model_columns_to_drop: set[str] = set()

for raw_column, model_column in mandatory_model_overwrite_specs:
    if raw_column not in final_cc.columns or model_column not in final_cc.columns:
        model_overwrite_rows.append(
            {
                "raw_column": raw_column,
                "model_column": model_column,
    ...
    raw_numeric = pd.to_numeric(final_cc[raw_column], errors="coerce")
    model_numeric = pd.to_numeric(final_cc[model_column], errors="coerce")
    changed_mask = ~(
        raw_numeric.eq(model_numeric) | (raw_numeric.isna() & model_numeric.isna())
    )
    changed_n = int(changed_mask.sum())
    changed_rate = float(changed_n / max(len(final_cc), 1))
    final_cc[raw_column] = model_numeric
    model_columns_to_drop.add(model_column)
    model_overwrite_rows.append(
        {
            "raw_column": raw_column,
            "model_column": model_column,
            "status": "mandatory_overwrite",
            "changed_n": changed_n,
...
for raw_column, model_column in conditional_model_overwrite_specs:
    if raw_column not in final_cc.columns or model_column not in final_cc.columns:
        model_overwrite_rows.append(
            {
                "raw_column": raw_column,
                ...
                "changed_n": pd.NA,
                "changed_rate": pd.NA,
                "threshold": model_overwrite_threshold,
                "overwrite_applied": False,
            }
        )
        continue
    ...
    changed_n = int(changed_mask.sum())
    changed_rate = float(changed_n / max(len(final_cc), 1))
    overwrite_applied = changed_rate <= model_overwrite_threshold

    if overwrite_applied:
        final_cc[raw_column] = model_numeric
        model_columns_to_drop.add(model_column)
        status = "conditional_overwrite"
    else:
        status = "conditional_retained_gt5pct_change"
...
model_overwrite_audit = pd.DataFrame(model_overwrite_rows)
model_overwrite_audit_path = PRIOR_RUNS_DIR / f"{out_date} model_overwrite_audit.csv"
model_overwrite_audit.to_csv(model_overwrite_audit_path, index=False)
print("Wrote:", model_overwrite_audit_path)

gt5_model_changes = model_overwrite_audit.loc[
    model_overwrite_audit["status"] == "conditional_retained_gt5pct_change",
    ["raw_column", "model_column", "changed_n", "changed_rate"],
]
if not gt5_model_changes.empty:
    print("Model-to-raw overwrite skipped (>5% changed rows):")
    print(gt5_model_changes.to_string(index=False))

ed_cc_audits_dir.mkdir(parents=True, exist_ok=True)
if not df_pre_clean.empty and "ed_triage_cc" in df_pre_clean.columns:
    ed_cc_pre_clean = df_pre_clean.loc[
        chief_complaint_inclusion_mask(df_pre_clean["ed_triage_cc"])
    ].copy()
else:
    ed_cc_pre_clean = pd.DataFrame()

vitals_outlier_ed_cc_pre_clean = build_vitals_outlier_phase_audit(
    ed_cc_pre_clean,
    scope_name="ed_cc",
    phase_name="raw_pre_clean",
    use_clean_columns=False,
)
vitals_outlier_ed_cc_post_clean = build_vitals_outlier_phase_audit(
    final_cc,
    scope_name="ed_cc",
    phase_name="clean_post_clean",
    use_clean_columns=False,
...
vitals_outlier_ed_cc_pre_clean.to_csv(
    vitals_outlier_ed_cc_pre_clean_path, index=False
)
vitals_outlier_ed_cc_post_clean.to_csv(
    vitals_outlier_ed_cc_post_clean_path, index=False
)
print("Wrote:", vitals_outlier_ed_cc_pre_clean_path)
print("Wrote:", vitals_outlier_ed_cc_post_clean_path)
...
vitals_and_gas_qc_only_columns = [
    "ed_triage_hr_model",
    "ed_triage_hr_outlier_flag",
    "ed_first_hr_model",
    "ed_first_hr_outlier_flag",
    "ed_triage_rr_model",
    "ed_triage_rr_outlier_flag",
    "ed_first_rr_model",
    "ed_first_rr_outlier_flag",
    "ed_triage_temp_f_clean",
    "ed_triage_temp_was_celsius_like",
    "ed_triage_temp_out_of_range",
    "ed_triage_temp_f_model",
    "ed_triage_temp_c_model",
    "ed_triage_temp_model",
    "ed_triage_temp_outlier_flag",
    "ed_triage_pain_clean",
    "ed_triage_pain_is_sentinel_13",
    "ed_triage_pain_out_of_range",
    "ed_triage_pain_parsed_from_fraction",
    "ed_triage_pain_parsed_from_text_numeric",
    "ed_triage_pain_non_numeric_set_na",
    "ed_triage_pain_model",
    "ed_triage_pain_outlier_flag",
    "ed_triage_sbp_clean",
    "ed_triage_dbp_clean",
    "ed_triage_sbp_out_of_range",
    "ed_triage_dbp_out_of_range",
    "ed_triage_sbp_model",
    "ed_triage_dbp_model",
    "ed_triage_sbp_outlier_flag",
    "ed_triage_dbp_outlier_flag",
    "ed_triage_o2sat_clean",
    "ed_triage_o2sat_gt_100",
    "ed_triage_o2sat_out_of_range",
    "ed_triage_o2sat_zero",
    "ed_triage_o2sat_model",
    "ed_triage_o2sat_outlier_flag",
    "ed_first_temp_f_clean",
    "ed_first_temp_was_celsius_like",
    "ed_first_temp_out_of_range",
    "ed_first_temp_f_model",
    "ed_first_temp_c_model",
    "ed_first_temp_model",
    "ed_first_temp_outlier_flag",
    "ed_first_pain_clean",
    "ed_first_pain_is_sentinel_13",
    "ed_first_pain_out_of_range",
    "ed_first_pain_parsed_from_fraction",
    ...
    "ed_first_pain_non_numeric_set_na",
    "ed_first_pain_model",
    "ed_first_pain_outlier_flag",
    "ed_first_sbp_clean",
    "ed_first_dbp_clean",
    "ed_first_sbp_out_of_range",
    "ed_first_dbp_out_of_range",
    "ed_first_sbp_model",
    "ed_first_dbp_model",
    "ed_first_sbp_outlier_flag",
    "ed_first_dbp_outlier_flag",
    "ed_first_o2sat_clean",
    "ed_first_o2sat_gt_100",
    "ed_first_o2sat_out_of_range",
    "ed_first_o2sat_zero",
    "ed_first_o2sat_model",
    "ed_first_o2sat_outlier_flag",
    "first_ph_model",
    "first_ph_outlier_flag",
    "first_pco2_model",
    "first_pco2_outlier_flag",
    "first_lactate_model",
    "first_lactate_outlier_flag",
]
# Capture source evidence before the internal field is removed. This sidecar is
# restricted input provenance, not an additional workbook field or result asset.
imv_source_provenance = build_imv_source_provenance(final_cc)
cohort_export_drop_columns = [
    ...
    "late_hypercapnia",
    *vitals_and_gas_qc_only_columns,
    *sorted(model_columns_to_drop),
]
cohort_export_drop_columns = list(dict.fromkeys(cohort_export_drop_columns))
...
if IMV_SOURCE_RECORD_NO_TIME_COLUMN in final_cc.columns:
    raise AssertionError("Canonical cohort export retains internal IMV source evidence.")

dated_out_path = PRIOR_RUNS_DIR / f"{out_date} MIMICIV all with CC.xlsx"
canonical_out_path = DATA_DIR / "MIMICIV all with CC.xlsx"
imv_source_provenance_path = DATA_DIR / IMV_SOURCE_PROVENANCE_FILENAME
dated_imv_source_provenance_path = (
    PRIOR_RUNS_DIR / f"{out_date} {IMV_SOURCE_PROVENANCE_FILENAME}"
)
...
def _manifest_relpath(path_value: Any) -> str | None:
...
# Verify the persisted representation too: no Excel timestamp precision loss or
# export transformation may silently invalidate the cohort-produced source proof.
exported_imv_source_projection = pd.read_excel(
    canonical_out_path, usecols=list(IMV_SOURCE_PROJECTION_COLUMNS), engine="openpyxl"
)
if imv_source_projection_sha256(exported_imv_source_projection) != imv_source_provenance["source_projection_sha256"]:
    raise AssertionError("Canonical cohort export changed the IMV source projection.")
write_imv_source_provenance(imv_source_provenance, dated_imv_source_provenance_path)
write_imv_source_provenance(imv_source_provenance, imv_source_provenance_path)
imv_source_provenance_sha256 = _sha256_file(imv_source_provenance_path)
print(
    "EXPORT_SHAPE cohort",
    f"rows={len(final_cc):,}",
...
manifest_results_date = os.getenv("RESULTS_DATE", out_date).strip() or out_date
cohort_manifest["results_date"] = manifest_results_date
cohort_manifest["outputs"] = {
    "canonical_output_path": _manifest_relpath(canonical_out_path),
    "canonical_output_sha256": canonical_sha256,
    "dated_output_path": _manifest_relpath(dated_out_path),
    "dated_output_sha256": dated_sha256,
    "imv_source_provenance_path": _manifest_relpath(imv_source_provenance_path),
    "imv_source_provenance_sha256": imv_source_provenance_sha256,
    "dated_imv_source_provenance_path": _manifest_relpath(dated_imv_source_provenance_path),
    "dated_imv_source_provenance_sha256": _sha256_file(dated_imv_source_provenance_path),
    "gas_source_audit_path": _manifest_relpath(gas_source_audit_path) if "gas_source_audit_path" in globals() else None,
    "gas_source_overlap_summary_path": _manifest_relpath(gas_overlap_summary_path) if "gas_overlap_summary_path" in globals() else None,
    "pco2_source_distribution_audit_path": _manifest_relpath(pco2_source_distribution_audit_path)
    if "pco2_source_distribution_audit_path" in globals()
    else None,
    "pco2_window_max_contributor_audit_path": _manifest_relpath(
        pco2_window_max_contributor_audit_path
    )
    if "pco2_window_max_contributor_audit_path" in globals()
    else None,
    "qualifying_pco2_distribution_by_type_audit_path": _manifest_relpath(
        qualifying_pco2_distribution_audit_path
    )
    if "qualifying_pco2_distribution_audit_path" in globals()
    else None,
    "anthropometrics_coverage_audit_path": _manifest_relpath(anthro_coverage_audit_path) if "anthro_coverage_audit_path" in globals() else None,
    "ed_vitals_distribution_summary_path": _manifest_relpath(ed_vitals_distribution_summary_path) if "ed_vitals_distribution_summary_path" in globals() else None,
    "ed_vitals_extreme_examples_path": _manifest_relpath(ed_vitals_extreme_examples_path) if "ed_vitals_extreme_examples_path" in globals() else None,
    "ed_vitals_model_delta_path": _manifest_relpath(ed_vitals_model_delta_path) if "ed_vitals_model_delta_path" in globals() else None,
    "outliers_audit_path": _manifest_relpath(outliers_audit_path) if "outliers_audit_path" in globals() else None,
    "ed_only_exclusion_audit_csv_path": _manifest_relpath(ed_only_exclusion_audit_csv_path)
    if "ed_only_exclusion_audit_csv_path" in globals()
    else None,
    "ed_only_exclusion_audit_json_path": _manifest_relpath(ed_only_exclusion_audit_json_path)
    if "ed_only_exclusion_audit_json_path" in globals()
    else None,
    "hco3_itemid_qc_audit_path": _manifest_relpath(hco3_itemid_qc_audit_path)
    if "hco3_itemid_qc_audit_path" in globals()
    else None,
    "hco3_coverage_audit_path": _manifest_relpath(hco3_coverage_audit_path)
    if "hco3_coverage_audit_path" in globals()
    else None,
    "qualifying_ph_pairing_completeness_audit_path": _manifest_relpath(
        qualifying_ph_pairing_completeness_audit_path
    )
    if "qualifying_ph_pairing_completeness_audit_path" in globals()
    else None,
    "imv_qualifying_gas_timing_audit_path": _manifest_relpath(
        imv_qualifying_gas_timing_audit_path
    )
    if "imv_qualifying_gas_timing_audit_path" in globals()
    else None,
    "vitals_outlier_scope_audit_paths": {
        "ed_spine_raw_pre_clean": _manifest_relpath(vitals_outlier_ed_spine_pre_clean_path),
        "ed_spine_clean_post_clean": _manifest_relpath(vitals_outlier_ed_spine_post_clean_path),
        "ed_cc_raw_pre_clean": _manifest_relpath(vitals_outlier_ed_cc_pre_clean_path),
        "ed_cc_clean_post_clean": _manifest_relpath(vitals_outlier_ed_cc_post_clean_path),
    },
}
...
cohort_manifest["ed_only_exclusion_audit"] = {
    "cohort_logic_changed": False,
    "gas_eligibility_assessability": "not_assessable",
    "gas_eligibility_note": ed_only_gas_eligibility_note
    if "ed_only_gas_eligibility_note" in globals()
...
cohort_manifest = sanitize_manifest_payload(cohort_manifest, work_dir=WORK_DIR)
cohort_manifest_path = PRIOR_RUNS_DIR / f"{manifest_results_date} cohort_run_manifest.json"
...
print("Saved restricted IMV source provenance:", imv_source_provenance_path)
...
META = {
    ...
    "gender": {
        "label": "Gender",
        "definition": "Canonical sex/gender field for cohort analysis.",
        "source": "mimiciv_hosp.patients (fallback: mimiciv_ed.edstays)",
        "derivation": "patients.gender with fallback to edstays.gender when missing",
    },
    "age": {
        "label": "Age",
        "definition": "Age at admission in years.",
        "source": "mimiciv_hosp.patients + mimiciv_hosp.admissions",
        "derivation": "anchor_age + (admission year - anchor_year), rounded to 0.1",
        "units": "years",
    },
    "race": {
        "label": "Race",
        "definition": "Canonical race field for cohort analysis.",
        "source": "mimiciv_ed.edstays (fallback: mimiciv_hosp.admissions)",
        "derivation": "ed_race with fallback to hosp_race when ED race is missing",
    },
    "hosp_race": {
        "label": "Hospital race",
    ...
    "imv_flag": {
        "label": "IMV flag",
        "definition": "Legacy indicator of invasive mechanical ventilation during admission, retained for reconciliation only.",
        "source": "ICD procedures + ICU chartevents",
        "derivation": "max(IMV ICD flag, IMV chart flag)",
        "units": "binary (0/1)",
    },
    "imv_chart_flag": {
        "label": "Legacy IMV chart flag",
        "definition": "Legacy broad-regex indicator of charted invasive mechanical ventilation, retained for QA comparison only.",
        "source": "mimiciv_icu.chartevents",
        "derivation": "Any broad legacy IMV label/value regex match during the admission",
        "units": "binary (0/1)",
    },
    "niv_flag": {
        "label": "NIV flag",
    ...
    "first_imv_time": {
        "label": "Legacy first IMV time",
        "definition": "Earliest broad-regex charted time of IMV in ICU chartevents, retained for QA comparison only.",
        "source": "mimiciv_icu.chartevents",
        "derivation": "min charttime among IMV charted events",
        "units": "datetime",
    },
    "first_derived_imv_starttime": {
        "label": "First derived IMV episode start",
        "definition": "Earliest start time of an official MIMIC-IV derived InvasiveVent episode across all ICU stays in the admission.",
        "source": "mimiciv_derived.ventilation",
        "derivation": "Minimum starttime where ventilation_status equals InvasiveVent after stay_id-to-hadm_id linkage",
        "units": "datetime",
    },
    "first_intubation_procedure_time": {
        "label": "First explicit intubation procedure time",
        "definition": "Earliest ICU procedure-event start time for itemid 224385 (Intubation) in the admission.",
        "source": "mimiciv_icu.procedureevents",
        "derivation": "Minimum starttime for validated d_items itemid 224385",
        "units": "datetime",
    },
    "first_invasive_ventilation_procedure_time": {
        "label": "First explicit invasive-ventilation procedure time",
        "definition": "Earliest ICU procedure-event start time for itemid 225792 (Invasive Ventilation) in the admission.",
        "source": "mimiciv_icu.procedureevents",
        "derivation": "Minimum starttime for validated d_items itemid 225792",
        "units": "datetime",
    },
    "first_observed_imv_time": {
        "label": "First reliable observed IMV time",
        "definition": "Earliest reliable documented IMV timestamp across explicit intubation, explicit invasive-ventilation procedure, and official derived ventilation episodes.",
        "source": "mimiciv_icu.procedureevents + mimiciv_derived.ventilation",
        "derivation": "Exact minimum nonmissing timestamp across the three reliable IMV source fields; no rounding or buffer",
        "units": "datetime",
    },
    "first_observed_imv_source": {
        "label": "First reliable observed IMV source",
        "definition": "Source attaining first_observed_imv_time.",
        "source": "Derived",
        "derivation": "intubation_procedure, invasive_ventilation_procedure, derived_ventilation_episode, multiple_sources_same_time, or missing",
        "units": "categorical",
    },
    "robust_imv_observed": {
        "label": "Reliable IMV evidence observed",
        "definition": "Indicator that at least one reliable IMV source timestamp is present.",
        "source": "Derived",
        "derivation": "Any nonmissing first_intubation_procedure_time, first_invasive_ventilation_procedure_time, or first_derived_imv_starttime",
        "units": "binary (0/1)",
    },
    "imv_qualifying_gas_order": {
        "label": "IMV timing relative to qualifying gas",
        "definition": "Strict temporal classification comparing first_observed_imv_time with qualifying_pco2_time; exact ties, missing required times, and legacy-only IMV evidence are indeterminate.",
        "source": "Derived",
        "derivation": "not_applicable_no_qualifying_gas, no_observed_imv, qualifying_gas_before_imv, imv_before_qualifying_gas, or timing_indeterminate using exact < and > comparisons without a buffer",
        "units": "categorical",
    },
    "imv_preceded_qualifying_gas": {
        "label": "Observed IMV preceded qualifying gas",
        "definition": "Nullable indicator that reliable observed IMV strictly preceded the earliest qualifying pCO2.",
        "source": "Derived",
        "derivation": "True for imv_before_qualifying_gas; False for no_observed_imv or qualifying_gas_before_imv; missing otherwise",
        "units": "nullable boolean",
    },
    "no_prior_observed_imv": {
        "label": "No observed IMV before qualifying gas",
        "definition": "Nullable indicator for the primary sensitivity cohort with no reliable observed IMV before qualifying pCO2.",
        "source": "Derived",
        "derivation": "True for no_observed_imv or qualifying_gas_before_imv; False for imv_before_qualifying_gas; missing otherwise",
        "units": "nullable boolean",
    },
    "hours_from_imv_to_qualifying_gas": {
        "label": "Hours from observed IMV to qualifying gas",
        "definition": "Signed hours from first reliable observed IMV to the earliest qualifying pCO2; negative means gas first, positive means IMV first, and zero is an exact tie.",
        "source": "Derived",
        "derivation": "(qualifying_pco2_time - first_observed_imv_time) in exact elapsed hours",
        "units": "hours",
    },
    "legacy_imv_timing_discordant": {
        "label": "Robust versus legacy IMV discordance",
        "definition": "Indicator that robust and legacy evidence disagree about IMV presence, or disagree in strict gas-relative ordering when both IMV timestamps and the qualifying gas timestamp are available.",
        "source": "Derived",
        "derivation": "1 when robust_imv_observed differs from imv_flag, or when both are present with comparable timestamps and first_observed_imv_time versus first_imv_time yield different strict relations to qualifying_pco2_time; missing legacy timing alone is not discordance",
        "units": "binary (0/1)",
    },
    "first_niv_time": {
    ...
    "pco2_threshold_any": {
        ...
        "definition": "Indicator that a qualifying ABG/VBG/UNKNOWN hypercapnic pCO2 event exists at any time in the admission stay.",
        "source": "Derived",
        "derivation": "1 when qualifying_pco2_time is present for stay-level qualifying logic",
        "units": "binary (0/1)",
    },
    "pco2_threshold_0_24h": {
        "label": "Within-24h hypercapnic marker",
        "definition": "Indicator that the first qualifying hypercapnic pCO2 event occurred within 24 hours of ED presentation anchor.",
        "source": "Derived",
        "derivation": "pco2_threshold_any == 1 and dt_qualifying_hypercapnia_hours <= 24",
        "units": "binary (0/1)",
    },
    "qualifying_pco2_time": {
        "label": "Qualifying hypercapnic pCO2 time",
        "definition": "Earliest hypercapnic pCO2 event during the admission stay (canonical qualifying event timestamp).",
        "source": "mimiciv_hosp.labevents + mimiciv_icu.chartevents",
        "derivation": "min charttime among threshold-qualifying ABG/VBG/UNKNOWN events after presentation anchor",
        "units": "datetime",
    },
    "qualifying_pco2_mmhg": {
        "label": "Qualifying hypercapnic pCO2 (mmHg)",
        "definition": "Canonical qualifying pCO2 value from earliest stay-level hypercapnic event.",
        "source": "mimiciv_hosp.labevents + mimiciv_icu.chartevents",
    ...
    "qualifying_ph": {
        "label": "Paired pH for qualifying pCO2",
        "definition": "pH from the same specimen or time-aligned panel as the canonical qualifying pCO2, when available.",
        "source": "mimiciv_hosp.labevents + mimiciv_icu.chartevents",
        "derivation": "pH paired to qualifying_pco2_time by admission, source branch, site, and specimen/time panel within a 10-minute tolerance",
        "units": "unitless",
    },
    "qualifying_ph_time": {
        "label": "Paired qualifying pH time",
        "definition": "Timestamp of pH paired to the canonical qualifying pCO2.",
        "source": "mimiciv_hosp.labevents + mimiciv_icu.chartevents",
        "derivation": "nearest pH timestamp on the matched qualifying pCO2 specimen or time-aligned panel",
        "units": "datetime",
    },
    "qualifying_ph_source_branch": {
        "label": "Paired qualifying pH source branch",
        "definition": "Data branch of the pH paired to the canonical qualifying pCO2.",
        "source": "Derived",
        "derivation": "inherits LAB or POC branch from qualifying_pco2_time",
        "units": "categorical",
    },
    "qualifying_ph_site": {
        "label": "Paired qualifying pH site",
        "definition": "Sampling site class for the pH paired to the canonical qualifying pCO2.",
        "source": "Derived",
        "derivation": "inherits arterial|venous|unknown site from qualifying_pco2_time",
        "units": "categorical",
    },
    "qualifying_ph_pairing_status": {
        "label": "Qualifying pH pairing status",
        "definition": "Aggregate-safe status describing whether pH was available on the qualifying pCO2 specimen or time-aligned panel.",
        "source": "Derived",
        "derivation": "paired_same_specimen_panel, paired_same_time_panel, or qualifying_panel_missing_ph",
        "units": "categorical",
    },
    "qualifying_threshold_mmhg": {
