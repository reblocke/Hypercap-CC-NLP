        ...
        mapping_rows.append(
            {
                ...
                "source": source if source is not None else "other",
                "source_inference_tier": "specimen_text" if source is not None else "fallback_other",
...
TARGET_RAW_FIELDS = {
    ...
    "labs_gas": [
        ...
        "gas_source_other_rate","poc_inclusion_enabled","poc_inclusion_reason",
    ...
    "timing": [
        ...
        "qualifying_gas_observed","presenting_hypercapnia_tri","late_hypercapnia_tri","hypercap_timing_class",
...
def pick_analyte(
    ...
    *,
    ...
    cols = ["ed_stay_id", "specimen_id", "charttime", "valuenum", "itemid", "fluid"]
    ...
    if tmp.empty:
        ...
        if include_item_meta:
            base_cols += [f"{name}_itemid", f"{name}_fluid"]
    ...
    if include_item_meta:
        first = first.rename(
            columns={"itemid": f"{name}_itemid", "fluid": f"{name}_fluid"}
        )
        keep_cols = ["ed_stay_id", "specimen_id", name, f"{name}_itemid", f"{name}_fluid"]
    else:
...
for col in ["pco2", "ph", "hco3", "lactate"]:
    if col not in panel.columns:
        panel[col] = pd.NA
for col in ["pco2_itemid", "pco2_fluid"]:
    if col not in panel.columns:
        panel[col] = pd.NA
...
if panel_anchor.empty:
    ...
    first_anchor["co2_other_is_blood_asserted"] = (
        first_anchor["source"].astype("string").str.lower().eq("other")
...
if len(icu_poc_long) > 0 and pco2_ids:
    ...
    if not pco2_long.empty:
        ...
        for itemid_value, group in pco2_long.groupby("itemid", dropna=False):
            ...
            numeric = pd.to_numeric(group["pco2_mmhg"], errors="coerce").dropna()
            median_value = float(numeric.median()) if len(numeric) else np.nan
            median_ok = bool(
                len(numeric)
                and ICU_PCO2_QC_MEDIAN_MIN <= median_value <= ICU_PCO2_QC_MEDIAN_MAX
            )
            contamination_flag = bool((not include_ok) or exclude_hit or (not unit_ok))
            ...
            pco2_itemid_qc_rows.append(
                {
                    ...
                    "p50_mmhg": median_value if len(numeric) else np.nan,
                    "p95_mmhg": float(numeric.quantile(0.95)) if len(numeric) else np.nan,
                    "max_mmhg": float(numeric.max()) if len(numeric) else np.nan,
...
if len(poc_flags) > 0:
    ...
    ed_df["flag_any_gas_hypercapnia_poc"] = 0
ed_df["flag_any_gas_hypercapnia_poc"] = (
    pd.to_numeric(ed_df["flag_any_gas_hypercapnia_poc"], errors="coerce")
    .fillna(0)
    .astype("Int64")
)

ed_df["poc_inclusion_enabled"] = bool(poc_inclusion_enabled)
ed_df["poc_inclusion_reason"] = str(poc_inclusion_reason)
...
anthro_model_specs = {
    "bmi_closest_pre_ed": ("bmi_closest_pre_ed_model", "bmi_outlier_flag", 0.0, 100.0),
    "height_closest_pre_ed": ("height_closest_pre_ed_model", "height_outlier_flag", 100.0, 250.0),
    "weight_closest_pre_ed": ("weight_closest_pre_ed_model", "weight_outlier_flag", 0.0, 400.0),
}
...
qualifying_hours = pd.to_numeric(ed_df["dt_first_qualifying_gas_hours"], errors="coerce")
qualifying_observed = qualifying_hours.notna()
ed_df["qualifying_gas_observed"] = qualifying_observed.astype("Int64")
ed_df["presenting_hypercapnia_tri"] = pd.Series(pd.NA, index=ed_df.index, dtype="Int64")
ed_df["late_hypercapnia_tri"] = pd.Series(pd.NA, index=ed_df.index, dtype="Int64")
ed_df.loc[qualifying_observed, "presenting_hypercapnia_tri"] = (
    qualifying_hours.loc[qualifying_observed].le(6).astype("Int64")
)
ed_df.loc[qualifying_observed, "late_hypercapnia_tri"] = (
    qualifying_hours.loc[qualifying_observed].gt(6)
    & qualifying_hours.loc[qualifying_observed].le(24)
).astype("Int64")
    ...
    [
        {
            ...
            "qualifying_gas_observed_n": int(pd.to_numeric(ed_df.get("qualifying_gas_observed"), errors="coerce").fillna(0).astype(int).sum()),
            "qualifying_gas_missing_n": int(pd.to_numeric(ed_df.get("qualifying_gas_observed"), errors="coerce").fillna(0).astype(int).eq(0).sum()),
        }
    ]
)
...
qa_summary = {
    ...
    "poc_inclusion_enabled": bool(poc_inclusion_enabled),
    "poc_inclusion_reason": str(poc_inclusion_reason),
