...
cohort_any["pco2_threshold_any"] = cohort_any["pco2_threshold_0_24h"].astype(int)
...
if "pco2_threshold_0_24h" in df.columns:
    ...
    df["pco2_threshold_any"] = df["pco2_threshold_0_24h"].astype("Int64")
...
gas_any = (
    ...
    if "pco2_threshold_any" in df.columns else (abg | vbg | unknown)
)
...
TARGET_RAW_FIELDS = {
    ...
    "labs_gas": [
        ...
        "gas_source_unknown_rate","gas_source_other_rate","poc_inclusion_enabled","poc_inclusion_reason",
...
for candidate in sorted(prior_runs_dir.glob("* gas_source_audit.json"), reverse=True):
    ...
    try:
        ...
        if current_source_mode and baseline_mode and baseline_mode != current_source_mode:
            ...
            continue
        baseline_rates = baseline_payload.get("source_rates", {})
        baseline_other_rate = float(
            baseline_rates.get(
                "unknown",
                baseline_rates.get("other", float("nan")),
            )
        )
...
if len(icu_poc_long) > 0:
    ...
    if po2_ids:
        panel_poc = panel_poc.merge(pick_from_ids(icu_poc_long, po2_ids, "po2"), on=["icu_stay_id","time_bin"], how="left")
    if ph_ids:
...
ed_df["pco2_threshold_any"] = ed_df["pco2_threshold_0_24h"].astype("Int64")
