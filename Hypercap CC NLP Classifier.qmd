...
# Workbook for MIMIC Hypercapnia Presenting Chief Concern NLP Classifier

This notebook is intentionally self-contained and deterministic. It
performs NLP-based mapping from free-text ED chief complaints to NHAMCS
top-level RVC categories, with strict compatibility outputs for
downstream analysis notebooks.

## Stage contract (pipeline-facing)

-   **Purpose:** Add NLP-derived RFV category outputs to the canonical
    cohort workbook.
-   **Inputs:** `MIMIC tabular data/MIMICIV all with CC.xlsx`.
-   **Canonical outputs:**
    `MIMIC tabular data/MIMICIV all with CC_with_NLP.xlsx`.
-   **Run-archived outputs:** dated classifier audits/manifests under
    `MIMIC tabular data/prior runs/`.
-   **Key env vars:** `WORK_DIR`, `CLASSIFIER_INPUT_FILENAME`,
    `CC_SPELL_CORRECTION_MODE`, `HF_TOKEN`.
-   **QA artifacts emitted:** spellfix guardrail audit, spell-mode
    comparison audit, classifier run manifest.

## Summary of functionality

This stage takes the cohort workbook from the previous notebook and adds
NLP-derived complaint categories.

Use this reading order:

1.  **Runtime contract/imports**: confirms all required tools are
    installed before any processing starts.
2.  **Config/constants**: defines the vocabulary, labels, and model
    settings used everywhere else.
3.  **Text normalization**: cleans complaint text while preserving
    clinical meaning with safety guardrails.
4.  **Embedding + scoring**: compares complaint segments to curated
    category prototypes.
5.  **Visit-level aggregation**: combines segment predictions into final
    RFV outputs (`RFV1` ... `RFV5`).
6.  **Schema + export checks**: verifies required columns and writes the
    canonical NLP workbook (`MIMICIV all with CC_with_NLP.xlsx`).

Documentation pattern used throughout: - Chunk headers explain why each
block exists. - Helper functions prefer explicit names and deterministic
behavior so outputs are reproducible run-to-run.
...
`ClassifierConfig` - `get_cc_column` - `normalize_cc` -
`build_prototype_resources` - `build_segment_cache` -
`classify_segments_cached` - `assign_rvc_per_segment_and_visit_cached` -
`validate_output_schema` - `save_with_suffix`
...
# ---------------------------------------------------------------------
# Prototype-group scoring configuration
# ---------------------------------------------------------------------
# Score each category as max cosine across its prototypes.
# This keeps classifier behavior parsimonious and interpretable.
...
class ClassifierConfig:
    ...
    annotation_benchmark_mode_env_var: str = "CLASSIFIER_ANNOTATION_BENCHMARK_MODE"
    annotation_benchmark_path_env_var: str = "CLASSIFIER_ANNOTATION_BENCHMARK_PATH"
    annotation_benchmark_sheet_env_var: str = "CLASSIFIER_ANNOTATION_BENCHMARK_SHEET"
    annotation_benchmark_cc_column_env_var: str = "CLASSIFIER_ANNOTATION_CC_COLUMN"
    annotation_benchmark_default_path: str = "Annotation/Final 2025-10-14 Annotation Sample.xlsx"
    annotation_benchmark_default_sheet: str = "cohort_cc_sample"
    annotation_benchmark_output_filename: str = "annotation_benchmark_with_NLP.xlsx"
    annotation_visit_candidate_scores_filename: str = "annotation_visit_candidate_scores.csv"
    annotation_segment_candidate_scores_filename: str = "annotation_segment_candidate_scores.csv"
...
# Lemmatization can alter idiom meaning in a small number of cases.
LEMMA_PROTECT_PHRASES = {
    "found down",
}
...
SPELL_DENYLIST_SUBSTITUTIONS = {
    # Known high-risk semantic flips that must never be accepted.
    ("found", "wound"),
    ("trach", "tract"),
    ("dental", "mental"),
    ("black", "back"),
...
# Token-level expansions for high-impact abbreviations so they fire inside longer segments.
TOK_OVR.update(
    {
        "trach": "tracheostomy",
        "mvc": "motor vehicle collision",
...
RX_NEURO_BLEED = re.compile(
    r"\b("
    r"(?:head|brain|intracranial|cerebral)\s+(?:bleed(?:ing)?|hemorrhag(?:e|ic)|haemorrhag(?:e|ic))"
    r"|(?:subdural|subarachnoid|epidural|intracerebral)\s+(?:hematoma|haematoma|hemorrhag(?:e|ic)|haemorrhag(?:e|ic))"
    r"|(?:ich|sah|sdh|edh|ivh)\b"
    r")\b",
    re.I,
)
RX_NEURO_BLEED_TRAUMA_CONTEXT = re.compile(
    r"\b(fall|fell|mvc|mva|collision|assault|trauma|injur(?:y|ies)|fracture|laceration|gsw|gunshot|stab)\b",
    re.I,
)
RX_NEURO_BLEED_DIAGNOSIS_CONTEXT = re.compile(
    r"\b(intracranial hemorrhage|subarachnoid hemorrhage|subdural hematoma|epidural hematoma|intracerebral hemorrhage|ich|sah|sdh|edh|ivh)\b",
    re.I,
)
RX_EXPLICIT_DISEASE_DIAGNOSIS = re.compile(
    r"\b(?:"
    r"abdominal aortic aneurysm|aaa|ruptured aaa|ruptured abdominal aortic aneurysm|"
    r"diabetic ketoacidosis|dka|hyperglyc(?:emia|emic)|hypoglyc(?:emia|emic)|"
    r"pneumothorax|pnx|pleural effusion"
    r")\b",
    re.I,
)
RX_ABSCESS_DISEASE = re.compile(
    r"\b(?:dental abscess|abscess|boil|furuncle|carbuncle|cellulitis)\b",
    re.I,
)
RX_SWELLING_LOWER_EXT = re.compile(
    r"\b(?:"
    r"(?:b\s+)?leg|ankle|foot|feet|calf|lower extremity|peripheral|le|left leg|right leg|bilateral leg"
    r")\s+(?:swell(?:ing)?|edema)\b"
    r"|\b(?:bilateral\s+)?le\s+(?:swell(?:ing)?|edema)\b",
    re.I,
)
RX_SWELLING_GU = re.compile(
    r"\b(?:testicular|scrotal|penile|vaginal|labial)\s+(?:swell(?:ing)?|edema)\b",
    re.I,
)
RX_SWELLING_FACIAL = re.compile(
    r"\b(?:facial|face|lip|tongue|periorbital|eyelid)\s+(?:swell(?:ing)?|edema)\b",
    re.I,
)
RX_SWELLING_ABDOMINAL = re.compile(
    r"\b(?:abdominal|abd(?:ominal)?)\s+(?:distention|distension|swell(?:ing)?|girth)\b"
    r"|\bincrease abdominal girth\b",
    re.I,
)
RX_NEAR_DROWNING = re.compile(r"\b(drown(?:ing)?|near drown(?:ing)?)\b", re.I)
EXACT_UNCODABLE_SEGMENTS = {"code", "perforation"}

RULE_RX: dict[str, re.Pattern[str]] = {
    "RVC-INJ": re.compile(
        r"\b(mvc|mva|collision|ped(\s|)vs(\s|)auto|assault|fell|fall|gsw|gunshot|stab|laceration|fracture|fx\b|burn|foreign body|poison(ing)?|overdose|od\b|adverse (drug|medication) reaction|drown(?:ing)?|near drown(?:ing)?)\b",
        re.I,
    ),
    ...
    "RVC-SYM-DIG": re.compile(
        r"\b(abd(ominal)? pain|abdo pain|nausea|vomit(ting)?|diarrhea|rectal bleeding|hematemesis|dysphagia|jaundice|gastrointestinal bleeding|upper gi bleed|gi bleeding|gi bleed)\b",
        re.I,
    ),
...
# Spell correction is intentionally conservative for short triage text.
# We only allow edit distance ≤ 1 in strict mode.
...
def _clean_tok(token: str) -> str:
    """Trim punctuation at token edges while preserving interior separators like '/'."""
    return _PUNCT_EDGE.sub("", token)


def _violates_spell_guard(original: str, candidate: str) -> bool:
    """Block high-risk spell changes (polarity/laterality flips) even if edit distance is small."""
...
def _spell_candidate(token: str, *, max_edit_distance: int) -> tuple[str, int | None]:
    """Return closest SymSpell candidate and edit distance, or the token unchanged."""
...
def _spell_strict(token: str) -> tuple[str, dict[str, Any] | None]:
    """Conservative spell correction for clinical text (alpha-only, min length, guardrails)."""
...
def _spell_token(token: str, *, mode: str) -> tuple[str, dict[str, Any] | None]:
    """Dispatch spell correction by configured mode."""
    if mode == "disabled":
        return token, None
    if mode == "strict":
...
def _looks_clinical(text: str) -> bool:
    """Heuristic: text contains a clinical keyword and at least one alphabetic character."""
    lowered = text.lower()
    return any(key in lowered for key in CLINICAL_KEYS) and bool(re.search(r"[a-z]", lowered))


def _should_force_uncodable_segment(
    segment: str,
    *,
    override_code: str | None = None,
) -> bool:
    """Force clearly unsupported short or placeholder-like segments to uncodable."""
    normalized = str(segment).strip().lower()
    if not normalized or override_code is not None:
        return False
    if normalized in EXACT_UNCODABLE_SEGMENTS:
        return True

    tokens = [token for token in normalized.split() if token]
    if not tokens:
        return False
    if any(token in TOK_OVR or token in ABBR_MAP for token in tokens):
        return False

    longest_token_len = max(len(token) for token in tokens)
    if len(tokens) == 1 and longest_token_len <= 4 and tokens[0] not in SAFE_SHORT_TOKENS:
        return not _looks_clinical(normalized)
    if len(tokens) <= 2 and longest_token_len <= 4:
        return not _looks_clinical(normalized)
    return False


def _split_on_and_if_clinical(segment: str) -> list[str]:
    """Split on 'and' only when both sides look clinically meaningful."""
...
def _expand_duration(segment: str) -> str:
    """Normalize shorthand durations (e.g., 'sob x 3d' -> 'sob for 3 days')."""
...
def _safe_lemma(token_text: str, lemma_text: str) -> str:
    """Protect against specific lemmatizer over-stemming artifacts."""
    token_norm = token_text.strip().lower()
    lemma_norm = lemma_text.strip().lower()
    if (
        token_norm.endswith("eed")
        and lemma_norm == token_norm[:-1]
    ):
        return token_text
    return lemma_text
...
def _lemmatize_segment(segment: str) -> str:
    """Lemmatize one segment with light safety checks."""
    doc = NLP(segment)
    lemmas: list[str] = []
    for token in doc:
        ...
        if not lemma or lemma == "-PRON-":
            ...
            lemma = _safe_lemma(token.text, lemma)
        lemmas.append(lemma)
...
def segment_cc(raw: str, keep_negated: bool = False) -> list[str]:
    ...
    for protected in PROTECT:  # Preserve slash-delimited abbreviations before generic slash splitting.
    ...
    text = text.replace("/", "; ")  # Treat remaining slashes as segment separators.
...
def normalize_cc(
    ...
    *,
    ...
    for segment_index, segment in enumerate(segment_cc(raw, keep_negated=keep_negated)):
        # Stage 1: deterministic abbreviation/context expansions.
        ...
        # Stage 2: guarded spell correction (optional skip for protected phrases).
        segment_lower = segment.strip().lower()
        skip_spellfix = any(phrase in segment_lower for phrase in SPELL_PROTECT_PHRASES)
        skip_lemma = any(phrase in segment_lower for phrase in LEMMA_PROTECT_PHRASES)
        ...
        # Stage 3: lemmatize unless phrase-level protection disables it.
        segment = corrected_segment if skip_lemma else _lemmatize_segment(corrected_segment)
...
def _run_normalization_mode(
    ...
    *,
    ...
    """Run normalization for one spell mode and return row-level outputs + mode summary."""
...
def _score_normalization_integrity(
    ...
    """Count known dangerous normalization error patterns for mode selection."""
    ...
    rules = {
        ...
        "bleed_token_truncation": raw.str.contains(r"\bbleed\b", na=False)
        & ~clean.str.contains(r"\bbleed\b", na=False)
        & clean.str.contains(r"\bblee\b", na=False),
        "femer_to_fever": raw.str.contains(r"\bfemer\b", na=False)
        & ~clean.str.contains(r"\bfemur\b", na=False)
        & clean.str.contains(r"\bfever\b", na=False),
    }
...
def choose_spell_mode(
    ...
    *,
    ...
    """Run disabled/strict normalization and choose the safer mode for this run.

    Selection priority:
    1) fewer integrity violations,
    2) more non-empty normalized segments,
    3) more accepted spell fixes (tie-break),
    4) stable lexical ordering of mode name.
    """
...
def rule_gate(segment: str) -> tuple[str | None, list[str]]:
    ...
    Returns:
        (code, hits)
        - code: selected RVC override or None
        - hits: matched rule labels for audit/debug visibility
    """
    text = f" {segment.lower()} "
    if re.search(r"\babnormal\s+labs?\b", text):
        return "RVC-TEST", ["abnormal labs"]
    if segment.strip().lower() in EXACT_UNCODABLE_SEGMENTS:
        return "RVC-UNCL", ["exact_uncodable_segment"]
    if RX_EXPLICIT_DISEASE_DIAGNOSIS.search(text):
        return "RVC-DIS", ["explicit_disease_diagnosis"]
    if RX_ABSCESS_DISEASE.search(text):
        return "RVC-DIS", ["abscess_disease"]
    if RX_SWELLING_GU.search(text):
        return "RVC-SYM-GU", ["gu_swelling_site"]
    if RX_SWELLING_ABDOMINAL.search(text):
        return "RVC-SYM-DIG", ["abdominal_swelling_site"]
    if RX_SWELLING_FACIAL.search(text):
        return "RVC-SYM-SKIN", ["facial_swelling_site"]
    if RX_SWELLING_LOWER_EXT.search(text):
        return "RVC-SYM-CIRC", ["lower_ext_swelling_site"]
    if RX_NEAR_DROWNING.search(text):
        return "RVC-INJ", ["near_drowning"]
    if "weakness" in text and RX_FOCAL_WEAK.search(text):
        return "RVC-SYM-NERV", ["focal weakness"]
    if RX_NEURO_BLEED.search(text):
        if RX_NEURO_BLEED_TRAUMA_CONTEXT.search(text):
            return "RVC-INJ", ["neuro_bleed_trauma_context"]
        if RX_NEURO_BLEED_DIAGNOSIS_CONTEXT.search(text):
            return "RVC-DIS", ["neuro_bleed_diagnosis_context"]
        return "RVC-SYM-NERV", ["neuro_bleed_symptom_context"]
...
def build_segment_cache(
    ...
    Note: this cache is in-memory for the current run only (no persisted cache key reuse).
    """
...
def group_scores_from_proto_row(
    ...
    """Aggregate prototype-level similarities to max-cosine group scores."""
    if method != "max":
        raise ValueError("scoring_method must be 'max'.")

    if isinstance(proto_row, torch.Tensor):
        vector = proto_row.detach().cpu().numpy()
    else:
        vector = np.asarray(proto_row)
    ...
    for group_code, indices in code_to_proto.items():
        ...
        scores[group_code] = float(np.max(values)) if len(values) else -1.0
...
def score_one_segment_soft(
    ...
    """Score one segment directly against prototype embeddings."""
    ...
    return group_scores_from_proto_row(similarities, resources=resources, method=method)
...
def classify_segments_cached(
    ...
    for seg_idx, segment in enumerate(segments):
        ...
        if cleaned in cache.seg2idx:
            ...
            score_map = group_scores_from_proto_row(cache.sim_proto[row_idx], resources=resources, method=method)
        else:
            score_map = score_one_segment_soft(cleaned, resources=resources, method=method)

        best_code, best_score = max(score_map.items(), key=lambda kv: kv[1])

        if override_code is not None:
            best_code = override_code
            best_score = float(score_map.get(best_code, best_score))
        elif _should_force_uncodable_segment(cleaned):
            best_code = "RVC-UNCL"
...
# --- Candidate-ranking sidecars for downstream validation extensions. ---
def _classifier_join_key_columns(df: pd.DataFrame) -> list[str]:
    """Return stable join keys to carry into classifier sidecars."""
    return [
        column
        for column in ("annotation_row_id", "ed_stay_id", "hadm_id", "subject_id")
        if column in df.columns
    ]


def _score_map_for_segment(
    segment: str,
    *,
    cache: SegmentEmbedCache,
    resources: dict[str, Any],
    ...
    """Return raw embedding scores for all canonical RVC categories."""
    cleaned = str(segment).strip()
    if not cleaned:
        return {code: -1.0 for code in PRECEDENCE_RVC}
    if cleaned in cache.seg2idx:
        row_idx = cache.seg2idx[cleaned]
        score_map = group_scores_from_proto_row(cache.sim_proto[row_idx], resources=resources, method=method)
    else:
        score_map = score_one_segment_soft(cleaned, resources=resources, method=method)
    return {code: float(score_map.get(code, -1.0)) for code in PRECEDENCE_RVC}


def _default_rfv_code_set(row: pd.Series, *, max_rfv: int) -> frozenset[str]:
    """Extract emitted default RFV category codes from exported RFV name slots."""
    label_to_code = {label: code for code, label in RVC_NAME.items()}
    emitted_codes: set[str] = set()
    for slot in range(1, max_rfv + 1):
        name = row.get(f"RFV{slot}_name", "")
        if pd.isna(name) or not str(name).strip():
            continue
        code = label_to_code.get(str(name).strip())
        if code is not None:
            emitted_codes.add(code)
    return frozenset(sorted(emitted_codes))


def _parse_segment_prediction_payload(value: object) -> list[dict[str, Any]]:
    """Parse one serialized segment prediction payload from classifier output."""
    if isinstance(value, list):
...
def _top_margin(score_rows: list[dict[str, object]], *, score_col: str) -> float:
    """Return nonnegative top1-top2 margin from category score rows."""
    scores = sorted(
        (float(row[score_col]) for row in score_rows if pd.notna(row.get(score_col))),
        reverse=True,
    )
    if not scores:
        return 0.0
    if len(scores) == 1:
        return max(float(scores[0]), 0.0)
    return max(float(scores[0] - scores[1]), 0.0)
...
def rank_candidate_scores(
    ...
    *,
    ...
    score_col: str,
    rank_col: str,
    rule_col: str = "any_rule_support",
    support_col: str = "first_support_seg_idx",
) -> pd.DataFrame:
    """Assign deterministic descending ranks within each visit or segment."""
    out = frame.copy()
    if rule_col in out.columns:
        out["_candidate_rule_sort"] = out[rule_col].fillna(False).astype(bool).astype(int)
    else:
        out["_candidate_rule_sort"] = 0
    if support_col in out.columns:
        out["_candidate_support_sort"] = pd.to_numeric(out[support_col], errors="coerce").fillna(10**9)
    else:
        out["_candidate_support_sort"] = 10**9
    out["_candidate_precedence_sort"] = out["category_code"].map(
        {code: idx for idx, code in enumerate(PRECEDENCE_RVC)}
    ).fillna(10**9)
    out["_candidate_score_sort"] = pd.to_numeric(out[score_col], errors="coerce").fillna(-1.0)
    out = out.sort_values(
        [
            *group_cols,
            "_candidate_score_sort",
            "_candidate_rule_sort",
            "_candidate_support_sort",
            "_candidate_precedence_sort",
            "category_code",
        ],
        ascending=[*(True for _ in group_cols), False, False, True, True, True],
        kind="mergesort",
    )
    out[rank_col] = out.groupby(group_cols, dropna=False).cumcount() + 1
    return out.drop(
        columns=[
            "_candidate_rule_sort",
            "_candidate_support_sort",
...
def build_segment_candidate_scores(
    df: pd.DataFrame,
    *,
    cache: SegmentEmbedCache,
    resources: dict[str, Any],
    ...
    """Build one row per visit-segment-category with raw and effective scores."""
    join_key_cols = _classifier_join_key_columns(df)
    rows: list[dict[str, object]] = []

    for row_idx, (_, source_row) in enumerate(df.iterrows()):
        key_payload = {column: source_row[column] for column in join_key_cols}
        payload = _parse_segment_prediction_payload(source_row.get("segment_preds", "[]"))
        for prediction in payload:
            segment_text = str(prediction.get("segment", "") or "").strip()
            seg_idx_value = pd.to_numeric(prediction.get("seg_idx", 0), errors="coerce")
            seg_idx = int(0 if pd.isna(seg_idx_value) else seg_idx_value)
            selected_code = str(prediction.get("code", "") or "")
            rule_code_value = prediction.get("rule_code")
            rule_code = "" if rule_code_value is None or str(rule_code_value) == "nan" else str(rule_code_value)
            rule_used = bool(prediction.get("rule_used", False))

            raw_scores = _score_map_for_segment(
                segment_text,
                cache=cache,
                resources=resources,
                method=cfg.scoring_method,
            )
            raw_best = max(raw_scores.values()) if raw_scores else -1.0
            effective_scores = dict(raw_scores)
            if selected_code in effective_scores:
                effective_scores[selected_code] = max(
                    float(effective_scores[selected_code]),
                    float(raw_best) + 1e-6,
                )
            effective_best = max(effective_scores.values()) if effective_scores else -1.0

            segment_rows: list[dict[str, object]] = []
            for code in PRECEDENCE_RVC:
                segment_rows.append(
                    {
                        **key_payload,
                        "classifier_row_id": row_idx,
                        "seg_idx": seg_idx,
                        "segment_text": segment_text,
                        "category_code": code,
                        "category_name": RVC_NAME[code],
                        "segment_score_raw": float(raw_scores.get(code, -1.0)),
                        "segment_score_effective": float(effective_scores.get(code, -1.0)),
                        "rule_used": bool(rule_used and rule_code == code),
                        "rule_code": rule_code if rule_used and rule_code == code else "",
                        "selected_at_default": bool(selected_code == code),
                        "segment_best_raw": float(raw_best),
                        "segment_best_effective": float(effective_best),
                        "segment_abstained_at_default": bool(selected_code == "RVC-UNCL"),
                    }
                )

            margin = _top_margin(segment_rows, score_col="segment_score_effective")
            for segment_row in segment_rows:
                segment_row["segment_margin_top1_top2"] = margin
        ...
        return pd.DataFrame(
            columns=[
                *join_key_cols,
                ...
                "selected_at_default",
                "segment_best_raw",
                "segment_best_effective",
                "segment_margin_top1_top2",
                "segment_abstained_at_default",
            ]
        )

    segment_scores = pd.DataFrame.from_records(rows)
    group_cols = [*join_key_cols, "classifier_row_id", "seg_idx"]
    segment_scores = rank_candidate_scores(
        segment_scores,
        group_cols=group_cols,
        score_col="segment_score_raw",
        rank_col="segment_rank_raw",
        rule_col="rule_used",
        support_col="seg_idx",
    )
    segment_scores = rank_candidate_scores(
        segment_scores,
        group_cols=group_cols,
        score_col="segment_score_effective",
        rank_col="segment_rank_effective",
        rule_col="rule_used",
        support_col="seg_idx",
    )
    return segment_scores.sort_values(
        [*group_cols, "segment_rank_effective"],
        kind="mergesort",
    ).reset_index(drop=True)


def aggregate_visit_candidate_scores(
    df: pd.DataFrame,
    segment_candidate_scores: pd.DataFrame,
    *,
    cfg: ClassifierConfig,
) -> pd.DataFrame:
    """Aggregate segment candidates to one row per visit-category."""
    join_key_cols = _classifier_join_key_columns(df)
    emitted_lookup = {
        row_idx: _default_rfv_code_set(source_row, max_rfv=cfg.max_rfv)
        for row_idx, (_, source_row) in enumerate(df.iterrows())
    }
    rows: list[dict[str, object]] = []
    ...
    for row_idx, (_, source_row) in enumerate(df.iterrows()):
        key_payload = {column: source_row[column] for column in join_key_cols}
        row_segments = segment_candidate_scores.loc[
            segment_candidate_scores["classifier_row_id"].eq(row_idx)
        ]
        emitted_codes = emitted_lookup[row_idx]
        for code in PRECEDENCE_RVC:
            category_segments = row_segments.loc[row_segments["category_code"].eq(code)]
            if category_segments.empty:
                visit_score_raw = -1.0
                visit_score_effective = -1.0
                any_rule_support = False
                first_support_seg_idx = np.nan
                support_segment_count = 0
            else:
                visit_score_raw = float(category_segments["segment_score_raw"].max())
                visit_score_effective = float(category_segments["segment_score_effective"].max())
                selected_segments = category_segments.loc[
                    category_segments["selected_at_default"].fillna(False).astype(bool)
                ]
                support_segment_count = int(len(selected_segments))
                first_support_seg_idx = (
                    float(selected_segments["seg_idx"].min()) if support_segment_count else np.nan
                )
                any_rule_support = bool(category_segments["rule_used"].fillna(False).astype(bool).any())

            if code in emitted_codes and visit_score_effective <= -1.0:
                visit_score_effective = 1.0
            ...
            rows.append(
                {
                    **key_payload,
                    ...
                    "category_code": code,
                    "category_name": RVC_NAME[code],
                    "visit_score_raw": visit_score_raw,
                    "visit_score_effective": visit_score_effective,
                    "any_rule_support": any_rule_support,
                    "first_support_seg_idx": first_support_seg_idx,
                    "support_segment_count": support_segment_count,
                    "emitted_at_default": bool(code in emitted_codes),
                }
            )

    visit_scores = pd.DataFrame.from_records(rows)
    group_cols = [*join_key_cols, "classifier_row_id"]
    visit_scores = rank_candidate_scores(
        visit_scores,
        group_cols=group_cols,
        score_col="visit_score_raw",
        rank_col="visit_rank_raw",
    )
    visit_scores = rank_candidate_scores(
        visit_scores,
        group_cols=group_cols,
        score_col="visit_score_effective",
        rank_col="visit_rank_effective",
    )
    margin_rows: list[dict[str, object]] = []
    for keys, group in visit_scores.groupby(group_cols, dropna=False):
        key_values = keys if isinstance(keys, tuple) else (keys,)
        margin_row = dict(zip(group_cols, key_values))
        margin_row["default_visit_margin_top1_top2"] = _top_margin(
            group.to_dict("records"),
            score_col="visit_score_effective",
        )
        margin_rows.append(margin_row)
    margins = pd.DataFrame.from_records(margin_rows)
    visit_scores = visit_scores.merge(margins, on=group_cols, how="left", validate="many_to_one")
    return visit_scores.sort_values([*group_cols, "visit_rank_effective"], kind="mergesort").reset_index(drop=True)


def validate_candidate_sidecars(
    df: pd.DataFrame,
    visit_candidate_scores: pd.DataFrame,
    segment_candidate_scores: pd.DataFrame,
    *,
    cfg: ClassifierConfig,
) -> None:
    """Fail fast if candidate sidecars drift from classifier output contracts."""
    join_key_cols = _classifier_join_key_columns(df)
    visit_group_cols = [*join_key_cols, "classifier_row_id"]
    expected_categories = set(PRECEDENCE_RVC)

    category_sets = visit_candidate_scores.groupby(visit_group_cols, dropna=False)["category_code"].agg(set)
    bad_visit_category_sets = int((category_sets != expected_categories).sum())
    if bad_visit_category_sets:
        raise ValueError(f"visit_candidate_scores category coverage failures: {bad_visit_category_sets}")

    if not segment_candidate_scores.empty:
        segment_group_cols = [*join_key_cols, "classifier_row_id", "seg_idx"]
        segment_category_sets = segment_candidate_scores.groupby(segment_group_cols, dropna=False)["category_code"].agg(set)
        bad_segment_category_sets = int((segment_category_sets != expected_categories).sum())
        if bad_segment_category_sets:
            raise ValueError(f"segment_candidate_scores category coverage failures: {bad_segment_category_sets}")

        for rank_col in ("segment_rank_raw", "segment_rank_effective"):
            rank_counts = segment_candidate_scores.groupby(segment_group_cols, dropna=False)[rank_col].nunique()
            if not rank_counts.eq(len(PRECEDENCE_RVC)).all():
                raise ValueError(f"{rank_col} must be unique within each visit-segment.")

    for rank_col in ("visit_rank_raw", "visit_rank_effective"):
        rank_counts = visit_candidate_scores.groupby(visit_group_cols, dropna=False)[rank_col].nunique()
        if not rank_counts.eq(len(PRECEDENCE_RVC)).all():
            raise ValueError(f"{rank_col} must be unique within each visit.")

    for frame, column in (
        (visit_candidate_scores, "default_visit_margin_top1_top2"),
        (segment_candidate_scores, "segment_margin_top1_top2"),
    ):
        if column in frame.columns and not pd.to_numeric(frame[column], errors="coerce").fillna(0).ge(0).all():
            raise ValueError(f"{column} must be nonnegative.")

    emitted_lookup = (
        visit_candidate_scores.loc[visit_candidate_scores["emitted_at_default"].fillna(False).astype(bool)]
        .groupby("classifier_row_id")["category_code"]
        .agg(lambda series: frozenset(sorted(series)))
        .to_dict()
    )
    mismatches: list[int] = []
    for row_idx, (_, source_row) in enumerate(df.iterrows()):
        expected = _default_rfv_code_set(source_row, max_rfv=cfg.max_rfv)
        observed = emitted_lookup.get(row_idx, frozenset())
        if expected != observed:
            mismatches.append(row_idx)
    if mismatches:
        raise ValueError(
            "visit_candidate_scores emitted_at_default does not reconstruct RFV slots "
            f"for {len(mismatches)} rows; first row ids={mismatches[:5]}"
        )

    print(
        "Candidate sidecar validation passed:",
        f"visits={len(df):,}",
        f"visit_rows={len(visit_candidate_scores):,}",
        f"segment_rows={len(segment_candidate_scores):,}",
    )


def write_classifier_candidate_sidecars(
    df: pd.DataFrame,
    *,
    cache: SegmentEmbedCache,
    resources: dict[str, Any],
    cfg: ClassifierConfig,
    output_dir: Path,
    visit_filename: str = "visit_candidate_scores.csv",
    segment_filename: str = "segment_candidate_scores.csv",
) -> dict[str, Path | pd.DataFrame]:
    """Write visit- and segment-level candidate score sidecars."""
    segment_candidate_scores = build_segment_candidate_scores(
        df,
        cache=cache,
        resources=resources,
        cfg=cfg,
    )
    visit_candidate_scores = aggregate_visit_candidate_scores(
        df,
        segment_candidate_scores,
        cfg=cfg,
    )
    validate_candidate_sidecars(
        df,
    ...
    visit_path = output_dir / visit_filename
    segment_path = output_dir / segment_filename
    visit_candidate_scores.to_csv(visit_path, index=False)
    segment_candidate_scores.to_csv(segment_path, index=False)
    return {
        "visit_path": visit_path,
        "segment_path": segment_path,
        "visit_candidate_scores": visit_candidate_scores,
        "segment_candidate_scores": segment_candidate_scores,
    }
...
def set_classifier_plot_style() -> None:
    """Apply the classifier notebook's compact figure style."""

    plt.rcParams.update(
        {
            "font.family": CLASSIFIER_FIGURE_STYLE["font_family"],
            "font.size": CLASSIFIER_FIGURE_STYLE["base_size"],
            "axes.titlesize": CLASSIFIER_FIGURE_STYLE["title_size"],
            "axes.labelsize": CLASSIFIER_FIGURE_STYLE["label_size"],
            "xtick.labelsize": CLASSIFIER_FIGURE_STYLE["tick_size"],
            "ytick.labelsize": CLASSIFIER_FIGURE_STYLE["tick_size"],
            "savefig.dpi": 300,
...
def annotate_horizontal_bar_counts(axis: plt.Axes) -> None:
    """Write count labels at the end of each horizontal bar."""

    for patch in axis.patches:
        width = float(patch.get_width())
        axis.text(
            width + max(axis.get_xlim()[1] * 0.01, 1.0),
            patch.get_y() + patch.get_height() / 2,
            f"{int(round(width)):,}",
            va="center",
            ha="left",
            fontsize=CLASSIFIER_FIGURE_STYLE["annotation_size"],
        )


set_classifier_plot_style()
...
def top_cc_plot(
    ...
    fig, axis = plt.subplots(figsize=(8.4, 10.0))
    axis.barh(counts.index, counts.values, color="#52606d", edgecolor="white", height=0.75)
    ...
    axis.set_xlim(0, float(counts.max()) * 1.12)
    annotate_horizontal_bar_counts(axis)
    plt.tight_layout()
    plt.show()
...
This section now groups helpers by domain so each block maps to one concern (I/O paths, missingness policy, QA audits, and contracts).

```{python}
#| label: hypercap-cc-nlp-classifier-cell-002b-paths
# Domain: workbook path and schema helpers.
# Core stage helpers remain embedded in this notebook by project policy.
...
def resolve_annotation_benchmark_input_path(
    work_dir: Path,
    *,
    cfg: ClassifierConfig,
) -> Path:
    """Resolve optional direct annotation-benchmark workbook path."""
    explicit_path = os.getenv(cfg.annotation_benchmark_path_env_var)
    rater_path = os.getenv("RATER_ANNOTATION_PATH")
    if explicit_path:
        return Path(explicit_path).expanduser().resolve()
    if rater_path:
        return Path(rater_path).expanduser().resolve()
    return (work_dir / cfg.annotation_benchmark_default_path).expanduser().resolve()


def get_annotation_benchmark_mode(cfg: ClassifierConfig) -> str:
    """Return validated annotation benchmark mode."""
    mode = os.getenv(cfg.annotation_benchmark_mode_env_var, "auto").strip().lower() or "auto"
    allowed = {"auto", "required", "off"}
    if mode not in allowed:
        raise ValueError(
            f"{cfg.annotation_benchmark_mode_env_var} must be one of {sorted(allowed)}, got {mode!r}."
        )
    return mode
...
def load_annotation_benchmark_frame(
    path: Path,
    *,
    cfg: ClassifierConfig,
) -> tuple[pd.DataFrame, str]:
    """Read and normalize the annotation workbook for direct classifier scoring."""
    sheet_name = os.getenv(
        cfg.annotation_benchmark_sheet_env_var,
        cfg.annotation_benchmark_default_sheet,
    ).strip() or cfg.annotation_benchmark_default_sheet
    frame = pd.read_excel(path, sheet_name=sheet_name, engine="openpyxl")
    frame.columns = frame.columns.str.strip()
    frame = assign_annotation_row_id(frame)

    explicit_cc_column = os.getenv(cfg.annotation_benchmark_cc_column_env_var, "").strip()
    if explicit_cc_column:
        ensure_required_columns(
            frame,
            [explicit_cc_column],
            context="annotation benchmark input",
        )
        cc_col = explicit_cc_column
    else:
        cc_col = get_cc_column(frame)
    return frame, cc_col
...
ENV_KNOBS: tuple[str, ...] = (
    "WORK_DIR",
    "CLASSIFIER_INPUT_FILENAME",
    "CLASSIFIER_ANNOTATION_BENCHMARK_MODE",
    "CLASSIFIER_ANNOTATION_BENCHMARK_PATH",
    "CLASSIFIER_ANNOTATION_BENCHMARK_SHEET",
    "CLASSIFIER_ANNOTATION_CC_COLUMN",
    "RATER_NLP_INPUT_FILENAME",
...
# Helper group: local contract-report helper.
# Classifier stage writes its own contract artifact without importing repo-local packages.
...
### 2B.1) Chief-complaint missingness policy helpers

```{python}
#| label: hypercap-cc-nlp-classifier-cell-002b-missingness
# Domain: chief-complaint missingness detection and deterministic uncodable policy.
...
### 2B.2) QA phrase/regression helpers

```{python}
#| label: hypercap-cc-nlp-classifier-cell-002b-qa
# Domain: phrase-level QA audits and guardrail checks.
...
def build_phrase_regression_audit(
    ...
    *,
    cc_column: str,
    cleaned_column: str = "cc_cleaned_str",
    segment_preds_column: str = "segment_preds",
    rfv_column: str = "RFV1_name",
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Audit phrase-level normalization regressions for high-risk sentinel complaints."""
    if cc_column not in df.columns:
        raise KeyError(f"Chief complaint column not found: {cc_column}")
    if cleaned_column not in df.columns:
        raise KeyError(f"Cleaned complaint column not found: {cleaned_column}")
    ...
    cleaned_cc = df[cleaned_column].fillna("").astype(str)
    rfv_name_columns = [
        column
        for column in df.columns
        if re.fullmatch(r"RFV\d+_name", str(column))
    ]
    if not rfv_name_columns:
        rfv_name_columns = [rfv_column]
    rfv_labels = df[rfv_name_columns].fillna("").astype(str)

    def _rows_with_any_label(pattern: str) -> pd.Series:
        return pd.DataFrame(
            {
                column: rfv_labels[column].str.contains(pattern, case=False, regex=True)
                for column in rfv_name_columns
            }
        ).any(axis=1)
    ...
    head_bleed_mask = raw_cc.str.contains(r"\bhead\s+bleed\b", case=False, regex=True)
    head_bleed_expected_mask = (
        df[rfv_column]
        .fillna("")
        .astype(str)
        .str.contains(
            r"Symptom\s+–\s+Nervous|Injuries\s+&\s+adverse\s+effects|Diseases\s+\(patient-stated\)",
            case=False,
            regex=True,
        )
    )
    head_bleed_misclassified_rows = int((head_bleed_mask & ~head_bleed_expected_mask).sum())

    upper_gi_bleed_mask = raw_cc.str.contains(r"\bupper\s+gi\s+bleed\b", case=False, regex=True)
    upper_gi_digestive_mask = (
        df[rfv_column]
        .fillna("")
        .astype(str)
        .str.contains(r"Symptom\s+–\s+Digestive", case=False, regex=True)
    )
    upper_gi_bleed_nondigestive_rows = int((upper_gi_bleed_mask & ~upper_gi_digestive_mask).sum())

    femur_variant_mask = raw_cc.str.contains(r"\b(?:femur|femer)\b", case=False, regex=True) & raw_cc.str.contains(
        r"\b(?:fx|fracture)\b",
        case=False,
        regex=True,
    )
    femur_injury_mask = (
        ...
        .fillna("")
        .astype(str)
        .str.contains(r"Injuries\s+&\s+adverse\s+effects", case=False, regex=True)
    )
    femur_variant_noninjury_rows = int((femur_variant_mask & ~femur_injury_mask).sum())

    bleed_token_truncation_rows = int(
        (
            raw_cc.str.contains(r"\bbleed\b", case=False, regex=True)
            & ~cleaned_cc.str.contains(r"\bbleed\b", case=False, regex=True)
            & cleaned_cc.str.contains(r"\bblee\b", case=False, regex=True)
        ).sum()
    )
    femer_to_fever_rows = int(
        (
            raw_cc.str.contains(r"\bfemer\b", case=False, regex=True)
            & ~cleaned_cc.str.contains(r"\bfemur\b", case=False, regex=True)
            & cleaned_cc.str.contains(r"\bfever\b", case=False, regex=True)
        ).sum()
    )
    parsed_payloads: list[list[dict[str, Any]]] = []
    unresolved_short_rows = 0
    unresolved_short_nonuncodable_rows = 0
    for payload_text in df[segment_preds_column].fillna("[]"):
        try:
            payload = json.loads(payload_text) if isinstance(payload_text, str) else []
        except json.JSONDecodeError:
            payload = []
        ...
        row_has_unresolved_short = False
        row_has_promoted_short = False
        for record in payload:
            if not isinstance(record, dict):
                continue
            segment_text = str(record.get("segment", "")).strip()
            predicted_code = str(record.get("code", "")).strip()
            override_code = record.get("rule_code")
            if _should_force_uncodable_segment(
                segment_text,
                override_code=str(override_code).strip() if override_code else None,
            ):
                row_has_unresolved_short = True
                if predicted_code != "RVC-UNCL":
                    row_has_promoted_short = True
        unresolved_short_rows += int(row_has_unresolved_short)
        unresolved_short_nonuncodable_rows += int(row_has_promoted_short)

    disease_mask = _rows_with_any_label(r"Diseases\s+\(patient-stated\)")
    circulatory_mask = _rows_with_any_label(r"Symptom\s+–\s+Circulatory")
    digestive_mask = _rows_with_any_label(r"Symptom\s+–\s+Digestive")
    gu_mask = _rows_with_any_label(r"Symptom\s+–\s+Genitourinary")
    skin_mask = _rows_with_any_label(r"Symptom\s+–\s+Skin/Hair/Nails")
    glycemia_mask = cleaned_cc.str.contains(
        r"\b(?:diabetic ketoacidosis|dka|hyperglyc(?:emia|emic)|hypoglyc(?:emia|emic))\b",
        case=False,
        regex=True,
    )
    glycemia_nondisease_rows = int((glycemia_mask & ~disease_mask).sum())
    abscess_mask = cleaned_cc.str.contains(
        r"\b(?:abscess|cellulitis|boil|furuncle|carbuncle)\b",
        case=False,
        regex=True,
    )
    abscess_nondisease_rows = int((abscess_mask & ~disease_mask).sum())
    lower_ext_swelling_mask = cleaned_cc.str.contains(RX_SWELLING_LOWER_EXT.pattern, case=False, regex=True)
    lower_ext_swelling_noncirculatory_rows = int(
        (lower_ext_swelling_mask & ~circulatory_mask).sum()
    )
    gu_swelling_mask = cleaned_cc.str.contains(RX_SWELLING_GU.pattern, case=False, regex=True)
    gu_swelling_nongu_rows = int((gu_swelling_mask & ~gu_mask).sum())
    facial_swelling_mask = cleaned_cc.str.contains(RX_SWELLING_FACIAL.pattern, case=False, regex=True)
    facial_swelling_nonskin_rows = int((facial_swelling_mask & ~skin_mask).sum())
    abdominal_swelling_mask = cleaned_cc.str.contains(
        RX_SWELLING_ABDOMINAL.pattern,
        case=False,
        regex=True,
    )
    abdominal_swelling_nondigestive_rows = int(
        (abdominal_swelling_mask & ~digestive_mask).sum()
    )
    ...
    summary = pd.DataFrame(
        [
            ...
            {"metric": "head_bleed_rows", "value": int(head_bleed_mask.sum())},
            {
                "metric": "head_bleed_misclassified_rows",
                "value": head_bleed_misclassified_rows,
            },
            {"metric": "upper_gi_bleed_rows", "value": int(upper_gi_bleed_mask.sum())},
            {
                "metric": "upper_gi_bleed_nondigestive_rows",
                "value": upper_gi_bleed_nondigestive_rows,
            },
            {"metric": "femur_variant_rows", "value": int(femur_variant_mask.sum())},
            {
                "metric": "femur_variant_noninjury_rows",
                "value": femur_variant_noninjury_rows,
            },
            {"metric": "bleed_token_truncation_rows", "value": bleed_token_truncation_rows},
            {"metric": "femer_to_fever_rows", "value": femer_to_fever_rows},
            {"metric": "unresolved_short_segment_rows", "value": int(unresolved_short_rows)},
            {
                "metric": "unresolved_short_segment_nonuncodable_rows",
                "value": unresolved_short_nonuncodable_rows,
            },
            {"metric": "glycemia_rows", "value": int(glycemia_mask.sum())},
            {"metric": "glycemia_nondisease_rows", "value": glycemia_nondisease_rows},
            {"metric": "abscess_rows", "value": int(abscess_mask.sum())},
            {"metric": "abscess_nondisease_rows", "value": abscess_nondisease_rows},
            {"metric": "lower_ext_swelling_rows", "value": int(lower_ext_swelling_mask.sum())},
            {
                "metric": "lower_ext_swelling_noncirculatory_rows",
                "value": lower_ext_swelling_noncirculatory_rows,
            },
            {"metric": "gu_swelling_rows", "value": int(gu_swelling_mask.sum())},
            {"metric": "gu_swelling_nongu_rows", "value": gu_swelling_nongu_rows},
            {"metric": "facial_swelling_rows", "value": int(facial_swelling_mask.sum())},
            {"metric": "facial_swelling_nonskin_rows", "value": facial_swelling_nonskin_rows},
            {"metric": "abdominal_swelling_rows", "value": int(abdominal_swelling_mask.sum())},
            {
                "metric": "abdominal_swelling_nondigestive_rows",
                "value": abdominal_swelling_nondigestive_rows,
            },
        ]
    )
...
def _classify_guardrail_phrase(
    phrase: str,
    *,
    resources: dict[str, Any],
    cfg: ClassifierConfig,
) -> tuple[str | None, list[str]]:
    normalized_segments, _ = normalize_cc(
        phrase,
        spell_mode="strict",
    )
    if not normalized_segments:
        return None, []
    empty_cache = SegmentEmbedCache(
        uniq_segments=[],
        seg2idx={},
        emb=torch.empty(0, 0),
        sim_proto=torch.empty(0, 0),
    ...
    visit_rfv = rfv_from_segment_preds(segment_predictions, max_rfv=cfg.max_rfv)
    if (
        not visit_rfv
        and cfg.allow_uncodable_primary
    ...
    primary_code = visit_rfv[0]["code"] if visit_rfv else None
    return primary_code, normalized_segments


def run_phrase_guardrail_suite(
    *,
    resources: dict[str, Any],
    cfg: ClassifierConfig,
) -> pd.DataFrame:
    """Evaluate sentinel phrase regressions against full classifier behavior."""
    cases: list[dict[str, str]] = [
        {"phrase": "HEAD BLEED", "expected_code": "RVC-SYM-NERV"},
        {"phrase": "Upper GI bleed", "expected_code": "RVC-SYM-DIG"},
        {"phrase": "FEMER FX", "expected_code": "RVC-INJ"},
        {"phrase": "s/p Fall, FEMER FX", "expected_code": "RVC-INJ"},
        {"phrase": "Femur fracture", "expected_code": "RVC-INJ"},
        {"phrase": "AAA", "expected_code": "RVC-DIS"},
        {"phrase": "ruptured AAA", "expected_code": "RVC-DIS"},
        {"phrase": "DKA", "expected_code": "RVC-DIS"},
        {"phrase": "hyperglycemia", "expected_code": "RVC-DIS"},
        {"phrase": "hypoglycemic", "expected_code": "RVC-DIS"},
        {"phrase": "PNX", "expected_code": "RVC-DIS"},
        {"phrase": "pleural effusion", "expected_code": "RVC-DIS"},
        {"phrase": "abscess", "expected_code": "RVC-DIS"},
        {"phrase": "facial swelling", "expected_code": "RVC-SYM-SKIN"},
        {"phrase": "testicular swelling", "expected_code": "RVC-SYM-GU"},
        {"phrase": "abdominal distention", "expected_code": "RVC-SYM-DIG"},
        {"phrase": "leg swelling", "expected_code": "RVC-SYM-CIRC"},
        {"phrase": "perforation", "expected_code": "RVC-UNCL"},
        {"phrase": "code", "expected_code": "RVC-UNCL"},
    ]
    rows: list[dict[str, Any]] = []
    for case in cases:
        predicted_code, normalized_segments = _classify_guardrail_phrase(
            case["phrase"],
            resources=resources,
            cfg=cfg,
        )
        rows.append(
            {
                ...
                "normalized_segments": "; ".join(normalized_segments),
                "passed": bool(predicted_code == case["expected_code"]),
            }
        )
    return pd.DataFrame(rows)
```

### 2B.3) Resource + contract helpers

```{python}
#| label: hypercap-cc-nlp-classifier-cell-002b-contract
# Domain: resource integrity checks and classifier output contracts.
...
def read_excel_with_sha256(
    ...
    """Hash the exact input bytes parsed, not a potentially replaced later file."""
...
early_annotation_benchmark_mode = get_annotation_benchmark_mode(cfg)
early_annotation_benchmark_path = resolve_annotation_benchmark_input_path(WORK_DIR, cfg=cfg)
if early_annotation_benchmark_mode == "required" and not early_annotation_benchmark_path.exists():
    raise FileNotFoundError(
        "CLASSIFIER_ANNOTATION_BENCHMARK_MODE=required but annotation benchmark workbook "
        f"was not found at {early_annotation_benchmark_path}. Restore the annotation workbook or set "
        f"{cfg.annotation_benchmark_path_env_var}."
    )
...
## 5) Prototype loading, deduplication, and embedding preparation
...
## 6) Segment cache build and classification
...
## 7) Output shaping and compatibility mapping

Standardized output columns: - `RFV1..RFV5` (legacy short-code
compatibility) - `RFV1_name..RFV5_name` - `RFV1_support..RFV5_support` -
`RFV1_sim..RFV5_sim` - `segment_preds`
...
bleed_token_truncation_rows = int(
    phrase_audit_summary.loc[
        phrase_audit_summary["metric"].eq("bleed_token_truncation_rows"), "value"
    ].iloc[0]
)
if bleed_token_truncation_rows > 0:
    raise AssertionError(
        "Detected bleed-token truncation regression: "
        f"{bleed_token_truncation_rows} rows replaced 'bleed' with truncated forms."
    )

femer_to_fever_rows = int(
    phrase_audit_summary.loc[
        phrase_audit_summary["metric"].eq("femer_to_fever_rows"), "value"
    ].iloc[0]
)
if femer_to_fever_rows > 0:
    raise AssertionError(
        "Detected femur typo normalization regression: "
        f"{femer_to_fever_rows} rows mapped 'femer' to 'fever'."
    )

targeted_phrase_audit_metrics = {
    "unresolved_short_segment_nonuncodable_rows": "Detected unresolved short-token rows promoted above uncodable.",
    "glycemia_nondisease_rows": "Detected glycemia/metabolic complaints routed outside Diseases (patient-stated).",
    "abscess_nondisease_rows": "Detected abscess/cellulitis complaints routed outside Diseases (patient-stated).",
    "lower_ext_swelling_noncirculatory_rows": "Detected lower-extremity swelling complaints routed outside Symptom – Circulatory.",
    "gu_swelling_nongu_rows": "Detected genitourinary swelling complaints routed outside Symptom – Genitourinary.",
    "facial_swelling_nonskin_rows": "Detected facial/periorbital swelling complaints routed outside Symptom – Skin/Hair/Nails.",
    "abdominal_swelling_nondigestive_rows": "Detected abdominal distention/swelling complaints routed outside Symptom – Digestive.",
}
for metric_name, message in targeted_phrase_audit_metrics.items():
    metric_value = int(
        phrase_audit_summary.loc[
            phrase_audit_summary["metric"].eq(metric_name), "value"
        ].iloc[0]
    )
    if metric_value > 0:
        raise AssertionError(f"{message} Count={metric_value}.")

phrase_guardrail_suite = run_phrase_guardrail_suite(resources=resources, cfg=cfg)
print("\nPhrase guardrail suite:")
print(phrase_guardrail_suite.to_string(index=False))
if not bool(phrase_guardrail_suite["passed"].fillna(False).all()):
    failed_phrase_suite = phrase_guardrail_suite.loc[
        ~phrase_guardrail_suite["passed"].fillna(False)
    ]
    raise AssertionError(
        "Phrase guardrail suite failed. "
        f"Unexpected codes for sentinel phrases: {failed_phrase_suite.to_dict(orient='records')}"
    )
...
candidate_sidecars = write_classifier_candidate_sidecars(
    df_out,
    cache=cache,
    resources=resources,
    cfg=cfg,
    output_dir=canonical_output_path.parent,
)
...
print(f"Wrote visit candidate sidecar: {candidate_sidecars['visit_path']}")
print(f"Wrote segment candidate sidecar: {candidate_sidecars['segment_path']}")

annotation_benchmark_status: dict[str, Any] = {
    "mode": get_annotation_benchmark_mode(cfg),
    "status": "not_run",
    "input_path": None,
    "output_path": None,
    "visit_candidate_scores_path": None,
    "segment_candidate_scores_path": None,
    "rows": 0,
    "chief_complaint_column": None,
    "sheet_name": os.getenv(
        cfg.annotation_benchmark_sheet_env_var,
        cfg.annotation_benchmark_default_sheet,
    ).strip()
    or cfg.annotation_benchmark_default_sheet,
}
annotation_benchmark_path = resolve_annotation_benchmark_input_path(WORK_DIR, cfg=cfg)
annotation_benchmark_status["input_path"] = str(annotation_benchmark_path)

if annotation_benchmark_status["mode"] == "off":
    annotation_benchmark_status["status"] = "skipped_off"
elif not annotation_benchmark_path.exists():
    annotation_benchmark_status["status"] = "missing_input"
    message = (
        "Annotation benchmark workbook not found at "
        f"{annotation_benchmark_path}; direct annotation scoring skipped in auto mode."
    )
    if annotation_benchmark_status["mode"] == "required":
        raise FileNotFoundError(
            message
            + " Restore the annotation workbook or set "
            f"{cfg.annotation_benchmark_path_env_var}."
        )
    print(message)
else:
    annotation_source_df, annotation_cc_column = load_annotation_benchmark_frame(
        annotation_benchmark_path,
        cfg=cfg,
    )
    annotation_benchmark_status["chief_complaint_column"] = annotation_cc_column
    annotation_scoring_df = annotation_source_df[["annotation_row_id", annotation_cc_column]].copy()
    annotation_scoring_df = annotate_cc_missingness(
        annotation_scoring_df,
        cc_column=annotation_cc_column,
    )
    annotation_normalization_payload, annotation_spell_mode_scores = choose_spell_mode(
        annotation_scoring_df,
        cc_source_column="cc_text_for_nlp",
        requested_mode=requested_spell_mode,
    )
    annotation_scoring_df = pd.concat(
        [annotation_scoring_df, annotation_normalization_payload["frame"]],
        axis=1,
    )
    annotation_scoring_df["cc_spell_mode_selected"] = annotation_normalization_payload["selected_mode"]
    annotation_cache = build_segment_cache(
        df=annotation_scoring_df,
        seg_col="cc_cleaned",
        resources=resources,
        batch_size=cfg.segment_batch_size,
    )
    annotation_out = assign_rvc_per_segment_and_visit_cached(
        df=annotation_scoring_df,
    ...
    annotation_out = apply_pseudomissing_uncodable_policy(
        annotation_out,
        max_rfv=cfg.max_rfv,
        pseudo_flag_column="cc_pseudomissing_flag",
    )
    annotation_out = apply_blank_primary_uncodable_policy(
        annotation_out,
        max_rfv=cfg.max_rfv,
        missing_reason_column="cc_missing_reason",
    )
    ...
    annotation_sidecars = write_classifier_candidate_sidecars(
        annotation_out,
        cache=annotation_cache,
        resources=resources,
        cfg=cfg,
        output_dir=canonical_output_path.parent,
        visit_filename=cfg.annotation_visit_candidate_scores_filename,
        segment_filename=cfg.annotation_segment_candidate_scores_filename,
    )
    annotation_output_path = canonical_output_path.parent / cfg.annotation_benchmark_output_filename
    annotation_export_keep_columns = [
        "annotation_row_id",
        annotation_cc_column,
        "chief_complaint_raw",
        "cc_missing_reason",
        "cc_cleaned_str",
        "segment_preds",
    ...
    for slot in range(1, cfg.max_rfv + 1):
        annotation_export_keep_columns.extend(
            [
                f"RFV{slot}",
                f"RFV{slot}_name",
                f"RFV{slot}_support",
    ...
    annotation_export = annotation_out[
        [column for column in annotation_export_keep_columns if column in annotation_out.columns]
    ].copy()
    annotation_export.to_excel(
        annotation_output_path,
        index=False,
        sheet_name=cfg.output_sheet_name,
    )
    annotation_benchmark_status.update(
        {
            "status": "written",
            "output_path": str(annotation_output_path),
            "visit_candidate_scores_path": str(annotation_sidecars["visit_path"]),
            "segment_candidate_scores_path": str(annotation_sidecars["segment_path"]),
            "rows": int(len(annotation_export)),
            "spell_mode_selected": annotation_normalization_payload["selected_mode"],
            "spell_mode_comparison_rows": int(len(annotation_spell_mode_scores)),
        }
    )
    print(
        "Annotation benchmark export:",
        f"rows={len(annotation_export):,}",
        f"path={annotation_output_path}",
    )

run_date = pd.Timestamp.now().strftime("%Y-%m-%d")
results_date = os.getenv("RESULTS_DATE", run_date).strip() or run_date
...
phrase_guardrail_path = archive_output_path.parent / f"{run_date} classifier_phrase_guardrail_cases.csv"
phrase_guardrail_suite.to_csv(phrase_guardrail_path, index=False)
    ...
    [
        {
            ...
            "pco2_threshold_0_24h_positive": int(
                pd.to_numeric(df_out.get("pco2_threshold_0_24h", 0), errors="coerce").fillna(0).astype(int).sum()
            ),
...
classifier_supplement_tables_path = results_dir / "NLP_Classifier_Supplement_Tables.xlsx"
classifier_config_supplement = pd.DataFrame(
    [
        {"parameter": "model_name", "value": cfg.model_name},
        {"parameter": "model_trust_remote_code", "value": cfg.model_trust_remote_code},
        {"parameter": "spacy_model", "value": metadata.get("spacy_model")},
        {"parameter": "device", "value": metadata.get("device")},
        {"parameter": "scoring_method", "value": cfg.scoring_method},
        {"parameter": "abstain_threshold", "value": cfg.abstain_threshold},
        {"parameter": "max_rfv", "value": cfg.max_rfv},
        {"parameter": "max_segments_per_visit", "value": cfg.max_segments_per_visit},
        {"parameter": "model_batch_size", "value": cfg.model_batch_size},
        {"parameter": "segment_batch_size", "value": cfg.segment_batch_size},
        {"parameter": "spell_correction_requested", "value": normalization_payload["requested_mode"]},
        {"parameter": "spell_correction_selected", "value": normalization_payload["selected_mode"]},
        {"parameter": "appendix_relpath", "value": cfg.appendix_relpath},
        {"parameter": "summary_relpath", "value": cfg.summary_relpath},
    ]
)
prototype_count_by_group = metadata.get("prototype_count_by_group", {})
prototype_supplement = pd.DataFrame(
    [
        {
            "rvc_code": code,
            "rvc_label": RVC_NAME[code],
            "precedence_rank": int(PRECEDENCE_RVC.index(code) + 1),
            "prototype_count": int(prototype_count_by_group.get(code, 0)),
...
normalization_resources_supplement = pd.DataFrame(
    [
        {"resource": "segment_substitutions", "count": len(SEG_OVR), "notes": "Whole-segment abbreviation expansions applied before token expansion."},
        {"resource": "token_substitutions", "count": len(TOK_OVR), "notes": "Token-level abbreviation expansions for longer presenting-concern segments."},
        {"resource": "context_substitutions", "count": len(CTX_OVR), "notes": "Context-gated short-token expansions."},
        {"resource": "rule_gate_regexes", "count": len(RULE_RX), "notes": "Deterministic high-confidence RFV overrides used before threshold abstention."},
        {"resource": "protected_slash_tokens", "count": len(PROTECT), "notes": "Slash-delimited tokens protected during early segmentation."},
        {"resource": "spell_protect_phrases", "count": len(SPELL_PROTECT_PHRASES), "notes": "Phrase-level spell-correction exclusions."},
        {"resource": "lemma_protect_phrases", "count": len(LEMMA_PROTECT_PHRASES), "notes": "Phrase-level lemmatization exclusions."},
        {"resource": "spell_protect_tokens", "count": len(SPELL_PROTECT_TOKENS), "notes": "Token-level spell-correction exclusions."},
        {"resource": "spell_denylist_substitutions", "count": len(SPELL_DENYLIST_SUBSTITUTIONS), "notes": "Known unsafe spell-correction substitutions that are rejected."},
    ]
)
classifier_example_inputs = [
...
classifier_example_rows: list[dict[str, Any]] = []
for raw_example in classifier_example_inputs:
    normalized_segments, spellfix_log = normalize_cc(
        raw_example,
        spell_mode=normalization_payload["selected_mode"],
    )
    segment_predictions = classify_segments_cached(
        normalized_segments,
        cache=cache,
    ...
    visit_rfv = rfv_from_segment_preds(segment_predictions, max_rfv=cfg.max_rfv)
    if not visit_rfv and cfg.allow_uncodable_primary and segment_predictions:
        uncodable_predictions = [prediction for prediction in segment_predictions if prediction.code == "RVC-UNCL"]
        if uncodable_predictions:
            chosen = max(uncodable_predictions, key=lambda prediction: (prediction.sim, -prediction.seg_idx))
            visit_rfv = [
                {
                    "code": "RVC-UNCL",
                    "name": RVC_NAME["RVC-UNCL"],
    ...
    classifier_example_rows.append(
        {
            "example_source": "synthetic_publication_example",
            "raw_presenting_concern": raw_example,
            "normalized_segments": "; ".join(normalized_segments),
            "spellfix_events_n": int(len(spellfix_log)),
            "segment_predictions": "; ".join(
                f"{prediction.segment} -> {prediction.name}"
                for prediction in segment_predictions
            ),
            "final_rfv_labels": "; ".join(str(item["name"]) for item in visit_rfv),
            "final_rfv_codes": "; ".join(str(item["code"]) for item in visit_rfv),
        }
    )
classifier_examples_supplement = pd.DataFrame(classifier_example_rows)
with pd.ExcelWriter(classifier_supplement_tables_path, engine="openpyxl") as writer:
    classifier_config_supplement.to_excel(writer, sheet_name="classifier_config", index=False)
    prototype_supplement.to_excel(writer, sheet_name="prototype_metadata", index=False)
    normalization_resources_supplement.to_excel(writer, sheet_name="normalization_resources", index=False)
    phrase_guardrail_suite.to_excel(writer, sheet_name="guardrail_examples", index=False)
    classifier_examples_supplement.to_excel(writer, sheet_name="classification_examples", index=False)
print(f"Wrote classifier supplement tables: {classifier_supplement_tables_path}")
...
stage_manifest["results_date"] = results_date
stage_manifest["inputs"] = {
    "cohort_path": _manifest_relpath(file_path),
...
stage_manifest["scoring_config"] = {
    "scoring_method": cfg.scoring_method,
    "abstain_threshold": cfg.abstain_threshold,
}
stage_manifest["resource_verification"] = resource_report
stage_manifest["outputs"] = {
    "canonical_output_path": _manifest_relpath(canonical_output_path),
    "canonical_output_sha256": canonical_output_sha256,
    "visit_candidate_scores_path": _manifest_relpath(candidate_sidecars["visit_path"]),
    "segment_candidate_scores_path": _manifest_relpath(candidate_sidecars["segment_path"]),
    "annotation_benchmark_output_path": _manifest_relpath(annotation_benchmark_status.get("output_path")),
    "annotation_visit_candidate_scores_path": _manifest_relpath(
        annotation_benchmark_status.get("visit_candidate_scores_path")
    ),
    "annotation_segment_candidate_scores_path": _manifest_relpath(
        annotation_benchmark_status.get("segment_candidate_scores_path")
    ),
    "archive_output_path": _manifest_relpath(archive_output_path) if write_archive_xlsx else None,
    "archive_output_sha256": archive_output_sha256,
    "cc_missing_audit_path": _manifest_relpath(cc_missing_audit_path),
    "phrase_audit_path": _manifest_relpath(phrase_audit_path),
    "phrase_guardrail_path": _manifest_relpath(phrase_guardrail_path),
    "spellfix_guardrail_audit_path": _manifest_relpath(spellfix_guardrail_audit_path),
    "spell_mode_comparison_path": _manifest_relpath(spell_mode_comparison_path),
    "spellfix_log_path": _manifest_relpath(spellfix_log_path),
    "hypercap_flags_audit_path": _manifest_relpath(hypercap_flags_audit_path),
    "classifier_contract_report_path": _manifest_relpath(contract_report_path),
    "nlp_classifier_supplement_tables_path": _manifest_relpath(classifier_supplement_tables_path),
}
stage_manifest["annotation_benchmark"] = annotation_benchmark_status
stage_manifest = sanitize_manifest_payload(stage_manifest, work_dir=WORK_DIR)
stage_manifest_path = archive_output_path.parent / f"{results_date} classifier_run_manifest.json"
...
print(f"Wrote phrase guardrail suite: {phrase_guardrail_path}")
