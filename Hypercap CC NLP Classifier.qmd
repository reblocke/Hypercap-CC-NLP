...
RULE_RX: dict[str, re.Pattern[str]] = {
    "RVC-INJ": re.compile(
        r"\b(mvc|mva|collision|ped(\s|)vs(\s|)auto|assault|fell|fall|gsw|gunshot|stab|laceration|fracture|fx\b|burn|foreign body|poison(ing)?|overdose|od\b|adverse (drug|medication) reaction)\b",
        re.I,
    ),
...
def _phrase_primary_rule_code(phrase: str) -> tuple[str | None, list[str]]:
    ...
    for segment in normalized_segments:
        matched_code, _ = rule_gate(str(segment))
        if matched_code is not None:
            matched_codes.append(matched_code)
    if not matched_codes:
        return None, normalized_segments
    for preferred_code in PRECEDENCE_RVC:
        if preferred_code in matched_codes:
            return preferred_code, normalized_segments
    return matched_codes[0], normalized_segments


def run_phrase_guardrail_suite() -> pd.DataFrame:
    """Evaluate sentinel phrase regressions and expected deterministic rule outputs."""
    ...
    rows: list[dict[str, Any]] = []
    for case in cases:
        predicted_code, normalized_segments = _phrase_primary_rule_code(case["phrase"])
        rows.append(
            {
                "phrase": case["phrase"],
...
phrase_guardrail_suite = run_phrase_guardrail_suite()
