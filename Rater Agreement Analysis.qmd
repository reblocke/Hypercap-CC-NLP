...
- **Canonical outputs:** artifacts in `annotation_agreement_outputs_nlp/` (join audit, set/binary metrics, manifests).
...
5. **Audit outputs**: writes CSV/TXT artifacts in `annotation_agreement_outputs_nlp/` for transparent review.
...
def select_join_key_columns(
    ...
    for key_cols, strategy in preferred_candidates:
        if not set(key_cols).issubset(df_r3.columns) or not set(key_cols).issubset(df_nlp.columns):
            continue
