...
def resolve_rater_candidate_sidecar_path(
    work_dir: Path,
    filename: str | None,
    default_filename: str,
) -> Path:
    resolved_filename = filename or default_filename
    input_path = (work_dir / "MIMIC tabular data" / resolved_filename).expanduser().resolve()
    if not input_path.exists():
        raise FileNotFoundError(
            "Expected classifier candidate sidecar was not found at "
            f"{input_path}. Run the classifier notebook first or set the "
            "corresponding RATER_*_CANDIDATE_SCORES_FILENAME override."
        )
    return input_path
...
visit_candidate_scores_path = resolve_rater_candidate_sidecar_path(
    WORK_DIR,
...
segment_candidate_scores_path = resolve_rater_candidate_sidecar_path(
    WORK_DIR,
...
for path in (ANNOTATION_PATH, NLP_PATH, VISIT_CANDIDATE_SCORES_PATH, SEGMENT_CANDIDATE_SCORES_PATH):
    if not path.exists():
...
def _prediction_sets_from_visit_candidates(
    ...
    *,
    ...
    for matched_visit_id, group in visit_scores.groupby("matched_visit_id", dropna=False):
        ...
        prediction_lookup[int(matched_visit_id)] = set(ranked.head(top_k_cutoff)["category_code"].astype(str))
...
nlp_present_keys = [column for column in nlp_key_candidates if column in df_nlp.columns]
...
r3_join_keys, join_key_strategy, join_key_diagnostics = select_join_key_columns(df_r3, df_nlp)
...
df_join, unmatched_adjudicated, unmatched_nlp, join_audit = build_r3_nlp_join_audit(
    ...
    key_diagnostics=join_key_diagnostics,
)
...
nlp_benchmark_denominator_text = (
    ...
    f"available join keys in both sources (N={n_after})."
)
...
summary_lines.append("All threshold summaries use the matched adjudicated sample only.")
